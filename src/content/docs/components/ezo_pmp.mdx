...
will fail. When using the [Dose Continuously](#ezo_pmp-dose_continuously-action) Action, this is the volume the pump will
dose every minute. This value will get updated after the pump is calibrated (see [`ezo_pmp.set_calibration_volume` Action](#ezo_pmp-set_calibration_volume-action)).
