...
The `exposure_notifications` component uses the [Esp32 Ble Tracker](/components/esp32_ble_tracker/) to discover
...
- **on_exposure_notification** (**Required**, [Automation](/automations)): An automation
  to run when an exposure notification bluetooth message is received.

  A variable `x` of type <APIStruct text="exposure_notifications::ExposureNotification" path="exposure_notifications::ExposureNotification" /> is passed to the automation.
...
- <APIRef text="exposure_notifications.h" path="exposure_notifications/exposure_notifications.h" />
