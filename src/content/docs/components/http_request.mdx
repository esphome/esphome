...
- All other options from [`http_request.post` Action](#http_request-post-action) and [`http_request.get` Action](#http_request-get-action).

## Triggers

<span id="http_request-on_response"></span>
...
- `body` as `std::string` which contains the response body when `capture_response`
  (see [`http_request.get` Action](#http_request-get-action)) is set to `true`.
...
<span id="http_request-on_error"></span>
...
- [json Component](/components/json/)
- <APIRef text="http_request.h" path="http_request/http_request.h" />
