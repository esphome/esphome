...
interval, or the `update_when_display_idle` option used, or make use of the [`on_draw_end`](#on_draw_end) trigger to manually update the display.
...
<Image src="/images/lvgl-hello.png" width={800} height={652} layout="constrained" alt="" />
...
- **disp_bg_color** (*Optional*, [color](#lvgl-color)): **Deprecated** — use `bottom_layer` with `bg_color` instead. Solid color used to fill the background.
- **disp_bg_image** (*Optional*, [image](/components/image#display-image)): **Deprecated** — use `bottom_layer` with `bg_image_src` instead.
- **disp_bg_opa** (*Optional*, [opacity](#lvgl-opacity)): **Deprecated** — use `bottom_layer` with `bg_opa` instead.
...
- **transform_scale_x** (*Optional*, 0.1-10): Horizontal transformation scale.
- **transform_scale_y** (*Optional*, 0.1-10): Vertical transformation scale.
- **transform_skew_x** (*Optional*, 0-360): Horizontal skew transformation angle.
- **transform_skew_y** (*Optional*, 0-360): Vertical skew transformation angle.
...
- **dither** (*Optional*, string): **Deprecated** — dithering is not supported in LVGL 9.x and will be ignored.
...
This [action](/automations/actions#actions-action) allows changing/updating the background styling of the LVGL display layers at runtime. You can style the `top_layer` and/or `bottom_layer` using any [style properties](#lvgl-styling). The deprecated `disp_bg_color`, `disp_bg_image` and `disp_bg_opa` options are also still accepted but will be mapped to `bottom_layer` styling.
