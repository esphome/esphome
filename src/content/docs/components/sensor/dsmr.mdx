...
import dsmrRequestPinCircuitExampleImg from './images/dsmr-request-pin-circuit-example.png';
...
dsmr:
  ...
  # You must set it to false if:
  #  * You have an old DSMR version that does not provide a CRC.
  #  * You have a smart meter that uses an incorrect algorithm to calculate the CRC. For example: EON Hungary.
  crc_check: true
...
sensor:
  - platform: dsmr
    ...
    # Don't use this sensor if you have an EON Hungary smart meter because it uses an incorrect format for that field.
...
<span id="sensor-dsmr-request_pin"></span>
...
<Image src={dsmrRequestPinCircuitExampleImg} layout="constrained" alt="" />
...
Log the raw input data and enable very verbose logging.

If you have problems receiving data, add these configuration options to the `logger` and `uart` components:
...
- <APIRef text="dsmr.h" path="dsmr/dsmr.h" />
