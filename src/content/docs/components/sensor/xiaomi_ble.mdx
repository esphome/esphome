...
New firmware requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)), and stopped broadcasting battery level.
...
(see [Obtaining the Bindkey](#obtaining-the-bindkey)). Unencrypted communication is supported only in custom firmware, but it's not recommended for obvious security reasons.
...
If you enable "Encrypted beacon" in the PVVX firmware, specify the `bindkey` with the value you see when you press the "Get BindKey" button in Telink Flasher (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
Hygro thermometer, small square body, e-ink display, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
Hygro thermometer, square body, e-ink display, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
1. Xiaomi stock firmware (requires a bindkey in order to decrypt the received data, see [Obtaining the Bindkey](#obtaining-the-bindkey))
...
Cleargrass (Qingping) alarm clock, segment LCD, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
Cleargrass (Qingping) Temp & RH Lite, round body, segment LCD, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
Qingping motion & ambient light sensor. Broadcasts motion detection, idle time since last motion event, lux value and battery status. Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
...
Requires a bindkey in order to decrypt the received data (see [Obtaining the Bindkey](#obtaining-the-bindkey)).
