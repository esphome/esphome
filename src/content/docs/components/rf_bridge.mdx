...
If you have *Portisch* or *Mightymos* firmware installed, these raw codes can be obtained with the help of [`rf_bridge.start_bucket_sniffing` Action](#rf_bridge-start_bucket_sniffing-action)
...
The raw data will be available in the log and can later be used with [`rf_bridge.send_raw` Action](#rf_bridge-send_raw-action) action.
