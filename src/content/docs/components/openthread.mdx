...
- **output_power** (*Optional*, integer): The amount of TX power for the Thread 802.15.4 radio in dBm.
  ...
  limits.
- **poll_period** (*Optional*, [Time](/guides/configuration-types#config-time)): When set on an MTD device, the parent
...
- **poll_period** (**Required**, [Time](/guides/configuration-types#config-time)): The poll period to apply, e.g. the
