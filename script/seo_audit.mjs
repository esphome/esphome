...
function analyzeIssues(local, live) {
  ...
  if (local.descriptionLength < 50 && local.description)
    issues.push(`Description too short (${local.descriptionLength} chars)`);
  ...
  if (live) {
    ...
    if (local.description && live.description && local.description !== live.description) {
...
async function main() {
  ...
  try {
    ...
    const robotsLive = await fetch(`${LIVE_SITE}/robots.txt`)
      .then((r) => r.text())
      .catch(() => null);
  ...
  try {
    ...
    const sitemapLive = await fetch(`${LIVE_SITE}/sitemap.xml`)
      .then((r) => (r.ok ? r.text() : null))
      .catch(() => null);

    console.log(
      `sitemap.xml: ${sitemapLocal ? "✓ Present" : "✗ Missing"} (local) | ${sitemapLive ? "✓ Present" : "✗ Missing"} (live)`,
    );
