...
def override_manifest(manifest: ComponentManifestOverride) -> None:
    ...
    # can't run (same root cause as the hub's own test stub, see
    # tests/components/mk2pvrouter/__init__.py). All the gtests need from
    # codegen here is MK2PVROUTER_LISTENER_COUNT being defined (>=1), so stub
    # to_code to build standalone IDs and call register_mk2pvrouter_listener()
    # directly. The dummy hub is never exercised at runtime (this build's
    # setup() is never called; gtests construct the C++ classes directly), so
    # it doesn't need to be a "real", config-resolved hub instance.
    #
    # ID(None, ...) objects compare/hash equal regardless of type (ID.__eq__
    # only looks at .id), so a second auto-generated ID here would collide
    # with the hub's own ID(None, ...) from tests/components/mk2pvrouter/
    # __init__.py's to_code_testing -- explicit id strings avoid that.
