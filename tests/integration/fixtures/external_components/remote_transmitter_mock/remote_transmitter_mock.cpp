...
void MockRemoteTransmitter::flush_pending_completion_() {
  ...
  ESP_LOGW(TAG, "Overlap: a frame was requested while seq=%" PRIu32 " was in flight", this->seq_);
...
void MockRemoteTransmitter::send_internal(uint32_t send_times, uint32_t send_wait) {
  ...
  this->busy_ = true;
  this->seq_++;
  ESP_LOGI(TAG, "TX seq=%" PRIu32 " timings=%zu repeat=%" PRIu32 " duration=%" PRIu32 "ms", this->seq_,
...
void MockRemoteTransmitter::finish_() {
  this->busy_ = false;
  ESP_LOGI(TAG, "Complete seq=%" PRIu32, this->seq_);
