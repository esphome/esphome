...
def handle_http_redirect(http_request_future, final_request_future, http_port: int):
    async def handler(reader, writer):
        try:
            async with asyncio.timeout(1.0):
            ...
            if (
                ...
                http_response = [
                    ...
                    f"Location: http://127.0.0.1:{http_port}/final.bmp".encode(),
...
async def test_online_image_auto_detects_redirected_image_bmp_mime(
    ...
    http_server_port = 0
    server = await asyncio.start_server(
        handle_http_redirect(http_request_future, final_request_future, 0),
        "127.0.0.1",
        0,
    )
    http_server_port = server.sockets[0].getsockname()[1]
    # Rebind the redirect helper with the actual port after the listener is assigned.
    server.close()
    server = await asyncio.start_server(
        handle_http_redirect(
            http_request_future, final_request_future, http_server_port
        ),
        "127.0.0.1",
        http_server_port,
    )
