...
The mirror is purely a drift guard now: the esp32 schema builder lazily
imports esp32_ble and asserts the two values agree, but that assert only
fires while building the esp32 schema. This test catches drift when the
upstream constant changes without any esp32 config being validated.
"""
