...
def _register_colliding_entities(
    validator: Callable[[ConfigType], ConfigType], *configs: ConfigType
) -> None:
    """Register entities whose names collide on the sanitized object_id.

    While collision-safe unique ids are disabled (USE_COLLISION_SAFE_UNIQUE_IDS,
    see entity_base.cpp), entity_duplicate_validator() rejects these collisions
    itself before the mqtt final-validate check could see them; testing_mode
    bypasses that rejection but still populates the object_id registry the
    checks under test read.
    """
