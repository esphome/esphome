...
@pytest.mark.parametrize(
    "restore_mode", ["RESTORE_DEFAULT_INITIAL_STATE", "ALWAYS_INITIAL_STATE"]
)
def test_initial_state_dependent_modes_require_initial_state(
    restore_mode: str,
) -> None:
    config = _base_config()
...
def test_initial_state_dependent_modes_require_state_key(
    restore_mode: str,
) -> None:
    config = _base_config()
    config["restore_mode"] = restore_mode
...
def test_initial_state_dependent_modes_with_initial_state_is_valid(
    restore_mode: str,
) -> None:
    config = _base_config()
    config["restore_mode"] = restore_mode
