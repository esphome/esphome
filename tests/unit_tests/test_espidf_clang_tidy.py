...
def test_setup_core_sets_arduino_env(
    ...
    """_setup_core sets USE_ARDUINO_COMPONENT, which gates arduino-only manifest deps."""
    ...
    monkeypatch.delenv("USE_ARDUINO_COMPONENT", raising=False)

    _setup_core(tmp_path / "proj", _settings(target_framework=target_framework))

    assert os.environ["USE_ARDUINO_COMPONENT"] == expected
