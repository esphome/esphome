...
async def test_add_includes_with_single_file(
    ...
    """Test add_includes with a single header file."""
    ...
    with patch("esphome.core.config.include_file") as mock_include_file:
        ...
        mock_include_file.assert_called_once_with(str(include_file), "my_header.h")


@pytest.mark.asyncio
async def test_add_includes_with_directory(tmp_path: Path) -> None:
    """Test add_includes with a directory containing multiple files."""
    CORE.config_path = str(tmp_path / "config.yaml")
    CORE.build_path = str(tmp_path / "build")
    ...
    (include_dir / "README.md").write_text("# Documentation")  # Should be ignored
    ...
    with patch("esphome.core.config.include_file") as mock_include_file:
        ...
        # Should include all valid files recursively
        calls = mock_include_file.call_args_list
        included_files = {call[0][1] for call in calls}

        assert "includes/header1.h" in included_files
        assert "includes/header2.hpp" in included_files
        assert "includes/source.cpp" in included_files
        assert "includes/subdir/nested.h" in included_files
        # README.md should not be included
        assert all("README.md" not in call[0][1] for call in calls)


@pytest.mark.asyncio
async def test_add_includes_with_multiple_sources(tmp_path: Path) -> None:
    ...
    with patch("esphome.core.config.include_file") as mock_include_file:
        ...
        assert "single.h" in included_files
        assert "dir1/file1.h" in included_files
        assert "dir2/file2.cpp" in included_files


@pytest.mark.asyncio
async def test_add_includes_empty_directory(tmp_path: Path) -> None:
    """Test add_includes with an empty directory."""
    CORE.config_path = str(tmp_path / "config.yaml")
    CORE.build_path = str(tmp_path / "build")
    ...
    with patch("esphome.core.config.include_file") as mock_include_file:
        ...
        # Should not call include_file for empty directory
        mock_include_file.assert_not_called()


@pytest.mark.asyncio
async def test_add_includes_preserves_directory_structure(tmp_path: Path) -> None:
    ...
    with patch("esphome.core.config.include_file") as mock_include_file:
        ...
        calls = mock_include_file.call_args_list
        basenames = {call[0][1] for call in calls}

        assert "lib/src/core.h" in basenames
        assert "lib/utils/helper.h" in basenames
