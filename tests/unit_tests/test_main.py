...
def mock_subprocess_call() -> Generator[Mock]:
    ...
    with patch("esphome.components.esphome.publish.subprocess.call") as mock:
        yield mock
...
def test_upload_program_publish_command_fails(
    ...
    """Test upload_program with PUBLISH host raises EsphomeError on non-zero exit code."""
    setup_core(platform=PLATFORM_ESP32, tmp_path=tmp_path)
    ...
    with pytest.raises(EsphomeError, match="Publish command exited with code 1"):
        upload_program(config, MockArgs(), ["PUBLISH"])
