...
from esphome.dashboard.entries import EntryState, EntryStateSource, ReachableState, bool_to_entry_state
...
class _FakeEntries:
    ...
    def set_state_if_online_or_source(self, entry: _FakeEntry, state: EntryState) -> None:
...
def test_extract_name_from_status_topic() -> None:
    assert MqttStatusThread._extract_name_from_status_topic("test-node/status") == "test-node"
    assert MqttStatusThread._extract_name_from_status_topic("esphome/discover/test-node") is None
...
def test_mqtt_status_online_is_sticky_against_periodic_offline_reset() -> None:
    ...
    entries.set_state_if_online_or_source(entry, bool_to_entry_state(True, EntryStateSource.MQTT))
    ...
    if entry.name not in online_from_status:
        entries.set_state_if_source(entry, bool_to_entry_state(False, EntryStateSource.MQTT))
