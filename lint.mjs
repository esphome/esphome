...
async function checkInternalLinks(fname, content, anchorCache) {
  ...
  const ignoreFiles = ["AGENTS.md", "CONTRIBUTING.md", "script/release_notes_template.md"];
  ...
  while ((match = linkPattern.exec(content)) !== null) {
    ...
    if (!linkUrl.startsWith("/")) {
      addError(fname, lineno, col, `Relative link '${linkUrl}' should use an absolute path (e.g., /components/...)`);
      continue;
    }
