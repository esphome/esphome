...
The [UART](#uart) is required to be set up in your configuration for this sensor to work, `parity` and
...
- **id** (*Optional*, [ID](#config-id)): Manually specify the ID for this {{< docref "ld2450/" >}} component.
- **uart_id** (*Optional*, [ID](#config-id)): Manually specify the ID of the [UART Component](#uart) to use.
...
- **ld2450_id** (*Optional*, [ID](#config-id)): Manually specify the ID for the {{< docref "ld2450/" >}} component.
...
> following [filter](#binary_sensor-filters) by default to prevent flooding Home Assistant with state updates:
>
> ```yaml
> - settle: 1000ms
> ```
...
- **target_N** (*Optional*): Details about the targets (N = 1 to 3). A maximum of 3 targets can be detected:
  ...
  - **x** (*Optional*, int): Distance in `millimeter (mm)` of the target from the `ld2450` sensor along the
      ...
      detect targets from -3000 to 3000 mm in `X` direction. All options from [Sensor](#config-sensor).
...
> [filters](#sensor-filters) by default to prevent flooding Home Assistant with state updates:
>
> ```yaml
> - timeout:
>     timeout: 1s
...
- **ld2450_id** (*Optional*, [ID](#config-id)): Manually specify the ID for the {{< docref "ld2450/" >}} component if you are
...
- **multi_target** (*Optional*): Turn on/off the Multi Target Tracking option. The initial state set based on the
  corresponding setting as read from LD2450 module at boot. All options from [Switch](#config-switch).
...
- **presence_timeout** (**Required**, int): The duration, in seconds, for which the
  ...
  seconds. All options from [Number](#config-number).
...
- **zone_N** (*Optional*): Configuration for the zone (N = 1 to 3), defined as a rectangular area in front of and
  ...
  **millimeters (mm)**.
  ...
  - **x1** (**Required**, int): Start X coordinate of the zone along the X-axis. Range is from -3000 (far left) to 3000
      ...
      (far right). All options from [Number](#config-number).

  - **y1** (**Required**, int): Start Y coordinate of the zone along the Y-axis. Range is 0 (closest) to 6000

      (furthest). All options from [Number](#config-number).
...
- **restart** (*Optional*): Restart the `ld2450` device. All options from [Button](#config-button).
...
- **target_N** (*Optional*): Targets (N = 1 to 3). A maximum of 3 targets can be detected: `target_1`, `target_2`,
  ...
  - **direction** (*Optional*): Direction of the target relative to the `ld2450` sensor. Possible values are:
      ...
      `Stationary`, `Moving away`, `Approaching`, `NA`. All options from [Sensor](#config-sensor).
...
- **baud_rate** (*Optional*): Control the UART serial port baud rate. Defaults to `256000`. Once changed, all sensors
  will stop working until a fresh install with an updated [UART Component](#uart) configuration. All options from
