...
digital potentiometer or a DAC, the sensor automation [`on_value`](#sensor-on_value) can be used.
This makes it possible, for example, to replace a physical NTC and thus make it `smart`.

An output component can accept values in the range of `0..1` (see [`output.set_level` Action](#output-set_level_action)),
