...
- **id** (*Optional*, [ID](#config-id)): Manually specify the ID for this {{< docref "ld2410/" >}} component if you need multiple
  components.
...
- **ld2410_id** (*Optional*, [ID](#config-id)): Manually specify the ID for the {{< docref "ld2410/" >}} component if you are
...
- **engineering_mode** (*Optional*): enable/disable engineering mode. Note that this requires more resources and is not
  recommended to be enabled when not necessary. All options from [Switch](#config-switch).
...
- **baud_rate** (*Optional*): Control the serial port baud rate. Defaults to `256000`. Once changed, all sensors will
  stop working until you reinstall your configuration with an updated [UART Component](#uart) configuration.
...
This is an [Action](#config-action) for setting the bluetooth password.
...
- **id** (**Required**, [ID](#config-id)): The ID of the {{< docref "ld2410/" >}} component to set.
- **password** (**Required**, string, [templatable](#config-templatable)):
...
If you connect the LD2410's `OUT` pin to a GPIO pin, you can set up a [GPIO Binary Sensor](#gpio-binary-sensor)
