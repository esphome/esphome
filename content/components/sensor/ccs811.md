...
- **address** (*Optional*, int): The [I²C](#i2c) address of the sensor. Defaults to `0x5A`.
...
- **temperature** (*Optional*, [ID](#config-id)): Give an external temperature sensor ID
  here. This can improve the sensor's internal calculations.

- **humidity** (*Optional*, [ID](#config-id)): Give an external humidity sensor ID
  here. This can improve the sensor's internal calculations.
