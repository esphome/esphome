...
- **uart_id** (*Optional*, [ID](#config-id)): Manually specify the ID of the [UART Component](#uart), which is
...
- **dynamic_background_correction_status** (*Optional*): True while the sensor is performing dynamic background
  correction. All options from [Binary Sensor](#config-binary_sensor).

- **ld2412_id** (*Optional*, [ID](#config-id)): Manually specify the ID for the component. Required when using multiple
  components.
...
- **detection_distance** (*Optional*, int): Distance in cm of target. All options from [Sensor](#config-sensor).
...
> [filter](#sensor-filters) by default to prevent flooding Home Assistant with state updates:
>
> ```yaml
...
- **engineering_mode** Turn on/off the engineering mode. All options from [Switch](#config-switch).
- **ld2412_id** (*Optional*, [ID](#config-id)): Manually specify the ID for the component. Required when using multiple
  components.
...
- **gate_X** (*Optional*): Threshold values for gate X, where X is in the range of 0 to 13.
  ...
  - **move_threshold** (**Required**, int): Threshold for the gate for motion detection. For the respective gate, a
      ...
      default values below. All options from [Number](#config-number).
...
- **version** (*Optional*): The firmware version. All options from [Text Sensor](#config-text_sensor).
- **mac_address** (*Optional*): The bluetooth mac address. Will be set to `unknown` when bluetooth is off. All
  options from [Text Sensor](#config-text_sensor).
...
- **distance_resolution** (*Optional*): Control the gates distance resolution. Can be `0.75m`, `0.5m` or `0.2m`.
  Defaults to `0.75m`. All options from [Select](#config-select).

- **baud_rate** (*Optional*): Allows changing the baud rate of the LD2412's serial port. Defaults to `115200`. Once
  changed, sensors will stop working until the [UART Component](#uart) is updated with the new baud rate in your
  device's configuration. All options from [Select](#config-select).

- **out_pin_level** (*Optional*): Allows selection of the LD2412's OUT pin behavior when the sensor detects presence.
  Can be `low` or `high`. Defaults to `low`. All options from [Select](#config-select).
...
light) or not. If you wish, you can set up a [GPIO Binary Sensor](#gpio-binary-sensor):
