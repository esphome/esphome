...
Note that this component is for displays that are connected via the [I²C Bus](#i2c) (see [Over I²C](#st7567-i2c))
or 3-Wire or 4-Wire [SPI bus](#spi) (see [Over SPI](#st7567-spi)).
...
Connect `SDA` and `SCL` pins on a module to pins you chose for the [I²C Bus](#i2c).
...
- **address** (*Optional*, int): Manually specify the [I²C](#i2c) address of the display. Defaults to 0x3F.
...
Connect `D0` to the `CLK` pin you chose for the [SPI bus](#spi), connect `D1` to the `MOSI` pin and `DC` and `CS`
...
- **cs_pin** (*Optional*, [Pin Schema](#config-pin_schema)): The Chip Select (CS) pin.
- **reset_pin** (*Optional*, [Pin Schema](#config-pin_schema)): The RESET pin. Defaults to not connected.
