...
ESPHome. Transceivers are connected via the [SPI Bus](#spi). Supported frequencies range from
137 MHz to 1020 MHz. Supported modulations include LoRa, OOK, FSK, GFSK, MSK and GMSK. The SX127x
component may be used as a platform for the [Packet Transport Component](#packet-transport) component, enabling sensor data
...
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): SPI chip select pin.
- **rst_pin** (**Required**, [Pin Schema](#config-pin_schema)): Reset pin.
- **dio0_pin** (**Optional**, [Pin Schema](#config-pin_schema)): Digital IO pin 0.
...
- **preamble_size** (**Optional**, int): Length of the preamble, in bytes, to be sent by the transmitter.
  This value should be larger than `preamble_detect` on the receive side to allow time for the receiver’s
...
This [action](#config-action) runs the `sx127x` image calibration, must be in standby mode.
...
This [action](#config-action) sets the `sx127x` mode to tx.
...
This [action](#config-action) sets the `sx127x` mode to rx.
...
This [action](#config-action) sets the `sx127x` mode to sleep.
...
This [action](#config-action) sets the `sx127x` mode to standby.
...
This [action](#config-action) sends a packet, the `sx127x` needs to be in packet mode.
