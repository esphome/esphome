...
# Draft standard names (-std suffix) to the CMake compile feature year
_CXX_STD_DRAFT_TO_FEATURE = {"2a": "20", "2b": "23", "2c": "26"}


def _cxx_std_feature(standard: str | None) -> str:
    """Map a -std value (e.g. ``gnu++20``, ``gnu++2b``) to a cxx_std_NN feature year."""
    if standard is None or "++" not in standard:
        return "20"
    year = standard.rsplit("++", 1)[1]
    return _CXX_STD_DRAFT_TO_FEATURE.get(year, year)
...
def get_component_cmakelists() -> str:
    ...
    cxx_std = _cxx_std_feature(CORE.cpp_standard)
...
# Apply C++ standard
target_compile_features(${{COMPONENT_LIB}} PUBLIC cxx_std_{cxx_std})
