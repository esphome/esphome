...
{
	ESP_LOGD(TAG, "Attention !!!");
	ESP_LOGD(TAG, "\tThis %s error could be caused by your I2C", f);
	ESP_LOGD(TAG, "\tcontroller not accepting \"clock stretching\"");
	ESP_LOGD(TAG, "\tas required by bootloader.");
	ESP_LOGD(TAG, "\tCheck \"I2C.txt\" in stm32flash source code.");
}
...
static stm32_err_t stm32_get_ack_timeout(const stm32_t *stm, uint32_t timeout)
{
	...
	do {
		...
		if (ret != 1) {
			ESP_LOGD(TAG, "Failed to read ACK byte");
		...
		if (byte != STM32_BUSY) {
			ESP_LOGD(TAG, "Got byte 0x%02x instead of ACK",
				byte);
			return STM32_ERR_UNKNOWN;
		}
	} while (1);
					      ...
					      uint32_t timeout)
{
	...
	if (ret != 2) {
		ESP_LOGD(TAG, "Failed to send command");
		return STM32_ERR_UNKNOWN;
	}
...
static stm32_err_t stm32_send_init_seq(const stm32_t *stm)
{
	...
	if (ret != 1) {
		ESP_LOGD(TAG, "Failed to send init to device");
		return STM32_ERR_UNKNOWN;
	}
	ret = stream->readBytes(&byte, 1);
...
static stm32_err_t stm32_pages_erase(const stm32_t *stm, uint32_t spage, uint32_t pages)
{
	...
	free(buf);
	if (ret != i) {
		ESP_LOGD(TAG, "Page-by-page erase error.");
		return STM32_ERR_UNKNOWN;
	}
