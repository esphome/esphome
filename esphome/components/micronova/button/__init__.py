...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        .extend({cv.Optional(CONF_MEMORY_DATA, default=0x06): cv.hex_int_range()}),
...
async def to_code(config):
    ...
    for key in TYPES:
        if key in config:
            ...
            cg.add(bt.set_memory_location(conf.get(CONF_MEMORY_LOCATION, 0)))
            cg.add(bt.set_memory_address(conf.get(CONF_MEMORY_ADDRESS, 0)))
