...
async def to_code(config):
    ...
    if room_temperature_config := config.get(CONF_ROOM_TEMPERATURE):
        ...
        cg.add(
            sens.set_memory_location(
                room_temperature_config.get(CONF_MEMORY_LOCATION, 0x00)
            )
        )
        cg.add(
            sens.set_memory_address(
                room_temperature_config.get(CONF_MEMORY_ADDRESS, 0x01)
            )
        )
    ...
    if fumes_temperature_config := config.get(CONF_FUMES_TEMPERATURE):
        ...
        cg.add(
            sens.set_memory_location(
                fumes_temperature_config.get(CONF_MEMORY_LOCATION, 0x00)
            )
        )
        cg.add(
            sens.set_memory_address(
                fumes_temperature_config.get(CONF_MEMORY_ADDRESS, 0x5A)
            )
        )
    ...
    if stove_power_config := config.get(CONF_STOVE_POWER):
        ...
        cg.add(mv.register_micronova_listener(sens))
        cg.add(
            sens.set_memory_location(stove_power_config.get(CONF_MEMORY_LOCATION, 0x00))
        )
        cg.add(
            sens.set_memory_address(stove_power_config.get(CONF_MEMORY_ADDRESS, 0x34))
        )
    ...
    if fan_speed_config := config.get(CONF_FAN_SPEED):
        ...
        cg.add(mv.register_micronova_listener(sens))
        cg.add(
            sens.set_memory_location(fan_speed_config.get(CONF_MEMORY_LOCATION, 0x00))
        )
        cg.add(sens.set_memory_address(fan_speed_config.get(CONF_MEMORY_ADDRESS, 0x37)))
    ...
    if memory_address_sensor_config := config.get(CONF_MEMORY_ADDRESS_SENSOR):
        ...
        cg.add(
            sens.set_memory_location(
                memory_address_sensor_config.get(CONF_MEMORY_LOCATION, 0x00)
            )
        )
        cg.add(
            sens.set_memory_address(
                memory_address_sensor_config.get(CONF_MEMORY_ADDRESS, 0x00)
            )
        )
        cg.add(
