...
static const std::string STOVE_STATES[11] = {"Off", "Start", "Pellets loading", "Igniton", "Working", "Brazier Cleaning", "Final Cleaning", "Stanby", "No pellets alarm", "No ignition alarm", "Undefined alarm"};
...
void MicroNovaButton::press_action() {
  ...
  switch (this->get_function()) {
    ...
    case STOVE_FUNCTION_TEMP_DOWN:
      new_temp = this->micronova_->get_thermostat_temperature() + (STOVE_FUNCTION_TEMP_UP==this->get_function()?1:-1);
...
void MicroNova::dump_config() {
  ...
  for (auto & mv_sensor : this->micronova_sensors_) {
    ESP_LOGCONFIG(TAG," Micronova sensor: %d, location:%02X, address:%02X", ((MicroNovaSensor*)mv_sensor)->get_function(),mv_sensor->get_memory_location(),mv_sensor->get_memory_address());
  }

  if (this->scan_memory_location_ >= 0 ) {
    ESP_LOGCONFIG(TAG,"  Memory %02X scan",this->scan_memory_location_);
    for (uint8_t i=0;i<0x0F;i++) {
      ESP_LOGCONFIG(TAG,"    Address %02X, Data %02X",i,this->read_address( (uint8_t) this->scan_memory_location_,i));
    }
  }
...
int MicroNova::read_address(uint8_t addr, uint8_t reg){
  ...
  this->enable_rx_pin_->digital_write(true);
  ESP_LOGD(TAG, "First 2 bytes from 0x%02X:0x%02X [0x%02X,0x%02X] dec: [%d,%d]",addr,reg,data[0],data[1],data[0],data[1]);
