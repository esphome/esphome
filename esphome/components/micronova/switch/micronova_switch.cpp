...
void MicroNovaSwitch::write_state(bool state) {
  if (state) {
    ...
    if (this->micronova_->get_current_stove_state() == 0) {
      ...
      ESP_LOGW(TAG, "Unable to turn stove on, invalid state: %d", this->micronova_->get_current_stove_state());
    }
  } else {
    // don't send power-off when status is Off or Final cleaning
    if (this->micronova_->get_current_stove_state() != 0 && this->micronova_->get_current_stove_state() != 6) {
      this->micronova_->write_address(this->memory_location_, this->memory_address_, this->memory_data_off_);
      this->publish_state(false);
    } else {
      ESP_LOGW(TAG, "Unable to turn stove off, invalid state: %d", this->micronova_->get_current_stove_state());
    }
  }
