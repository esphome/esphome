...
void SimpleEvseChargingSwitch::update(bool running,
                                      const std::array<uint16_t, COUNT_STATUS_REGISTER> &status_register) {
...
void SimpleEvseChargingSwitch::write_state(bool state) {
  ...
  auto trans = make_unique<ModbusWriteHoldingRegistersTransaction>(
      FIRST_STATUS_REGISTER + REGISTER_CTRL_BITS, state ? CHARGING_ENABLED_ON : CHARGING_ENABLED_OFF,
      [this, state](ModbusTransactionResult result) {
        this->active_writes_--;
        ESP_LOGD(TAG, "Command executed. (Success=%d, Active writes=%d)", static_cast<int>(result),
                 this->active_writes_);
