...
def format_pins(pins: dict[str, int] | str) -> str:
    """Format a pin dict or alias as Python source."""
    if isinstance(pins, str):
        return f'"{pins}"'
    items = ", ".join(f'"{k}": {v}' for k, v in sorted(pins.items()))
    return f"{{{items}}}"
...
def generate(arduino_pico_path: Path) -> str:
    ...
    lines = [
        ...
        "",
        f"# arduino-pico maps pins >= {CYW43_GPIO_OFFSET} to CYW43 wireless chip GPIOs",
        f"CYW43_GPIO_OFFSET = {CYW43_GPIO_OFFSET}",
        f"CYW43_MAX_GPIO = {CYW43_GPIO_OFFSET + CYW43_GPIO_COUNT - 1}",
        f"DEFAULT_MAX_PIN = {DEFAULT_MAX_PIN}",
        "",
        "RP2040_BASE_PINS = {}",
        "",
        "RP2040_BOARD_PINS = {",
    ...
    for name, pins in sorted(board_pins.items()):
        lines.append(f"    {name!r}: {format_pins(pins)},")
