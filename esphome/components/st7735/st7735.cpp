...
void ST7735::display_init_(const uint8_t *addr) {
  ...
  num_commands = progmem_read_8(addr++);  // Number of commands to follow
  while (num_commands--) {               // For each command...
    cmd = progmem_read_8(addr++);         // Read command
    num_args = progmem_read_8(addr++);    // Number of args to follow
    ...
    if (ms) {
      ms = progmem_read_8(addr++);  // Read post-command delay time (ms)
      if (ms == 255)
...
void HOT ST7735::senddata_(const uint8_t *data_bytes, uint8_t num_data_bytes) {
  ...
  for (uint8_t i = 0; i < num_data_bytes; i++) {
    this->write_byte(progmem_read_8(data_bytes++));  // write byte - SPI library
  }
  this->cs_->digital_write(true);
  this->disable();
}
