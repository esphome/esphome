...
from esphome.const import CONF_ASSUMED_STATE, CONF_CLOSE_ACTION, CONF_CURRENT_OPERATION, CONF_ID, \
    ...
    CONF_STATE, CONF_STOP_ACTION, CONF_TILT, CONF_TILT_ACTION, CONF_TILT_LAMBDA, CONF_POSITION_ACTION, CONF_POSITION_LAMBDA
...
def to_code(config):
    ...
    if CONF_POSITION_ACTION in config:
        yield automation.build_automation(var.get_position_trigger(), [(float, 'position')],
                                          config[CONF_TILT_ACTION])
    ...
    if CONF_POSITION_LAMBDA in config:
        position_template_ = yield cg.process_lambda(config[CONF_POSITION_LAMBDA], [],
                                                 return_type=cg.optional.template(float))
        cg.add(var.set_position_lambda(position_template_))
