...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  uint32_t old_key = this->get_preference_hash() + extra;
#pragma GCC diagnostic pop
#if defined(USE_COLLISION_SAFE_UNIQUE_IDS) && defined(USE_PREFERENCE_KEY_LOOKUP)
  // TextSaver::setup() migrates data stored under old_key to the raw-name key once
  uint32_t key = this->raw_name_preference_key_base_() + extra;
#else
  // Collision-safe unique ids are disabled; the saver stores under the old key
