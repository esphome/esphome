...
template<uint8_t N> class MCP23XXXBase : public Component, public gpio_expander::CachedGpioExpander<uint8_t, N> {
 public:
  ...
  void loop() override {
    ...
    // straddle the I2C read leave INT asserted without re-firing a falling edge, which would
    // strand us with stale state forever; keep looping until the line is released so we self-heal.
    // digital_read() is logical, so compare against the inverted flag to get the physical level.
    if (this->interrupt_pin_ != nullptr &&
        this->interrupt_pin_->digital_read() != this->interrupt_pin_->is_inverted()) {
      this->disable_loop();
    }
  }
