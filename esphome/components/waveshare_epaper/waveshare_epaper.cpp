...
void WaveshareEPaper6C::setup() {
  this->init_internal_6c_(this->get_buffer_length_());
  this->setup_pins_();
  this->spi_setup();
  this->reset_();
  this->initialize();
}
void WaveshareEPaper6C::init_internal_6c_(uint32_t buffer_length) {
...
uint8_t WaveshareEPaper6C::color_to_hex(Color color) {
  uint8_t hex_code;
  if (color.red > 127) {
    if (color.green > 170) {
      if (color.blue > 127) {
...
void WaveshareEPaper6C::fill(Color color) {
  uint8_t pixel_color;
  if (color.is_on()) {
...
uint32_t WaveshareEPaper6C::get_buffer_length_() {
...
void HOT WaveshareEPaper6C::draw_absolute_pixel_internal(int x, int y, Color color) {
  if (x >= this->get_width_internal() || y >= this->get_height_internal() || x < 0 || y < 0)
    return;
...
uint32_t WaveshareEPaper7P3InE::idle_timeout_() { return 12000; }
