...
static const uint8_t LUT_SIZE_TTGO_B72B = 100;
...
static const uint8_t FULL_UPDATE_LUT_TTGO_B72B[LUT_SIZE_TTGO_B72B] = {
...
static const uint8_t PARTIAL_UPDATE_LUT_TTGO_B72B[LUT_SIZE_TTGO_B72B] = {
...
void WaveshareEPaperTypeA::initialize() {
  LOG_DISPLAY("", "michi: initialize() called", this);
...
void WaveshareEPaperTypeA::dump_config() {
  ...
  switch (this->model_) {
    ...
    case TTGO_EPAPER_2_13_IN:
      ...
      break;
    case TTGO_EPAPER_2_13_IN_B72B:
      ESP_LOGCONFIG(TAG, "  Model: 2.13in (TTGO B72B)");
      break;
...
void HOT WaveshareEPaperTypeA::display() {
  LOG_DISPLAY("", "michi: display() called", this);
  ...
  if (this->full_update_every_ >= 2) {
    ...
    if (full_update != prev_full_update) {
      ...
      } else if (this->model_ == TTGO_EPAPER_2_13_IN_B72B) {
        this->write_lut_(full_update ? FULL_UPDATE_LUT_TTGO_B72B : PARTIAL_UPDATE_LUT_TTGO_B72B, LUT_SIZE_TTGO_B72B);
      ...
      if (full_update) {
          LOG_DISPLAY("", "michi: display() full update", this);
      }
      else {
          LOG_DISPLAY("", "michi: display() partial update", this);
      }
    }
