...
BLEDescriptor::BLEDescriptor(ESPBTUUID uuid, uint16_t max_len) {
  ...
  this->value_.attr_value = (uint8_t *) malloc(max_len);  // NOLINT(cppcoreguidelines-no-malloc)
}

BLEDescriptor::~BLEDescriptor() { free(this->value_.attr_value); }  // NOLINT(cppcoreguidelines-no-malloc)
