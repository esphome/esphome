...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_ECO2_BASELINE): cv.hex_uint16_t,
    cv.Optional(CONF_TVOC_BASELINE): cv.hex_uint16_t,
...
def to_code(config):
    ...
    if CONF_ECO2_BASELINE in config:
        cg.add(var.set_baseline(config[CONF_ECO2_BASELINE]))

    if CONF_TVOC_BASELINE in config:
        cg.add(var.set_baseline(config[CONF_TVOC_BASELINE]))
