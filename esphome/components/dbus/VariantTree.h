...
struct VariantTree {
    std::variant <
        std::monostate,  // Für "leere" oder null-Werte
        int,
        unsigned int,
        bool,
        double,
    ...
    // Für Listen
    VariantTree(std::initializer_list<VariantTree> list)
        : data(std::make_shared<ValueList>(list)) {}
    VariantTree(const ValueList& list)
        : data(std::make_shared<ValueList>(list)) {}
    ...
    VariantTree(const ValueDict& dict)
        : data(std::make_shared<ValueDict>(dict)) {}

    VariantTree()
	    : data(std::make_shared<ValueList>()) { std::cout << "default constructor" << std::endl; }
    ...
    bool is_list() const {
        return std::holds_alternative<std::shared_ptr<ValueList>>(data);
    }

    bool is_dict() const {
        return std::holds_alternative<std::shared_ptr<ValueDict>>(data);
    }
...
// Helper-Funktionen
inline VariantTree dict(std::initializer_list<std::pair<const std::string, VariantTree>> d) {
    return ValueDict(d);
}

inline VariantTree list(std::initializer_list<VariantTree> l) {
    return ValueList(l);
}
