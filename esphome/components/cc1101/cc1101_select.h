...
class CC1101Select : public select::Select, public PollingComponent, public Parented<CC1101Component> {
 public:
  ...
  static constexpr Option SYNC_MODE_OPTIONS[4] = {
      {"None", 0}, {"15/16", 1}, {"16/16", 2}, {"30/32", 3}};

  static constexpr Option MODULATION_OPTIONS[5] = {
      {"2-FSK", 0}, {"GFSK", 1}, {"ASK/OOK", 3}, {"4-FSK", 4}, {"MSK", 7}};

  static constexpr Option MAGN_TARGET_OPTIONS[8] = {
      {"24dB", 0}, {"27dB", 1}, {"30dB", 2}, {"33dB", 3},
      {"36dB", 4}, {"38dB", 5}, {"40dB", 6}, {"42dB", 7}};

  static constexpr Option MAX_LNA_GAIN_OPTIONS[8] = {
      {"Default", 0}, {"2.6dB", 1}, {"6.1dB", 2}, {"7.4dB", 3},
      {"9.2dB", 4}, {"11.5dB", 5}, {"14.6dB", 6}, {"17.1dB", 7}};

  static constexpr Option MAX_DVGA_GAIN_OPTIONS[4] = {
      {"Default", 0}, {"-1", 1}, {"-2", 2}, {"-3", 3}};

  static constexpr Option CARRIER_SENSE_REL_THR_OPTIONS[4] = {
      {"Default", 0}, {"+6dB", 1}, {"+10dB", 2}, {"+14dB", 3}};

  static constexpr Option FILTER_LENGTH_FSK_MSK_OPTIONS[4] = {
      {"8", 0}, {"16", 1}, {"32", 2}, {"64", 3}};

  static constexpr Option FILTER_LENGTH_ASK_OOK_OPTIONS[4] = {
      {"4dB", 0}, {"8dB", 1}, {"12dB", 2}, {"16dB", 3}};

  static constexpr Option FREEZE_OPTIONS[4] = {
      {"Default", 0}, {"On Sync", 1}, {"Analog Only", 2}, {"Analog And Digital", 3}};

  static constexpr Option WAIT_TIME_OPTIONS[4] = {
      {"8", 0}, {"16", 1}, {"24", 2}, {"32", 3}};

  static constexpr Option HYST_LEVEL_OPTIONS[4] = {
  ...
  void update() override {
    ...
    switch (this->type_) {
      ...
      case CARRIER_SENSE_REL_THR:
        value = get_option_name(static_cast<int>(this->parent_->get_carrier_sense_rel_thr()), CARRIER_SENSE_REL_THR_OPTIONS, 4);
        break;
      case FILTER_LENGTH_FSK_MSK:
        value = get_option_name(static_cast<int>(this->parent_->get_filter_length_fsk_msk()), FILTER_LENGTH_FSK_MSK_OPTIONS, 4);
        break;
      case FILTER_LENGTH_ASK_OOK:
        value = get_option_name(static_cast<int>(this->parent_->get_filter_length_ask_ook()), FILTER_LENGTH_ASK_OOK_OPTIONS, 4);
        break;
      case FREEZE:
 ...
 protected:
  void control(const std::string &value) override {
    ...
    switch (this->type_) {
      case RX_ATTENUATION:
        ...
        if (val != -1) this->parent_->set_rx_attenuation(static_cast<RxAttenuation>(val));
        break;
      case SYNC_MODE:
        val = get_option_value(value, SYNC_MODE_OPTIONS, 4);
        if (val != -1) this->parent_->set_sync_mode(static_cast<SyncMode>(val));
        break;
      case MODULATION_TYPE:
        val = get_option_value(value, MODULATION_OPTIONS, 5);
        if (val != -1) this->parent_->set_modulation_type(static_cast<Modulation>(val));
        break;
      case MAGN_TARGET:
        val = get_option_value(value, MAGN_TARGET_OPTIONS, 8);
        if (val != -1) this->parent_->set_magn_target(static_cast<MagnTarget>(val));
        break;
      case MAX_LNA_GAIN:
        val = get_option_value(value, MAX_LNA_GAIN_OPTIONS, 8);
        if (val != -1) this->parent_->set_max_lna_gain(static_cast<MaxLnaGain>(val));
        break;
      case MAX_DVGA_GAIN:
        val = get_option_value(value, MAX_DVGA_GAIN_OPTIONS, 4);
        if (val != -1) this->parent_->set_max_dvga_gain(static_cast<MaxDvgaGain>(val));
        break;
      case CARRIER_SENSE_REL_THR:
        val = get_option_value(value, CARRIER_SENSE_REL_THR_OPTIONS, 4);
        if (val != -1) this->parent_->set_carrier_sense_rel_thr(static_cast<CarrierSenseRelThr>(val));
        break;
      case FILTER_LENGTH_FSK_MSK:
        val = get_option_value(value, FILTER_LENGTH_FSK_MSK_OPTIONS, 4);
        if (val != -1) this->parent_->set_filter_length_fsk_msk(static_cast<FilterLengthFskMsk>(val));
        break;
      case FILTER_LENGTH_ASK_OOK:
        val = get_option_value(value, FILTER_LENGTH_ASK_OOK_OPTIONS, 4);
        if (val != -1) this->parent_->set_filter_length_ask_ook(static_cast<FilterLengthAskOok>(val));
        break;
      case FREEZE:
        val = get_option_value(value, FREEZE_OPTIONS, 4);
        if (val != -1) this->parent_->set_freeze(static_cast<Freeze>(val));
        break;
      case WAIT_TIME:
        val = get_option_value(value, WAIT_TIME_OPTIONS, 4);
        if (val != -1) this->parent_->set_wait_time(static_cast<WaitTime>(val));
        break;
      case HYST_LEVEL:
        val = get_option_value(value, HYST_LEVEL_OPTIONS, 4);
        if (val != -1) this->parent_->set_hyst_level(static_cast<HystLevel>(val));
        break;
      case FREQUENCY_PRESET:
  ...
  int get_option_value(const std::string &name, const Option *opts, size_t size) {
    for (size_t i = 0; i < size; i++) {
      if (name == opts[i].name) return opts[i].value;
    }
    return -1;
  }
};
