...
struct MeterInfo {
  ...
  MeterInfo(string b, string n, string e, vector<AddressExpression> aes, string k, LinkModeSet lms, int baud,
            vector<string> &s, vector<string> &ms, vector<string> &j, vector<string> &calcfs) {
    name = n;
...
struct DriverInfo {
 private:
  ...
  function<shared_ptr<Meter>(MeterInfo &, DriverInfo &di)>
      constructor_;  // Invoke this to create an instance of the driver.
...
struct FieldInfo {
  ~FieldInfo();
  FieldInfo(int index, string vname, Quantity quantity, Unit display_unit, VifScaling vif_scaling,
            DifSignedness dif_signedness, double scale, FieldMatcher matcher, string help,
            PrintProperties print_properties, function<double(Unit)> get_numeric_value_override,
            function<string()> get_string_value_override, function<void(Unit, double)> set_numeric_value_override,
            function<void(string)> set_string_value_override, Translate::Lookup lookup, Formula *formula);
 ...
 private:
  ...
  double
      scale_;  // A hardcoded scale factor. Used only for manufacturer specific values with unknown units for the vifs.
...
struct Meter {
  ...
  virtual void createMeterEnv(string id, vector<string> *envs, vector<string> *more_json) = 0;
  virtual void printJsonMeter(Telegram *t, string *json, bool pretty_print_json) = 0;
  virtual void printMeter(Telegram *t, string *human_readable, string *fields, char separator, string *json,
                          vector<string> *envs, vector<string> *more_json, vector<string> *selected_fields,
  ...
  virtual bool handleTelegram(AboutTelegram &about, vector<uchar> input_frame, bool simulated,
                              std::vector<Address> *addresses, bool *id_match, Telegram *out_t = NULL) = 0;
