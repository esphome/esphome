...
Driver::Driver(MeterInfo &mi, DriverInfo &di) : MeterCommonImplementation(mi, di) {
  addNumericFieldWithExtractor(
      "current_consumption", "The current heat cost allocation.", DEFAULT_PRINT_PROPERTIES, Quantity::HCA,
      VifScaling::Auto, DifSignedness::Signed,
      FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::HeatCostAllocation));

  addNumericField("average_ambient_temperature", Quantity::Temperature, DEFAULT_PRINT_PROPERTIES,
                  "Average ambient temperature since this beginning of this month.");

  addNumericField("max_ambient_temperature", Quantity::Temperature, DEFAULT_PRINT_PROPERTIES,
                  "Max ambient temperature  since the beginning of this month.");

  addNumericField("average_ambient_temperature_last_month", Quantity::Temperature, DEFAULT_PRINT_PROPERTIES,
                  "Average ambient temperature last month.");

  addNumericField("average_heater_temperature_last_month", Quantity::Temperature, DEFAULT_PRINT_PROPERTIES,
                  "Average heater temperature last month.");
}

double toTemperature(uchar hi, uchar lo) { return ((double) ((hi << 8) | lo)) / 100.0; }

double toIndicationU(uchar hi, uchar lo) { return ((double) ((hi << 8) | lo)) / 10.0; }
...
void Driver::processContent(Telegram *t) {
  if (t->mfct_0f_index == -1)
    return;  // Check that there is mfct data.
...
void Driver::decodeRF_RKN0(Telegram *t) {
  ...
  t->addSpecialExplanation(i + offset, 1, KindOfData::PROTOCOL, Understanding::FULL, "*** %02X frame identifier %s",
                           frame_identifier, frame_identifier == 0x10 ? "OK" : "UNKNOWN");
  i++;

  if (i + 1 >= len)
    return;
  uint16_t status = bytes[i + 1] << 8 | bytes[i];
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X status", bytes[i],
                           bytes[i + 1], status);
  i += 2;
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X time", bytes[i],
                           bytes[i + 1], time);
  i += 2;
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X date %x", bytes[i],
                           bytes[i + 1], date);
  i += 2;
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X (%s)", bytes[i],
                           bytes[i + 1], info.c_str());
  i += 2;
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X max date %x",
                           bytes[i], bytes[i + 1], max_date);
  i += 2;

  if (i + 1 >= len)
    return;
  uint16_t num_measurements = bytes[i + 1] << 8 | bytes[i];
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X num measurements %d",
                           bytes[i], bytes[i + 1], num_measurements);
  i += 2;

  if (i + 1 >= len)
    return;
  double average_ambient_temperature_last_month_c = toTemperature(bytes[i + 1], bytes[i]);
  setNumericValue("average_ambient_temperature_last_month", Unit::C, average_ambient_temperature_last_month_c);
  ...
  setNumericValue("average_heater_temperature_last_month", Unit::C, average_heater_temperature_last_month_c);
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X indication u %f",
                           bytes[i], bytes[i + 1], indication_u);
  i += 2;
  ...
  t->addSpecialExplanation(i + offset, 3, KindOfData::CONTENT, Understanding::FULL,
                           "*** %02X%02X%02X total indication u %f", bytes[i], bytes[i + 1], bytes[i + 2],
                           total_indication_u);
  i += 3;
}
...
void Driver::decodeRF_RKN9(Telegram *t) {
  ...
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X date case opened %x",
                           bytes[i], bytes[i + 1], date_case_opened);
  i += 2;

  if (i + 1 >= len)
    return;
  uint16_t date_start_month = bytes[i + 1] << 8 | bytes[i];
  t->addSpecialExplanation(i + offset, 2, KindOfData::CONTENT, Understanding::FULL, "*** %02X%02X date start month %x",
                           bytes[i], bytes[i + 1], date_start_month);
  i += 2;
...
// {"media":"heat cost
// allocation","meter":"hydroclima","name":"HCA","id":"68036198","current_consumption_hca":0,"average_ambient_temperature_c":18.66,"max_ambient_temperature_c":47.51,"average_ambient_temperature_last_month_c":15.78,"average_heater_temperature_last_month_c":17.47,"timestamp":"1111-11-11T11:11:11Z"}
...
// {"media":"heat cost
// allocation","meter":"hydroclima","name":"HCAA","id":"74393723","timestamp":"1111-11-11T11:11:11Z"}
