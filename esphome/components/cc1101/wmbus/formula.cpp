...
double NumericFormulaAddition::calculate(SIUnit to_siunit) {
  ...
  if (isDebugEnabled()) {
    debug("(formula) ADD %g (%s) %g (%s) --> %g %s --> %g %s", l, left_->siunit().info().c_str(), r,
          right_->siunit().info().c_str(), v, v_siunit.info().c_str(), result, siunit().info().c_str());
  }
...
double NumericFormulaSubtraction::calculate(SIUnit to_siunit) {
  ...
  if (isDebugEnabled()) {
    debug("(formula) SUB %g (%s) %g (%s) --> %g %s --> %g %s", l, left_->siunit().info().c_str(), r,
          right_->siunit().info().c_str(), v, v_siunit.info().c_str(), result, siunit().info().c_str());
  }
...
double NumericFormulaMultiplication::calculate(SIUnit to_siunit) {
  ...
  if (isDebugEnabled()) {
    debug("(formula) MUL %g (%s) %g (%s) --> %g --> %g %s", l, left_->siunit().info().c_str(), r,
          right_->siunit().info().c_str(), m, v, to_siunit.info().c_str());
  }
  return v;
}
...
double NumericFormulaDivision::calculate(SIUnit to_siunit) {
  ...
  if (isDebugEnabled()) {
    debug("(formula) DIV %g (%s) %g (%s) --> %g --> %g %s", l, left_->siunit().info().c_str(), r,
          right_->siunit().info().c_str(), d, v, to_siunit.info().c_str());
  }
...
double NumericFormulaSquareRoot::calculate(SIUnit to_siunit) {
  ...
  if (isDebugEnabled()) {
    debug("(formula) SQRT %g (%s) --> %g --> %g %s", i, inner_->siunit().info().c_str(), s, v,
...
double Token::val(const string &s) {
  ...
  if (type == TokenType::NUMBER) {
    ...
    if (!ok)
      ok = strptime(v.c_str(), "'%Y-%m-%dT%H:%M:%SZ'", &time);
...
string Token::vals(const string &s) { return s.substr(start, len); }
...
bool is_letter_or_underscore(char c) { return c == '_' || (c >= 'a' && c <= 'z'); }

bool is_letter_digit_or_underscore(char c) { return c == '_' || (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9'); }
...
size_t FormulaImplementation::findNumber(size_t i) {
  ...
  while (true) {
    ...
    if (c == '.') {
      if (start == i)
        return 0;  // Numbers do not start with a dot.
      num_dots++;
    }
    i++;
    len++;
  ...
  if (num_dots > 1)
    return 0;  // More than one decimal dot is an error.
...
size_t FormulaImplementation::findTime(size_t i) {
  ...
  if (i + 9 < formula_.length() && '\'' == formula_[i + 0] && isdigit(formula_[i + 1]) && isdigit(formula_[i + 2]) &&
      ':' == formula_[i + 3] && isdigit(formula_[i + 4]) && isdigit(formula_[i + 5]) && ':' == formula_[i + 6] &&
      isdigit(formula_[i + 7]) && isdigit(formula_[i + 8]) && '\'' == formula_[i + 9]) {
    return 10;
  }

  if (i + 6 < formula_.length() && '\'' == formula_[i + 0] && isdigit(formula_[i + 1]) && isdigit(formula_[i + 2]) &&
      ':' == formula_[i + 3] && isdigit(formula_[i + 4]) && isdigit(formula_[i + 5]) && '\'' == formula_[i + 6]) {
    return 7;
  }
...
#define X(cname, lcname, hrname, quantity, explanation) \
  if ((i + sizeof(#lcname) - 1 <= len) && !is_letter_digit_or_underscore(formula_[i + sizeof(#lcname) - 1]) && \
      !strncmp(#lcname, formula_.c_str() + i, sizeof(#lcname) - 1) && longest_unit < (sizeof(#lcname) - 1)) { \
    longest_unit = (sizeof(#lcname) - 1); \
  }
  LIST_OF_UNITS
#undef X
...
bool FormulaImplementation::tokenize() {
  ...
  while (i < formula_.length()) {
    ...
    len = findDateTime(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::DATETIME, i, len));
      i += len;
      continue;
    }
    ...
    len = findTime(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::TIME, i, len));
      i += len;
      continue;
    }
    ...
    len = findNumber(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::NUMBER, i, len));
      i += len;
      continue;
    }
    ...
    len = findLPar(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::LPAR, i, len));
      i += len;
      continue;
    }
    ...
    len = findRPar(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::RPAR, i, len));
      i += len;
      continue;
    }
    ...
    len = findPlus(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::PLUS, i, len));
      i += len;
      continue;
    }
    ...
    len = findMinus(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::MINUS, i, len));
      i += len;
      continue;
    }
    ...
    len = findTimes(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::TIMES, i, len));
      i += len;
      continue;
    }
    ...
    len = findDiv(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::DIV, i, len));
      i += len;
      continue;
    }
    ...
    len = findExp(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::EXP, i, len));
      i += len;
      continue;
    }
    ...
    len = findSqrt(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::SQRT, i, len));
      i += len;
      continue;
    }
    ...
    len = findUnit(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::UNIT, i, len));
      i += len;
      continue;
    }
    ...
    len = findField(i);
    if (len > 0) {
      tokens_.push_back(Token(TokenType::FIELD, i, len));
      i += len;
      continue;
    }
...
void FormulaImplementation::handleConstant(Token *number, Token *unit) {
  ...
  if (u == Unit::Unknown) {
    errors_.push_back(
        tostrprintf("Unknown unit \"%s\"!\n%s", unit->vals(formula_).c_str(), unit->withMarker(formula_).c_str()));
    valid_ = false;
    return;
  }
...
void FormulaImplementation::handleAddition(Token *tok) {
  ...
  if (!ok) {
    ...
    errors_.push_back(tostrprintf("Cannot add %s to %s!\n%s", left_siunit.info().c_str(), right_siunit.info().c_str(),
                                  tok->withMarker(formula_).c_str()));
    valid_ = false;
    return;
  }
...
void FormulaImplementation::handleSubtraction(Token *tok) {
  ...
  if (!ok) {
    string lsis = left_siunit.str();
    string rsis = right_siunit.str();
    errors_.push_back(tostrprintf("Cannot subtract %s from %s!\n%s", left_siunit.info().c_str(),
                                  right_siunit.info().c_str(), tok->withMarker(formula_).c_str()));
    valid_ = false;
    return;
  }
...
void FormulaImplementation::handleSquareRoot(Token *tok) { doSquareRoot(); }
...
bool FormulaImplementation::valid() { return valid_ == true && op_stack_.size() == 1; }
...
void FormulaImplementation::doConstant(Unit u, double c) { pushOp(new NumericFormulaConstant(this, u, c)); }
...
Formula *newFormula() { return new FormulaImplementation(); }

Formula::~Formula() {}

FormulaImplementation::~FormulaImplementation() {}

string FormulaImplementation::str() { return topOp()->str(); }
...
void FormulaImplementation::setMeter(Meter *m) { meter_ = m; }

void FormulaImplementation::setDVEntry(DVEntry *dve) { dventry_ = dve; }

string NumericFormulaConstant::str() { return tostrprintf("%.17g %s", constant_, siunit()); }
...
string NumericFormulaConstant::tree() {
  ...
  return tostrprintf("<CONST %.17g %s[%s]%s> ", constant_, unitToStringLowerCase(nearest).c_str(), sis.c_str(),
                     toString(q));
}
...
string NumericFormulaDVEntryField::str() { return toString(counter_); }

string NumericFormulaDVEntryField::tree() { return string("<DVENTRY ") + toString(counter_) + "> "; }

void FormulaImplementation::pushOp(NumericFormula *nf) { op_stack_.push_back(unique_ptr<NumericFormula>(nf)); }
...
bool StringInterpolatorImplementation::parse(const std::string &f) {
  ...
  while (next_start_brace != string::npos) {
    ...
    if (next_end_brace == string::npos)
      return false;  // Oups, missing closing }
...
string StringInterpolatorImplementation::apply(DVEntry *dve) {
  ...
  for (;;) {
    if (s >= strings_.size() && f >= formulas_.size())
      break;
