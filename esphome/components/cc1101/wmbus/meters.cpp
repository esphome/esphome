...
vector<DriverInfo *> &allDrivers() { return *registered_drivers_list_; }
...
MeterCommonImplementation::MeterCommonImplementation(MeterInfo &mi, DriverInfo &di)
    : type_(di.type()),
...
void MeterCommonImplementation::addShellMeterAdded(string cmdline) { shell_cmdlines_added_.push_back(cmdline); }

void MeterCommonImplementation::addShellMeterUpdated(string cmdline) { shell_cmdlines_updated_.push_back(cmdline); }

void MeterCommonImplementation::addExtraConstantField(string ecf) { extra_constant_fields_.push_back(ecf); }
...
void MeterCommonImplementation::addExtraCalculatedField(string ecf) {
  ...
  if (existing != NULL) {
    if (!canConvert(unit, existing->displayUnit())) {
      warning("Warning! Cannot add the calculated field: %s since it would conflict with the already declared field %s "
              "for quantity %s.",
  ...
  addNumericFieldWithCalculator(vname, "Calculated: " + ecf, DEFAULT_PRINT_PROPERTIES, quantity, parts[1], unit);
}

vector<string> &MeterCommonImplementation::shellCmdlinesMeterAdded() { return shell_cmdlines_added_; }

vector<string> &MeterCommonImplementation::shellCmdlinesMeterUpdated() { return shell_cmdlines_updated_; }

vector<string> &MeterCommonImplementation::meterExtraConstantFields() { return extra_constant_fields_; }
...
void MeterCommonImplementation::addLinkMode(LinkMode lm) { link_modes_.addLinkMode(lm); }

void MeterCommonImplementation::setMfctTPLStatusBits(Translate::Lookup &lookup) { mfct_tpl_status_bits_ = lookup; }

void MeterCommonImplementation::addNumericFieldWithExtractor(string vname, string help,
                                                             PrintProperties print_properties, Quantity vquantity,
                                                             VifScaling vif_scaling, DifSignedness dif_signedness,
                                                             FieldMatcher matcher, Unit display_unit, double scale) {
  field_infos_.emplace_back(FieldInfo(field_infos_.size(), vname, vquantity,
                                      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity) : display_unit,
                                      vif_scaling, dif_signedness, scale, matcher, help, print_properties, NULL, NULL,
                                      NULL, NULL, NoLookup, /* Lookup table */
                                      NULL                  /* Formula */
                                      ));
}

void MeterCommonImplementation::addNumericFieldWithCalculator(string vname, string help,
                                                              PrintProperties print_properties, Quantity vquantity,
                                                              string formula, Unit display_unit) {
  Formula *f = newFormula();
  bool ok = f->parse(this, formula);
  if (!ok) {
    string err = f->errors();
    warning("Warning! Ignoring calculated field %s because parse failed:\n%s", vname.c_str(), err.c_str());
    delete f;
    return;
  }
  assert(ok);

  field_infos_.push_back(FieldInfo(field_infos_.size(), vname, vquantity,
                                   display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity) : display_unit,
                                   VifScaling::Auto, DifSignedness::Signed, 1.0, FieldMatcher::noMatcher(), help,
                                   print_properties, NULL, NULL, NULL, NULL, NoLookup, /* Lookup table */
                                   f                                                   /* Formula */
                                   ));
}

void MeterCommonImplementation::addNumericFieldWithCalculatorAndMatcher(string vname, string help,
                                                                        PrintProperties print_properties,
                                                                        Quantity vquantity, string formula,
                                                                        FieldMatcher matcher, Unit display_unit) {
  Formula *f = newFormula();
  bool ok = f->parse(this, formula);
  if (!ok) {
    string err = f->errors();
  ...
  field_infos_.push_back(FieldInfo(field_infos_.size(), vname, vquantity,
                                   display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity) : display_unit,
                                   VifScaling::Auto, DifSignedness::Signed, 1.0, matcher, help, print_properties, NULL,
                                   NULL, NULL, NULL, NoLookup, /* Lookup table */
                                   f                           /* Formula */
                                   ));
}

void MeterCommonImplementation::addNumericField(string vname, Quantity vquantity, PrintProperties print_properties,
                                                string help, Unit display_unit) {
  field_infos_.emplace_back(FieldInfo(field_infos_.size(), vname, vquantity,
                                      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity) : display_unit,
                                      VifScaling::None, DifSignedness::Signed, 1.0, FieldMatcher::noMatcher(), help,
                                      print_properties,
                                      NULL,  // getValueFunc,
                                      NULL,
                                      NULL,           // setValueFunc
...
void MeterCommonImplementation::addStringFieldWithExtractor(string vname, string help, PrintProperties print_properties,
                                                            FieldMatcher matcher) {
  field_infos_.emplace_back(FieldInfo(
      field_infos_.size(), vname, Quantity::Text, defaultUnitForQuantity(Quantity::Text), VifScaling::None,
      DifSignedness::Signed, 1.0, matcher, help, print_properties, NULL, NULL, NULL, NULL, NoLookup, /* Lookup table */
      NULL                                                                                           /* Formula */
      ));
}

void MeterCommonImplementation::addStringFieldWithExtractorAndLookup(string vname, string help,
                                                                     PrintProperties print_properties,
                                                                     FieldMatcher matcher, Translate::Lookup lookup) {
  field_infos_.emplace_back(FieldInfo(
      field_infos_.size(), vname, Quantity::Text, defaultUnitForQuantity(Quantity::Text), VifScaling::None,
      DifSignedness::Signed, 1.0, matcher, help, print_properties, NULL, NULL, NULL, NULL, lookup, NULL /* Formula */
      ));
}

void MeterCommonImplementation::addStringField(string vname, string help, PrintProperties print_properties) {
  field_infos_.emplace_back(FieldInfo(field_infos_.size(), vname, Quantity::Text,
                                      defaultUnitForQuantity(Quantity::Text), VifScaling::None, DifSignedness::Signed,
                                      1.0, FieldMatcher(), help, print_properties, NULL, NULL, NULL, NULL,
                                      NoLookup, /* Lookup table */
                                      NULL      /* Formula */
                                      ));
}

vector<AddressExpression> &MeterCommonImplementation::addressExpressions() { return address_expressions_; }

vector<FieldInfo> &MeterCommonImplementation::fieldInfos() { return field_infos_; }

vector<string> &MeterCommonImplementation::extraConstantFields() { return extra_constant_fields_; }

string MeterCommonImplementation::name() { return name_; }

void MeterCommonImplementation::onUpdate(function<void(Telegram *, Meter *)> cb) { on_update_.push_back(cb); }
...
string toString(DriverInfo &di) { return di.name().str(); }
    ...
    if (isVerboseEnabled() || isDebugEnabled()) {
      ...
      if (t->beingAnalyzed() == false && driver_name != "auto") {
        debug("(meter) %s: meter detection did not match the selected driver '%s', detected driver is: '%s'",
              name.c_str(), driver_name.c_str(), possible_drivers.c_str());
      }
    }
  }
...
string MeterCommonImplementation::bus() { return bus_; }
...
bool checkPrintableField(string *buf, string desired_field, Meter *m, Telegram *t, char c, vector<FieldInfo> &fields,
                         bool human_readable) {
...
bool MeterCommonImplementation::handleTelegram(AboutTelegram &about, vector<uchar> input_frame, bool simulated,
                                               vector<Address> *addresses, bool *id_match, Telegram *out_analyzed) {
  ...
  *id_match = true;
  verbose("(meter) %s(%d) %s  handling telegram from %s", name().c_str(), index(), driverName().str().c_str(),
...
void MeterCommonImplementation::processFieldExtractors(Telegram *t) {
  ...
  for (FieldInfo &fi : field_infos_) {
    ...
    if (!fi.hasMatcher()) {
      ...
      debug("(meters) skipping field without matcher %s(%s)[%d]...", fi.vname().c_str(), toString(fi.xuantity()),
            fi.index());
      continue;
    }

    debug("(meters) trying field info %s(%s)[%d]...", fi.vname().c_str(), toString(fi.xuantity()), fi.index());
    ...
    for (DVEntry *dve : sorted_entries) {
      if (fi.hasMatcher() && fi.matches(dve)) {
        ...
        if (fi.matcher().index_nr != IndexNr(current_match_nr) &&
          ...
          debug("(meters) using field info %s(%s)[%d] to extract %s at offset %d", fi.vname().c_str(),
                toString(fi.xuantity()), fi.index(), dve->dif_vif_key.str().c_str(), dve->offset);
...
void MeterCommonImplementation::processFieldCalculators() {
  ...
  for (FieldInfo &fi : field_infos_) {
    if (fi.hasFormula() && !fi.hasMatcher()) {
      debug("(meters) calculating field %s(%s)[%d]", fi.vname().c_str(), toString(fi.xuantity()), fi.index());
...
bool MeterCommonImplementation::hasProcessContent() { return has_process_content_; }
...
void MeterCommonImplementation::setNumericValue(string vname, Unit u, double v) {
  ...
  if (fi == NULL) {
    warning("(meter) cannot set numeric value %g %s for non-existant field \"%s\" %s", v,
            unitToStringLowerCase(u).c_str(), vname.c_str(), toString(q));
    return;
  }
  setNumericValue(fi, NULL, u, v);
}

bool MeterCommonImplementation::hasValue(FieldInfo *fi) { return hasStringValue(fi) || hasNumericValue(fi); }
...
bool MeterCommonImplementation::hasStringValue(FieldInfo *fi) { return string_values_.count(fi->vname()) != 0; }

bool MeterCommonImplementation::hasStringValue(string name) { return string_values_.count(name) != 0; }
...
FieldInfo::FieldInfo(int index, string vname, Quantity xuantity, Unit display_unit, VifScaling vif_scaling,
...
string FieldInfo::renderJson(Meter *m, DVEntry *dve) {
  ...
  if (xuantity() == Quantity::Text) {
    ...
    if (displayUnit() == Unit::DateLT) {
      ...
      // All numeric values.
      s += "\"" + field_name + "_" + display_unit_s +
           "\":" + valueToString(m->getNumericValue(field_name, displayUnit()), displayUnit());
    }
  }
...
string FieldInfo::renderJsonOnlyDefaultUnit(Meter *m) { return renderJson(m, NULL); }

string FieldInfo::renderJsonText(Meter *m, DVEntry *dve) { return renderJson(m, dve); }
...
void MeterCommonImplementation::createMeterEnv(string id, vector<string> *envs, vector<string> *extra_constant_fields) {
...
void MeterCommonImplementation::printJsonMeter(Telegram *t, string *json, bool pretty_print_json) {
  string media;
  if (t->tpl_id_found) {
...
void MeterCommonImplementation::printMeter(Telegram *t, string *human_readable, string *fields, char separator,
                                           string *json, vector<string> *envs, vector<string> *extra_constant_fields,
                                           vector<string> *selected_fields, bool pretty_print_json) {
  ...
  *fields = concatFields(this, t, separator, field_infos_, false, selected_fields, extra_constant_fields);
  ...
  for (FieldInfo& fi : field_infos_)
  {
      ...
      if (founds.count(&fi) != 0)
      {
          ...
          for (DVEntry *dve : founds[&fi])
          {
              debug("(meters) render field %s(%s %s)[%d] with dventry @%d key %s data %s",
                    fi.vname().c_str(), toString(fi.xuantity()), unitToStringLowerCase(fi.displayUnit()).c_str(),
  fi.index(), dve->offset, dve->dif_vif_key.str().c_str(), dve->value.c_str()); string out = fi.renderJson(this, dve);
      ...
      else
      {
          ...
          // I.e. we have two rules that store into target_m3, this check will prevent target_m3 from being printed
  twice. if (fi.printProperties().hasREQUIRED() || (hasValue(&fi) && ( found_vnames.count(fi.vname()) == 0 ||
  ...
  *json = s;
  ...
  for (FieldInfo &fi : field_infos_) {
    ...
    if (fi.xuantity() == Quantity::Text) {
      ...
      string envvar = "METER_" + var + "_" + display_unit_s + "=" +
                      valueToString(getNumericValue(&fi, fi.displayUnit()), fi.displayUnit());
      envs->push_back(envvar);
    }
  }
...
void MeterCommonImplementation::setExpectedTPLSecurityMode(TPLSecurityMode tsm) { expected_tpl_sec_mode_ = tsm; }

void MeterCommonImplementation::setExpectedELLSecurityMode(ELLSecurityMode dsm) { expected_ell_sec_mode_ = dsm; }

TPLSecurityMode MeterCommonImplementation::expectedTPLSecurityMode() { return expected_tpl_sec_mode_; }

ELLSecurityMode MeterCommonImplementation::expectedELLSecurityMode() { return expected_ell_sec_mode_; }
...
bool isMeterDriverReasonableForMedia(string driver_name, int media) {
  if (media == 0x37)
    return false;  // Skip converter meter side since they do not give any useful information.
...
shared_ptr<Meter> createMeter(MeterInfo *mi) {
  ...
  if (di != NULL) {
    ...
    verbose("(meter) created %s %s %s %s", mi->name.c_str(), di->name().str().c_str(), aesc.c_str(), keymsg);
    return newm;
  }
...
string FieldInfo::str() {
  return tostrprintf("%d %s_%s (%s) %s [%s] \"%s\"", index_, vname_.c_str(),
                     unitToStringLowerCase(display_unit_).c_str(), toString(xuantity_), toString(vif_scaling_),
...
bool FieldInfo::extractNumeric(Meter *m, Telegram *t, DVEntry *dve) {
  ...
  if (dve == NULL) {
    if (key == "") {
      // Search for key.
      bool ok = findKeyWithNr(matcher_.measurement_type, matcher_.vif_range, matcher_.storage_nr_from.intValue(),
                              matcher_.tariff_nr_from.intValue(), matcher_.index_nr.intValue(), &key, &t->dv_entries);
      // No entry was found.
      if (!ok)
        return false;
    }
  ...
  if (dve->extractDouble(&extracted_double_value, auto_vif_scaling, force_unsigned)) {
    ...
    if (matcher_.vif_range == VIFRange::DateTime) {
      ...
      extracted_double_value = tmp;
    } else if (matcher_.vif_range == VIFRange::AnyEnergyVIF || matcher_.vif_range == VIFRange::AnyVolumeVIF ||
               matcher_.vif_range == VIFRange::AnyPowerVIF) {
      // Find the actual unit used in the telegram.
      decoded_unit = toDefaultUnit(dve->vif);
    } else if (matcher_.vif_range != VIFRange::Any && matcher_.vif_range != VIFRange::None) {
      // Pick the default unit for this range.
      decoded_unit = toDefaultUnit(matcher_.vif_range);
    }

    debug("(meter) %s %s decoded %s default %s value %g (scale %g)", toString(matcher_.vif_range), field_name.c_str(),
          unitToStringLowerCase(decoded_unit).c_str(), unitToStringLowerCase(display_unit_).c_str(),
...
bool FieldInfo::extractString(Meter *m, Telegram *t, DVEntry *dve) {
  ...
  if (lookup_.hasLookups() || (print_properties_.hasINCLUDETPLSTATUS())) {
    ...
    t->addMoreExplanation(dve->offset, renderJsonText(m, dve));
    found = true;
  } else if (matcher_.vif_range == VIFRange::Any || matcher_.vif_range == VIFRange::EnhancedIdentification ||
             matcher_.vif_range == VIFRange::FabricationNo || matcher_.vif_range == VIFRange::HardwareVersion ||
             matcher_.vif_range == VIFRange::FirmwareVersion || matcher_.vif_range == VIFRange::Medium ||
             matcher_.vif_range == VIFRange::Manufacturer || matcher_.vif_range == VIFRange::ModelVersion ||
             matcher_.vif_range == VIFRange::SoftwareVersion || matcher_.vif_range == VIFRange::Customer ||
             matcher_.vif_range == VIFRange::Location || matcher_.vif_range == VIFRange::SpecialSupplierInformation ||
    ...
    error("Internal error: Cannot extract text string from vif %s in %s:%d", toString(matcher_.vif_range), __FILE__,
          __LINE__);
  }
  return found;
}
...
void checkFieldsEmpty(set<string> &fields, string name) {
  if (fields.size() > 0) {
    ...
    warning("(meter) when adding common fields to driver %s, these fields were not found: %s", name.c_str(),
            info.c_str());
  }
}
...
void MeterCommonImplementation::addOptionalLibraryFields(string field_names) {
  ...
  if (checkIf(fields, "actuality_duration_s")) {
    addNumericFieldWithExtractor(
        ...
        FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::ActualityDuration), Unit::Second);
  }
  ...
  if (checkIf(fields, "fabrication_no")) {
    addStringFieldWithExtractor("fabrication_no", "Fabrication number.", DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::FabricationNo));
  }

  if (checkIf(fields, "enhanced_id")) {
    addStringFieldWithExtractor(
        "enhanced_id", "Enhanced identification number.", DEFAULT_PRINT_PROPERTIES,
        FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::EnhancedIdentification));
  }

  if (checkIf(fields, "software_version")) {
    addStringFieldWithExtractor(
        "software_version", "Software version.", DEFAULT_PRINT_PROPERTIES,
        FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::SoftwareVersion));
  }

  if (checkIf(fields, "manufacturer")) {
    addStringFieldWithExtractor("manufacturer", "Meter manufacturer.", DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::Manufacturer));
  }

  if (checkIf(fields, "model_version")) {
    addStringFieldWithExtractor("model_version", "Meter model version.", DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::ModelVersion));
  }

  if (checkIf(fields, "firmware_version")) {
    addStringFieldWithExtractor(
        "firmware_version", "Meter firmware version.", DEFAULT_PRINT_PROPERTIES,
        FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::FirmwareVersion));
  }

  if (checkIf(fields, "parameter_set")) {
    addStringFieldWithExtractor("parameter_set", "Parameter set for this meter.", DEFAULT_PRINT_PROPERTIES,
  ...
  if (checkIf(fields, "location")) {
    addStringFieldWithExtractor("location", "Meter installed at this customer location.", DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::Location));
  }

  if (checkIf(fields, "operating_time_h")) {
    addNumericFieldWithExtractor(
        "operating_time", "How long the meter has been collecting data.", DEFAULT_PRINT_PROPERTIES, Quantity::Time,
        VifScaling::Auto, DifSignedness::Signed,
        FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::OperatingTime));
  }

  if (checkIf(fields, "on_time_h")) {
    addNumericFieldWithExtractor("on_time", "How long the meter has been powered up.", DEFAULT_PRINT_PROPERTIES,
                                 Quantity::Time, VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::OnTime));
  }

  if (checkIf(fields, "on_time_at_error_h")) {
    addNumericFieldWithExtractor("on_time_at_error", "How long the meter has been in an error state while powered up.",
                                 DEFAULT_PRINT_PROPERTIES, Quantity::Time, VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build().set(MeasurementType::AtError).set(VIFRange::OnTime));
  }

  if (checkIf(fields, "meter_date")) {
    addStringFieldWithExtractor("meter_date", "Date when the meter sent the telegram.", DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::Date));
  }

  if (checkIf(fields, "meter_date_at_error")) {
    addStringFieldWithExtractor("meter_date_at_error", "Date when the meter was in error.", DEFAULT_PRINT_PROPERTIES,
  ...
  if (checkIf(fields, "meter_datetime_at_error")) {
    addStringFieldWithExtractor("meter_datetime_at_error", "Date and time when the meter was in error.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build().set(MeasurementType::AtError).set(VIFRange::DateTime));
  }

  if (checkIf(fields, "total_m3")) {
    addNumericFieldWithExtractor("total", "The total media volume consumption recorded by this meter.",
  ...
  if (checkIf(fields, "target_m3")) {
    addNumericFieldWithExtractor(
        "target", "The volume recorded by this meter at the target date.", DEFAULT_PRINT_PROPERTIES, Quantity::Volume,
        VifScaling::Auto, DifSignedness::Signed,
  ...
  if (checkIf(fields, "target_date")) {
    addNumericFieldWithExtractor(
        "target", "The target date. Usually the end of the previous billing period.", DEFAULT_PRINT_PROPERTIES,
  ...
  if (checkIf(fields, "total_forward_m3")) {
    addNumericFieldWithExtractor("total_forward", "The total media volume flowing forward.", DEFAULT_PRINT_PROPERTIES,
  ...
  if (checkIf(fields, "total_backward_m3")) {
    addNumericFieldWithExtractor("total_backward", "The total media volume flowing backward.", DEFAULT_PRINT_PROPERTIES,
                                 Quantity::Volume, VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build()
                                     .set(MeasurementType::Instantaneous)
                                     .set(VIFRange::Volume)
  ...
  if (checkIf(fields, "flow_temperature_c")) {
    addNumericFieldWithExtractor(
        "flow_temperature", "Forward media temperature.", DEFAULT_PRINT_PROPERTIES, Quantity::Temperature,
        VifScaling::Auto, DifSignedness::Signed,
  ...
  if (checkIf(fields, "external_temperature_c")) {
    addNumericFieldWithExtractor(
        "external_temperature", "Temperature outside of meter.", DEFAULT_PRINT_PROPERTIES, Quantity::Temperature,
        VifScaling::Auto, DifSignedness::Signed,
  ...
  if (checkIf(fields, "return_temperature_c")) {
    addNumericFieldWithExtractor(
        "return_temperature", "Return media temperature.", DEFAULT_PRINT_PROPERTIES, Quantity::Temperature,
        VifScaling::Auto, DifSignedness::Signed,
  ...
  if (checkIf(fields, "flow_return_temperature_difference_c")) {
    addNumericFieldWithExtractor(
        "flow_return_temperature_difference", "The difference between flow and return media temperatures.",
  ...
  if (checkIf(fields, "volume_flow_m3h")) {
    addNumericFieldWithExtractor("volume_flow", "Media volume flow.", DEFAULT_PRINT_PROPERTIES, Quantity::Flow,
                                 VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build().set(MeasurementType::Instantaneous).set(VIFRange::VolumeFlow));
  }

  if (checkIf(fields, "access_counter")) {
    addNumericFieldWithExtractor("access", "Meter access counter.", DEFAULT_PRINT_PROPERTIES, Quantity::Dimensionless,
  ...
  if (checkIf(fields, "consumption_hca")) {
    addNumericFieldWithExtractor(
        "consumption", "The current heat cost allocation for this meter.", DEFAULT_PRINT_PROPERTIES, Quantity::HCA,
        VifScaling::Auto, DifSignedness::Signed,
...
#define X(m) \
  if (MeterType::m != MeterType::AutoMeter && MeterType::m != MeterType::UnknownMeter) { \
