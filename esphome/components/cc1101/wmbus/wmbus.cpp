  ...
  if (rf_mbus_.task()) {
    ...
    if (t.parseHeader(mbus_data.frame) && t.addresses.empty()) {
      ...
      if (this->log_all_ || meter_in_config) {  // No need to do sth if logging is disabled and meter is not configured
        ...
        this->led_blink();
        ESP_LOGI(TAG, "%s [0x%08x] RSSI: %ddBm T: %s %c1 %c", (used_driver.empty() ? "Unknown!" : used_driver.c_str()),
                 meter_id, mbus_data.rssi, telegram.c_str(), mbus_data.mode, mbus_data.block);
...
void WMBusComponent::send_mqtt_raw(Telegram &t, WMbusFrame &mbus_data) {
  ...
  switch (this->mqtt_raw_format) {
    case RAW_FORMAT_RTLWMBUS:
      ...
      payload +=
          std::string(1, mbus_data.mode) + "1;1;1;" + telegram_time + ";" + std::to_string(mbus_data.rssi) + ";;;0x";
  ...
  if (this->mqtt_client_.connect("", this->mqtt_->name.c_str(), this->mqtt_->password.c_str())) {
    ...
    ESP_LOGV(TAG, "Publishing raw(topic='%s' payload='%s' retain=%d)", mqtt_topic.c_str(), payload.c_str(),
             this->mqtt_->retained);
    this->mqtt_client_.disconnect();
  } else {
