...
struct SIExp {
 ...
 private:
  ...
  int8_t year_{};  // be auto-converted to seconds, ie you cannot add/subtract months/years without knowing what you add
                   // to.
...
struct SIUnit {
  ...
  SIUnit(Quantity q, double scale, SIExp exponents) : quantity_(q), scale_(scale), exponents_(exponents) {}
  ...
  bool mathOpTo(MathOp op, double left, double right, const SIUnit &right_siunit, SIUnit *out_siunit,
                double *out) const;
