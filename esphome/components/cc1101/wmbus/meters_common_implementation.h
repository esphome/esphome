...
struct MeterCommonImplementation : public virtual Meter {
 ...
 protected:
  ...
  void addNumericFieldWithExtractor(
      ...
      Unit display_unit =
          Unit::Unknown,    // If specified use this unit for the json field instead instead of the default unit.
  ...
  void addNumericField(string vname,                      // Name of value without unit, eg total
                       Quantity vquantity,                // Value belongs to this quantity.
                       PrintProperties print_properties,  // Should this be printed by default in fields,json and hr.
                       string help,
                       Unit display_unit = Unit::Unknown);  // If specified use this unit for the json field instead
                                                            // instead of the default unit.

  void addStringFieldWithExtractor(string vname, string help, PrintProperties print_properties, FieldMatcher matcher);

  void addStringFieldWithExtractorAndLookup(string vname, string help, PrintProperties print_properties,
                                            FieldMatcher matcher,
                                            Translate::Lookup lookup);  // Translate the bits/indexes.

  // Used only for status field from tpl_status only.
  void addStringField(string vname, string help, PrintProperties print_properties);

  bool handleTelegram(AboutTelegram &about, vector<uchar> frame, bool simulated, std::vector<Address> *addresses,
                      bool *id_match, Telegram *out_analyzed = NULL);
  void createMeterEnv(string id, vector<string> *envs,
                      vector<string> *more_json);  // Add this json "key"="value" strings.

  void printJsonMeter(Telegram *t, string *json, bool pretty_print);

  void printMeter(Telegram *t, string *human_readable, string *fields, char separator, string *json,
                  vector<string> *envs,
                  vector<string> *more_json,        // Add this json "key"="value" strings.
