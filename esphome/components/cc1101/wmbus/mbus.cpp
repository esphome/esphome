...
bool mBusDecodeFormatA(const WMbusData &t_in, WMbusFrame &t_frame) {
  ...
  if ((L < 9) ||
      (((L - 9 + (numDataBlocks * 2))) > (t_in.length - BLOCK1A_SIZE))) {  // add CRC bytes for each data block
  ...
  for (uint8_t n{0}; n < numDataBlocks; ++n) {
    const uint8_t *blockStartPtr =
        (t_in.data + BLOCK1A_SIZE + (n * 18));          // Pointer to where data starts. Each block is 18 bytes
    ...
    // Get block data
    t_frame.frame.insert((t_frame.frame.begin() + ((n * 16) + BLOCK1A_SIZE - 2)), blockStartPtr,
                         (blockStartPtr + blockSize));
  }
