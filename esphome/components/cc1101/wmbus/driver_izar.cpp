...
Driver::Driver(MeterInfo &mi, DriverInfo &di) : MeterCommonImplementation(mi, di) {
  ...
  addStringField("last_month_measure_date", "The date when the meter recorded the most recent billing value.",
                 DEFAULT_PRINT_PROPERTIES);

  addNumericField("remaining_battery_life", Quantity::Time, DEFAULT_PRINT_PROPERTIES,
                  "How many more years the battery is expected to last", Unit::Year);
  ...
  addNumericField("transmit_period", Quantity::Time, DEFAULT_PRINT_PROPERTIES,
                  "The period at which the meter transmits its data.", Unit::Second);
...
void Driver::processContent(Telegram *t) {
  ...
  if (detectDiehlFrameInterpretation(frame) == DiehlFrameInterpretation::SAP_PRIOS) {
    ...
    int manufacture_year =
        yy > 70 ? (1900 + yy) : (2000 + yy);  // Maybe to adjust in 2070, if this code stills lives :D
...
// |IzarWater;21242472;C19UA;145842;3.488;3.486;2019-09-30;14.5;meter_blocked,underflow;no_alarm;8;2019;1111-11-11
// 11:11.11
