...
struct AboutTelegram {
  ...
  AboutTelegram(std::string dv, int rs, FrameType t, time_t ts = 0)
      : device(dv), rssi_dbm(rs), type(t), timestamp(ts) {}
  AboutTelegram() {}
};
...
struct Telegram {
 ...
 public:
  ...
  int mfct_0f_index =
      -1;  // -1 if not found, else index of the 0f byte, if found, inside the difvif data after the header.
  ...
  void extractPayload(
      std::vector<uchar> *pl);  // Extract frame data containing the measurements, after the header and not the suffix.
  ...
  void addExplanationAndIncrementPos(std::vector<uchar>::iterator &pos, int len, KindOfData k, Understanding u,
                                     const char *fmt, ...);
 ...
 private:
  ...
  bool checkMAC(std::vector<uchar> &frame, std::vector<uchar>::iterator from, std::vector<uchar>::iterator to,
                std::vector<uchar> &mac, std::vector<uchar> &mackey);
...
bool decrypt_ELL_AES_CTR(Telegram *t, std::vector<uchar> &frame, std::vector<uchar>::iterator &pos,
                         std::vector<uchar> &aeskey);
bool decrypt_TPL_AES_CBC_IV(Telegram *t, std::vector<uchar> &frame, std::vector<uchar>::iterator &pos,
                            std::vector<uchar> &aeskey, int *num_encrypted_bytes, int *num_not_encrypted_at_end);
bool decrypt_TPL_AES_CBC_NO_IV(Telegram *t, std::vector<uchar> &frame, std::vector<uchar>::iterator &pos,
                               std::vector<uchar> &aeskey, int *num_encrypted_bytes, int *num_not_encrypted_at_end);
