...
enum PrintProperty {
  ...
  INCLUDE_TPL_STATUS =
      8,  // This text field also includes the tpl status decoding. multiple OK:s collapse to a single OK.
  INJECT_INTO_STATUS =
      16,     // This text field is injected into the already defined status field. multiple OK:s collapse.
...
enum class MeasurementType { Any, Instantaneous, Minimum, Maximum, AtError, Unknown };
...
struct DifVifKey {
  DifVifKey(std::string key) : key_(key) { extractDV(key, &dif_, &vif_, &has_difes_, &has_vifes_); }
...
enum class DVEntryCounterType { UNKNOWN, STORAGE_COUNTER, TARIFF_COUNTER, SUBUNIT_COUNTER };
...
struct DVEntry {
  ...
  DVEntry(int off, DifVifKey dvk, MeasurementType mt, Vif vi, std::set<VIFCombinable> vc, std::set<uint16_t> vc_raw,
          StorageNr st, TariffNr ta, SubUnitNr su, std::string &val)
      : offset(off),
        dif_vif_key(dvk),
        measurement_type(mt),
        vif(vi),
