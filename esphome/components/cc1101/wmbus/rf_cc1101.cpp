...
bool RxLoop::init(uint8_t mosi, uint8_t miso, uint8_t clk, uint8_t cs, uint8_t gdo0, uint8_t gdo2, float freq,
                  bool syncMode) {
  bool retVal = false;
  ...
  for (uint8_t i = 0; i < TMODE_RF_SETTINGS_LEN; i++) {
    ELECHOUSE_cc1101.SpiWriteReg(TMODE_RF_SETTINGS_BYTES[i << 1], TMODE_RF_SETTINGS_BYTES[(i << 1) + 1]);
  }
  ...
  ESP_LOGD(TAG, "Set CC1101 frequency to %3.3fMHz [%02X %02X %02X]", freq / 1.0, freq2, freq1, freq0);
...
bool RxLoop::task() {
  do {
    ...
    if ((!overfl) && (!digitalRead(gdo2)) && (rxLoop.state > WAIT_FOR_DATA)) {
      ...
      ESP_LOGV(TAG, "Have %d bytes from CC1101 Rx, RSSI: %d dBm LQI: %d", rxLoop.bytesRx, this->returnFrame.rssi,
               this->returnFrame.lqi);
...
WMbusFrame RxLoop::get_frame() { return this->returnFrame; }
