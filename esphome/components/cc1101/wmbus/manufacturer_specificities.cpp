...
std::set<int> diehl_manufacturers = {MANUFACTURER_DME, MANUFACTURER_EWT, MANUFACTURER_HYD, MANUFACTURER_SAP,
                                     MANUFACTURER_SPL};
...
uint32_t uint32FromBytes(const vector<uchar> &data, int offset, bool reverse) {
  if (reverse)
    return ((uint32_t) data[offset + 3] << 24) | ((uint32_t) data[offset + 2] << 16) |
...
void addDefaultManufacturerKeyIfAny(const vector<uchar> &frame, TPLSecurityMode tpl_sec_mode, MeterKeys *meter_keys) {
  if (!meter_keys->hasConfidentialityKey() && tpl_sec_mode == TPLSecurityMode::AES_CBC_IV &&
...
bool decryptDielhRealData(Telegram *t, vector<uchar> &frame, vector<uchar>::iterator &pos,
                          const vector<uchar> &confidentiality_key) {
  vector<uint32_t> keys;
  initializeDiehlDefaultKeySupport(confidentiality_key, keys);

  vector<uchar> decoded_content;
  for (auto &key : keys) {
    decoded_content = decodeDiehlLfsr(t->original.empty() ? frame : t->original, frame, key,
...
void qdsExtractWalkByField(Telegram *t, Meter *driver, DVEntry &mfctEntry, int pos, int n, const string &key_s,
  ...
  DifVifKey key(key_s);
  DVEntry fieldEntry(0, key, MeasurementType::Instantaneous, key.vif(), set<VIFCombinable>(), set<uint16_t>(),
