...
bool overrideConversion(Unit from, Unit to) {
  ...
  if (from == Unit::KWH && (to == Unit::KVARH || to == Unit::KVAH))
    return true;
  return false;
}
...
bool canConvert(Unit ufrom, Unit uto) {
  if (ufrom == uto)
    return true;
#define X(from, to, code) \
  if (Unit::from == ufrom && Unit::to == uto) \
    return true;
  LIST_OF_CONVERSIONS
#undef X
  return false;
...
bool isKCF(const SIExp &e) { return e == SI_K.exp() || e == SI_C.exp() || e == SI_F.exp(); }

bool is_S_MONTH_YEAR_UT(const SIExp &e) {
  return e == SI_Second.exp() || e == SI_UnixTimestamp.exp() || e == SI_Month.exp() || e == SI_Year.exp();
}
...
bool SIUnit::convertTo(double left, const SIUnit &out_siunit, double *out) const {
  if (exp() == out_siunit.exp()) {
    if (out != NULL)
      *out = (left * scale_) / out_siunit.scale_;
    return true;
  }
  ...
  if (isKCF(exp()) && isKCF(out_siunit.exp())) {
    ...
    if (out != NULL)
      *out = ((left + from_offset) * from_scale) / to_scale - to_offset;
    return true;
  }
...
bool forbidden_op(MathOp op, const SIExp &a, const SIExp &b) {
  ...
  if (op == MathOp::ADD && a == SI_UnixTimestamp.exp() && b == SI_UnixTimestamp.exp())
    return true;
...
bool SIUnit::mathOpTo(MathOp op, double left, double right, const SIUnit &right_siunit, SIUnit *out_siunit,
                      double *out) const {
  // Adding all values with the same units.
  if (exp() == right_siunit.exp()) {
    if (forbidden_op(op, exp(), right_siunit.exp())) {
      if (out_siunit != NULL)
        *out_siunit = SI_COUNTER;
      if (out != NULL)
        *out = std::numeric_limits<double>::quiet_NaN();
      return false;
    }
...
SIUnit SIUnit::mul(const SIUnit &m) const {
  ...
  SIUnit tmp(Quantity::Unknown, new_scale, exps);
...
SIUnit whenMultiplied(SIUnit left, SIUnit right) { return Unit::Unknown; }

double multiply(double l, SIUnit left, double r, SIUnit right) { return 0; }
...
#define X(qname, qunit) \
  if (q == #qname) \
    return Quantity::qname;
  LIST_OF_QUANTITIES
#undef X
  return Quantity::Unknown;
}
...
#define X(quantity, default_unit) \
...
#define X(cname, si_scale, si_exponents) \
  if ((scale_ == si_scale) && (exponents_ == (si_exponents)) && quantity_ == toQuantity(Unit::cname)) \
    return Unit::cname;
  LIST_OF_SI_CONVERSIONS
#undef X
...
#define X(cname, si_scale, si_exponents) \
  if ((scale_ == si_scale) && (exponents_ == (si_exponents)) && q == toQuantity(Unit::cname)) \
    return Unit::cname;
  LIST_OF_SI_CONVERSIONS
#undef X
...
string SIUnit::info() const {
  ...
  return tostrprintf("[%s%s%s]", unit.c_str(), quantity.c_str(), str().c_str());
}
...
bool SIExp::operator!=(const SIExp &e) const { return !(*this == e); }

bool SIExp::operator==(const SIExp &e) const {
  return s_ == e.s_ && m_ == e.m_ && kg_ == e.kg_ && a_ == e.a_ && mol_ == e.mol_ && cd_ == e.cd_ && k_ == e.k_ &&
         c_ == e.c_ && f_ == e.f_ && month_ == e.month_ && year_ == e.year_ && unix_timestamp_ == e.unix_timestamp_;
}
...
#define DO_UNIT_SIEXP(var, name) \
  if (var != 0) { \
    if (r.length() > 0) { \
    } \
    r += #name; \
    if (var != 1) { \
      r += to_superscript(var); \
    } \
  }
...
const char *availableQuantities() {
  if (available_quantities_[0])
    return available_quantities_;

#define X(q, u) \
  if (Quantity::q != Quantity::Unknown) { \
    strcat(available_quantities_, #q); \
    strcat(available_quantities_, "\n"); \
...
#define X(n, suffix, sn, q, ln) \
  if (Unit::n != Unit::Unknown) { \
    strcat(available_units_, #suffix); \
    strcat(available_units_, "\n"); \
