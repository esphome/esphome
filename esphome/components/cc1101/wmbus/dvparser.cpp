...
bool isInsideVIFRange(Vif vif, VIFRange vif_range) {
  ...
  if (vif_range == VIFRange::AnyEnergyVIF) {
    return isInsideVIFRange(vif, VIFRange::EnergyWh) || isInsideVIFRange(vif, VIFRange::EnergyMJ) ||
...
bool parseDV(Telegram *t, vector<uchar> &databytes, vector<uchar>::iterator data, size_t data_len,
             std::map<string, pair<int, DVEntry>> *dv_entries, vector<uchar>::iterator *format, size_t format_len,
  ...
  for (;;) {
    ...
    if (force_mfct_index != -1) {
      ...
      if (index >= force_mfct_index) {
        ...
        t->addExplanationAndIncrementPos(data, datalen, KindOfData::CONTENT, Understanding::NONE,
    ...
    if (datalen == -2) {
      ...
      if (dif == 0x1f) {
        ...
        t->mfct_0f_index = 1 + std::distance(data_start, data);
        assert(t->mfct_0f_index >= 0);
        t->addExplanationAndIncrementPos(data, datalen, KindOfData::CONTENT, Understanding::FULL,
                                         "%02X more data in next telegram %s", dif, value.c_str());
        break;
      }
      DEBUG_PARSER(
          "(dvparser) reached unknown dif %02x treating remaining data as manufacturer specific, parsing is done.",
          dif);
      datalen = std::distance(data, data_end);
      string value = bin2hex(data + 1, data_end, datalen - 1);
      t->mfct_0f_index = 1 + std::distance(data_start, data);
    ...
    if (data_has_difvifs) {
      ...
      t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X dif (%s)", dif,
                                       difType(dif).c_str());
    } else {
    ...
    while (has_another_dife) {
      ...
      if (data_has_difvifs) {
        ...
        t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL,
                                         "%02X dife (subunit=%d tariff=%d storagenr=%d)", dife, subunit, tariff,
                                         storage_nr);
      } else {
        id_bytes.push_back(**format);
        (*format)++;
    ...
    if (data_has_difvifs) {
      ...
      t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X vif (%s)", vif,
                                       vifType(vif).c_str());
    } else {
      id_bytes.push_back(**format);
      (*format)++;
    ...
    if (vif == 0x7c) {
      ...
      t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X viflen (%d)",
                                       viflen, viflen);
      for (uchar i = 0; i < viflen; ++i) {
        if (*format == format_end) {
          debug("(dvparser) warning: unexpected end of data (vif varlen byte %d/%d expected)", i + 1, viflen);
          break;
        }
        uchar v = **format;
        t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X vif (%c)", v, v);
    ...
    while (has_another_vife) {
      ...
      if (extension_vif) {
        ...
        extension_vif = false;
        if (data_has_difvifs) {
          t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X vife (%s)",
    ...
    if (variable_length) {
      ...
      datalen = *(data);
      t->addExplanationAndIncrementPos(data, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X varlen=%d",
                                       *(data + 0), datalen);
    ...
    (*dv_entries)[key] = {offset,
                          DVEntry(offset, key, mt, Vif(full_vif), found_combinable_vifs, found_combinable_vifs_raw,
                                  StorageNr(storage_nr), TariffNr(tariff), SubUnitNr(subunit), value)};
...
bool findKey(MeasurementType mit, VIFRange vif_range, StorageNr storagenr, TariffNr tariffnr, std::string *key,
...
bool findKeyWithNr(MeasurementType mit, VIFRange vif_range, StorageNr storagenr, TariffNr tariffnr, int nr,
  ...
  for (auto &v : *dv_entries) {
    ...
    if (isInsideVIFRange(vi, vif_range) && (mit == MeasurementType::Instantaneous || mit == ty) &&
        (storagenr == AnyStorageNr || storagenr == sn) && (tariffnr == AnyTariffNr || tariffnr == tn)) {
      *key = v.first;
      nr--;
      if (nr <= 0)
        return true;
      debug("(dvparser) found key %s for type=%s vif=%x storagenr=%d", v.first.c_str(), measurementTypeName(ty).c_str(),
...
bool extractDVuint8(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, uchar *value) {
...
bool extractDVuint16(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, uint16_t *value) {
  if ((*dv_entries).count(key) == 0) {
...
bool extractDVuint24(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, uint32_t *value) {
  if ((*dv_entries).count(key) == 0) {
...
bool extractDVuint32(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, uint32_t *value) {
  if ((*dv_entries).count(key) == 0) {
...
bool extractDVdouble(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, double *value,
...
bool checkSizeHex(size_t expected_len, DifVifKey &dvk, string &v) {
  ...
  warning("(dvparser) bad decode since difvif %s expected %d hex chars but got \"%s\"", dvk.str().c_str(), expected_len,
          v.c_str());
  return false;
}
...
bool DVEntry::extractDouble(double *out, bool auto_scale, bool force_unsigned) {
             ...
             t == 0x7)    // 64 Bit Integer/Binary
  {
    ...
    if (t == 0x1) {
      ...
      if (!checkSizeHex(8, dif_vif_key, value))
        return false;
      assert(v.size() == 4);
      raw = ((unsigned int) v[3]) * 256 * 256 * 256 + ((unsigned int) v[2]) * 256 * 256 + ((unsigned int) v[1]) * 256 +
            ((unsigned int) v[0]);
      ...
      if (!checkSizeHex(12, dif_vif_key, value))
        return false;
      assert(v.size() == 6);
      raw = ((uint64_t) v[5]) * 256 * 256 * 256 * 256 * 256 + ((uint64_t) v[4]) * 256 * 256 * 256 * 256 +
            ((uint64_t) v[3]) * 256 * 256 * 256 + ((uint64_t) v[2]) * 256 * 256 + ((uint64_t) v[1]) * 256 +
            ((uint64_t) v[0]);
      ...
      raw = ((uint64_t) v[7]) * 256 * 256 * 256 * 256 * 256 * 256 * 256 +
            ((uint64_t) v[6]) * 256 * 256 * 256 * 256 * 256 * 256 + ((uint64_t) v[5]) * 256 * 256 * 256 * 256 * 256 +
            ((uint64_t) v[4]) * 256 * 256 * 256 * 256 + ((uint64_t) v[3]) * 256 * 256 * 256 +
            ((uint64_t) v[2]) * 256 * 256 + ((uint64_t) v[1]) * 256 + ((uint64_t) v[0]);
             ...
             t == 0xE)    // 12 digit BCD
  {
    ...
    if (t == 0x9) {
      ...
      raw = (v[2] - '0') * 10 * 10 * 10 + (v[3] - '0') * 10 * 10 + (v[0] - '0') * 10 + (v[1] - '0');
    } else if (t == 0xB) {
      ...
      raw = (v[4] - '0') * 10 * 10 * 10 * 10 * 10 + (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 +
      ...
      raw = (v[6] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 + (v[7] - '0') * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[4] - '0') * 10 * 10 * 10 * 10 * 10 + (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 +
            (v[3] - '0') * 10 * 10 + (v[0] - '0') * 10 + (v[1] - '0');
    } else if (t == 0xE) {
      ...
      raw = (v[10] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[11] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[8] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[9] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 + (v[6] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[7] - '0') * 10 * 10 * 10 * 10 * 10 * 10 + (v[4] - '0') * 10 * 10 * 10 * 10 * 10 +
            (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 + (v[3] - '0') * 10 * 10 +
            (v[0] - '0') * 10 + (v[1] - '0');
    }
    double scale = 1.0;
    double draw = (double) raw;
...
bool extractDVlong(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, uint64_t *out) {
  if ((*dv_entries).count(key) == 0) {
...
bool extractDVHexString(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, string *value) {
  if ((*dv_entries).count(key) == 0) {
...
bool extractDVReadableString(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, string *out) {
  if ((*dv_entries).count(key) == 0) {
    verbose("(dvparser) warning: cannot extract string from non-existant key \"%s\"", key.c_str());
    *offset = -1;
    return false;
...
string DVEntry::str() {
  string s = tostrprintf("%d: %s %s vif=%x %s%s st=%d ta=%d su=%d", offset, dif_vif_key.str().c_str(),
                         toString(measurement_type), vif.intValue(), combinable_vifs.size() > 0 ? "HASCOMB " : "",
                         combinable_vifs_raw.size() > 0 ? "HASCOMBRAW " : "", storage_nr.intValue(),
                         tariff_nr.intValue(), subunit_nr.intValue());
...
bool extractDVdate(std::map<string, pair<int, DVEntry>> *dv_entries, string key, int *offset, struct tm *out) {
  if ((*dv_entries).count(key) == 0) {
...
bool FieldMatcher::matches(DVEntry &dv_entry) {
  ...
  bool b = (!match_vif_range || isInsideVIFRange(dv_entry.vif, vif_range)) &&
  ...
  if (vif_combinables.size() == 0 && vif_combinables_raw.size() == 0) {
    ...
    if (dv_entry.combinable_vifs.size() == 0 && dv_entry.combinable_vifs_raw.size() == 0)
      return true;
