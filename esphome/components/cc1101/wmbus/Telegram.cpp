...
uchar vec87[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x87};
...
bool decrypt_TPL_AES_CBC_IV(Telegram *t, vector<uchar> &frame, vector<uchar>::iterator &pos, vector<uchar> &aeskey,
  ...
  *num_encrypted_bytes = num_bytes_to_decrypt;
  ...
  if (buffer.size() < num_bytes_to_decrypt) {
    warning("(TPL) warning: aes-cbc-iv decryption received less bytes than expected for decryption! "
            ...
            buffer.size(), num_bytes_to_decrypt, t->tpl_num_encr_blocks);
  ...
  *num_not_encrypted_at_end = buffer.size() - num_bytes_to_decrypt;
  ...
  debug("(TPL) num encrypted blocks %zu (%d bytes and remaining unencrypted %zu bytes)", t->tpl_num_encr_blocks,
        num_bytes_to_decrypt, buffer.size() - num_bytes_to_decrypt);
...
bool decrypt_TPL_AES_CBC_NO_IV(Telegram *t, vector<uchar> &frame, vector<uchar>::iterator &pos, vector<uchar> &aeskey,
  ...
  *num_not_encrypted_at_end = buffer.size() - num_bytes_to_decrypt;
  ...
  debug("(TPL) num encrypted blocks %d (%d bytes and remaining unencrypted %d bytes)", t->tpl_num_encr_blocks,
        num_bytes_to_decrypt, buffer.size() - num_bytes_to_decrypt);
...
void Telegram::print() {
  ...
  if (ell_sec_mode != ELLSecurityMode::NoSecurity || tpl_sec_mode != TPLSecurityMode::NoSecurity) {
    enc = " encrypted";
  }

  debug("Received telegram from: %02x%02x%02x%02x", a, b, c, d);
  debug("          manufacturer: (%s) %s (0x%02x)", manufacturerFlag(dll_mfct).c_str(), manufacturer(dll_mfct).c_str(),
        dll_mfct);
  ...
  if (tpl_id_found) {
    ...
    debug("          manufacturer: (%s) %s (0x%02x)", manufacturerFlag(tpl_mfct).c_str(),
          manufacturer(tpl_mfct).c_str(), tpl_mfct);
...
void Telegram::printDLL() {
  if (about.type == FrameType::WMBUS) {
    ...
    debug("(telegram) DLL L=%02x C=%02x (%s) M=%04x (%s) A=%02x%02x%02x%02x VER=%02x TYPE=%02x (%s) (driver %s) DEV=%s "
          "RSSI=%d",
          dll_len, dll_c, cType(dll_c).c_str(), dll_mfct, man.c_str(), dll_id[0], dll_id[1], dll_id[2], dll_id[3],
          dll_version, dll_type, mediaType(dll_type, dll_mfct).c_str(), possible_drivers.c_str(), about.device.c_str(),
          about.rssi_dbm);
  }

  if (about.type == FrameType::MBUS) {
    verbose("(telegram) DLL L=%02x C=%02x (%s) A=%02x", dll_len, dll_c, cType(dll_c).c_str(), mbus_primary_address);
  }
}
...
void Telegram::printELL() {
  ...
  verbose("(telegram) ELL CI=%02x CC=%02x (%s) ACC=%02x", ell_ci, ell_cc, ell_cc_info.c_str(), ell_acc);

  if (ell_ci == 0x8d || ell_ci == 0x8f) {
    string ell_sn_info = toStringFromELLSN(ell_sn);

    verbose(" SN=%02x%02x%02x%02x (%s) CRC=%02x%02x", ell_sn_b[0], ell_sn_b[1], ell_sn_b[2], ell_sn_b[3],
            ell_sn_info.c_str(), ell_pl_crc_b[0], ell_pl_crc_b[1]);
  }
  if (ell_ci == 0x8e || ell_ci == 0x8f) {
    string man = manufacturerFlag(ell_mfct);
    verbose(" M=%02x%02x (%s) ID=%02x%02x%02x%02x", ell_mfct_b[0], ell_mfct_b[1], man.c_str(), ell_id_b[0], ell_id_b[1],
            ell_id_b[2], ell_id_b[3]);
  }
}
...
void Telegram::printNWL() {
  ...
  verbose("(telegram) NWL CI=%02x", nwl_ci);
}
...
void Telegram::printAFL() {
  ...
  verbose("(telegram) AFL CI=%02x", afl_ci);
}
...
void Telegram::printTPL() {
  ...
  if (tpl_ci == 0x7a || tpl_ci == 0x72) {
    ...
    debug(" ACC=%02x STS=%02x CFG=%04x (%s)", tpl_acc, tpl_sts, tpl_cfg, tpl_cfg_info.c_str());
  }

  if (tpl_ci == 0x72) {
    string info = mediaType(tpl_type, tpl_mfct);
    debug(" ID=%02x%02x%02x%02x MFT=%02x%02x VER=%02x TYPE=%02x (%s)", tpl_id_b[0], tpl_id_b[1], tpl_id_b[2],
          tpl_id_b[3], tpl_mfct_b[0], tpl_mfct_b[1], tpl_version, tpl_type, info.c_str());
  }
}
...
bool isCiFieldManufacturerSpecific(int ci_field) { return ci_field >= 0xA0 && ci_field <= 0xB7; }
...
string ciType(int ci_field) {
  ...
  switch (ci_field) {
    ...
    case 0x63:
      return "?";
    case 0x64:
      return "Reserved for OBIS-based Data sent by the Readout device to the meter (long tplh)";
    case 0x65:
      return "Reserved for OBIS-based Data sent by the Readout device to the meter (short tplh)";
    case 0x66:
    ...
    case 0x80:
      return "EN 13757-3 Transport Layer (long tplh) from other device to the meter";
    ...
    case 0x8A:
      return "EN 13757-3 Transport Layer (short tplh) from the meter to the other device";  // No application layer,
                                                                                            // e.g. ACK
    case 0x8B:
      return "EN 13757-3 Transport Layer (long tplh) from the meter to the other device";  // No application layer, e.g.
                                                                                           // ACK
    ...
    case 0x8F:
      return "ELL: Extended Link Layer IV (16 Byte)";  // CC, ACC, M2, A2, SN, Payload CRC
...
void Telegram::addExplanationAndIncrementPos(vector<uchar>::iterator &pos, int len, KindOfData k, Understanding u,
                                             const char *fmt, ...) {
  char buf[1024];
  buf[1023] = 0;
...
void Telegram::addMoreExplanation(int pos, string json) { addMoreExplanation(pos, " (%s)", json.c_str()); }
...
bool Telegram::parseMBusDLLandTPL(vector<uchar>::iterator &pos) {
  ...
  dll_len = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x length (%d bytes)", dll_len,
                                dll_len);

  // Two identical length bytes are expected!
  if (*pos != dll_len)
    return false;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x length again (%d bytes)",
                                dll_len, dll_len);
  ...
  dll_c = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x dll-c (%s)", dll_c,
                                mbusCField(dll_c));

  mbus_primary_address = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x dll-a primary (%d)",
  ...
  mbus_ci = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-ci (%s)", mbus_ci,
                                mbusCiField(mbus_ci));
...
bool Telegram::parseELL(vector<uchar>::iterator &pos) {
  ...
  if (!isCiFieldOfType(ci_field, CI_TYPE::ELL))
    return true;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x ell-ci-field (%s)", ci_field,
  ...
  ell_cc = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x ell-cc (%s)", ell_cc,
                                ccType(ell_cc).c_str());
  ...
  if (has_session_number_pl_crc) {
    ...
    addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL,
                                  "%02x%02x payload crc (calculated %02x%02x %s)", ell_pl_crc_b[0], ell_pl_crc_b[1],
    ...
    if (ell_pl_crc == check || FUZZING) {
      ...
      addExplanationAndIncrementPos(pos, num_encrypted_bytes, KindOfData::CONTENT, Understanding::ENCRYPTED,
                                    info.c_str());
      ...
      if (parser_warns_) {
        if (!beingAnalyzed() && (isVerboseEnabled() || isDebugEnabled())) {
          ...
          warning("(wmbus) WARNING! decrypted payload crc failed check, did you use the correct decryption key? "
                  ...
                  "Permanently ignoring telegrams from id: %02x%02x%02x%02x mfct: (%s) %s (0x%02x) type: %s (0x%02x) "
                  "ver: 0x%02x",
                  ell_pl_crc_b[0], ell_pl_crc_b[1], check & 0xff, check >> 8, dll_id_b[3], dll_id_b[2], dll_id_b[1],
                  dll_id_b[0], manufacturerFlag(dll_mfct).c_str(), manufacturer(dll_mfct).c_str(), dll_mfct,
...
bool Telegram::parseNWL(vector<uchar>::iterator &pos) {
  ...
  if (!isCiFieldOfType(ci_field, CI_TYPE::NWL))
    return true;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x nwl-ci-field (%s)", ci_field,
                                ciType(ci_field).c_str());
  nwl_ci = ci_field;
...
bool Telegram::parseAFL(vector<uchar>::iterator &pos) {
  ...
  if (!isCiFieldOfType(ci_field, CI_TYPE::AFL))
    return true;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x afl-ci-field (%s)", ci_field,
                                ciType(ci_field).c_str());
  afl_ci = ci_field;

  afl_len = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x afl-len (%d)", afl_len,
                                afl_len);
  ...
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL, "%02x%02x afl-fc (%s)", afl_fc_b[0],
                                afl_fc_b[1], afl_fc_info.c_str());
  ...
  if (has_control) {
    ...
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x afl-mcl (%s)", afl_mcl,
                                  afl_mcl_info.c_str());
  }
  ...
  if (has_counter) {
    ...
    afl_counter = afl_counter_b[3] << 24 | afl_counter_b[2] << 16 | afl_counter_b[1] << 8 | afl_counter_b[0];
  ...
  if (has_mac) {
    ...
    addExplanationAndIncrementPos(pos, len, KindOfData::PROTOCOL, Understanding::FULL, "%s afl-mac %d bytes", s.c_str(),
                                  len);
    must_check_mac = true;
  }
...
bool Telegram::parseTPLConfig(std::vector<uchar>::iterator &pos) {
  ...
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL, "%02x%02x tpl-cfg %04x (%s)", cfg1,
                                cfg2, tpl_cfg, info.c_str());

  if (has_cfg_ext) {
    tpl_cfg_ext = *(pos + 0);
    tpl_kdf_selection = (tpl_cfg_ext >> 4) & 3;

    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-cfg-ext (KDFS=%d)",
                                  tpl_cfg_ext, tpl_kdf_selection);
    ...
    if (tpl_kdf_selection == 1) {
      ...
      AES_CMAC(safeButUnsafeVectorPtr(meter_keys->confidentiality_key), safeButUnsafeVectorPtr(input), 16,
...
string decodeTPLStatusByteOnlyStandardBits(uchar sts) {
  ...
  if ((sts & 0x03) == 0x03)
    s += "ALARM ";  // E.g. an abnormal condition like water is continuously running.

  if ((sts & 0x04) == 0x04)
    s += "POWER_LOW ";  // E.g. battery end of life or external power supply failure
  if ((sts & 0x08) == 0x08)
...
bool Telegram::parseShortTPL(std::vector<uchar>::iterator &pos) {
  tpl_acc = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-acc-field", tpl_acc);

  tpl_sts = *pos;
  tpl_sts_offset = distance(frame.begin(), pos);
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-sts-field (%s)", tpl_sts,
...
bool Telegram::parseLongTPL(std::vector<uchar>::iterator &pos) {
  ...
  addExplanationAndIncrementPos(pos, 4, KindOfData::PROTOCOL, Understanding::FULL,
                                "%02x%02x%02x%02x tpl-id (%02x%02x%02x%02x)", tpl_id_b[0], tpl_id_b[1], tpl_id_b[2],
                                tpl_id_b[3], tpl_id_b[3], tpl_id_b[2], tpl_id_b[1], tpl_id_b[0]);

  tpl_mfct_b[0] = *(pos + 0);
  tpl_mfct_b[1] = *(pos + 1);
  tpl_mfct = tpl_mfct_b[1] << 8 | tpl_mfct_b[0];
  string man = manufacturerFlag(tpl_mfct);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL, "%02x%02x tpl-mfct (%s)",
  ...
  tpl_a[5] = *(pos + 0);
  string info = mediaType(tpl_type, tpl_mfct);
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-type (%s)", tpl_type,
                                info.c_str());
...
bool Telegram::checkMAC(std::vector<uchar> &frame, std::vector<uchar>::iterator from, std::vector<uchar>::iterator to,
  ...
  AES_CMAC(safeButUnsafeVectorPtr(mackey), safeButUnsafeVectorPtr(input), input.size(), safeButUnsafeVectorPtr(mac));
...
bool Telegram::potentiallyDecrypt(vector<uchar>::iterator &pos) {
  if (tpl_sec_mode == TPLSecurityMode::AES_CBC_IV) {
    if (alreadyDecryptedCBC(pos)) {
      if (meter_keys && meter_keys->hasConfidentialityKey()) {
        ...
        warning("(wmbus) WARNING!! telegram should have been fully encrypted, but was not! "
                ...
                dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0], manufacturerFlag(dll_mfct).c_str(),
                manufacturer(dll_mfct).c_str(), dll_mfct, mediaType(dll_type, dll_mfct).c_str(), dll_type, dll_version);
        return false;
      }
      return true;
    }
    ...
    bool ok = decrypt_TPL_AES_CBC_IV(this, frame, pos, meter_keys->confidentiality_key, &num_encrypted_bytes,
      ...
      if (parser_warns_) {
        if (!beingAnalyzed() && (isVerboseEnabled() || isDebugEnabled())) {
          ...
          warning("(wmbus) WARNING! no key to decrypt payload! "
                  ...
                  "ver: 0x%02x",
                  dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0], manufacturerFlag(dll_mfct).c_str(),
                  manufacturer(dll_mfct).c_str(), dll_mfct, mediaType(dll_type, dll_mfct).c_str(), dll_type,
                  dll_version);
        }
      }
      return false;
    ...
    if (!mac_ok) {
      if (parser_warns_) {
        ...
        addExplanationAndIncrementPos(pos, frame.end() - pos, KindOfData::CONTENT, Understanding::ENCRYPTED,
                                      info.c_str());
    ...
    int num_not_encrypted_at_end = 0;
    bool ok =
        decrypt_TPL_AES_CBC_NO_IV(this, frame, pos, tpl_generated_key, &num_encrypted_bytes, &num_not_encrypted_at_end);
    if (!ok) {
...
bool Telegram::parse_TPL_79(vector<uchar>::iterator &pos) {
  ...
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL, "%02x%02x format signature", ecrc0,
                                ecrc1);
  ...
  if (!ok) {
    ...
    if (!ok) {
      ...
      addExplanationAndIncrementPos(pos, distance(pos, frame.end()), KindOfData::CONTENT, Understanding::COMPRESSED,
                                    info.c_str());
  ...
  int ecrc2 = *(pos + 0);
  int ecrc3 = *(pos + 1);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL, Understanding::FULL, "%02x%02x data crc", ecrc2, ecrc3);
...
bool Telegram::parseTPL(vector<uchar>::iterator &pos) {
  ...
  if (!isCiFieldOfType(ci_field, CI_TYPE::TPL) && !mfct_specific) {
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::NONE, "%02x unknown ci-field", ci_field);
    if (parser_warns_) {
  ...
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x tpl-ci-field (%s)", tpl_ci,
                                ciType(tpl_ci).c_str());
...
void Telegram::explainParse(string intro, int from) {
  for (auto &p : explanations) {
    ...
    if (p.kind == KindOfData::PROTOCOL && p.understanding == Understanding::FULL)
      u = " ";
...
string renderAnalysisAsJson(vector<Explanation> &explanations) { return "{ \"TODO\": true }\n"; }
...
string Telegram::analyzeParse(OutputFormat format, int *content_length, int *understood_content_length) {
  ...
  for (auto &e : explanations) {
    if (e.kind == KindOfData::CONTENT) {
      l += e.len;
      if (e.understanding == Understanding::PARTIAL || e.understanding == Understanding::FULL) {
...
string vifUnit(int vif) {
  ...
  switch (t) {
    ...
    default:
      warning("(wmbus) warning: generic type %d cannot be scaled!", t);
      return "unknown";
  }
}
...
bool handleTelegram(AboutTelegram &about, vector<uchar> frame) {
  ...
  if (about.type == FrameType::WMBUS) {
    ...
    if (frame.size() > 0 && expected_len != frame.size()) {
      warning("(wmbus) telegram length byte (the first) 0x%02x (%d) is probably wrong. Expected 0x%02x (%zu) based on "
              "the length of the telegram.",
...
void Telegram::extractFrame(vector<uchar> *fr) { *fr = frame; }
...
bool trimCRCsFrameFormatAInternal(std::vector<uchar> &payload, bool fail_is_ok) {
  ...
  if (calc_crc != check_crc && !FUZZING) {
    if (!fail_is_ok) {
      debug("(wmbus) ff a dll crc first (calculated %04x) did not match (expected %04x) for bytes 0-%zu!", calc_crc,
            check_crc, 10);
    }
    return false;
  }
  ...
  for (pos = 12; pos + 18 <= len; pos += 18) {
    ...
    if (calc_crc != check_crc && !FUZZING) {
      if (!fail_is_ok) {
        debug("(wmbus) ff a dll crc mid (calculated %04x) did not match (expected %04x) for bytes %zu-%zu!", calc_crc,
              check_crc, pos, to - 1);
      }
      return false;
    }
  ...
  if (pos < len - 2) {
    ...
    if (calc_crc != check_crc && !FUZZING) {
      if (!fail_is_ok) {
        debug("(wmbus) ff a dll crc final (calculated %04x) did not match (expected %04x) for bytes %zu-%zu!", calc_crc,
              check_crc, pos, tto - 1);
      }
      return false;
    }
  ...
  debug("(wmbus) trimmed %zu dll crc bytes from frame a and ignored %zu suffix bytes.", (len - new_len),
        (old_size - new_size) - (len - new_len));
...
bool trimCRCsFrameFormatBInternal(std::vector<uchar> &payload, bool fail_is_ok) {
  ...
  if (calc_crc != check_crc && !FUZZING) {
    if (!fail_is_ok) {
      debug("(wmbus) ff b dll crc (calculated %04x) did not match (expected %04x) for bytes 0-%zu!", calc_crc,
            check_crc, crc1_pos);
    }
    return false;
  }
  ...
  if (crc2_pos > 0) {
    ...
    if (calc_crc != check_crc && !FUZZING) {
      if (!fail_is_ok) {
        debug("(wmbus) ff b dll crc (calculated %04x) did not match (expected %04x) for bytes %zu-%zu!", calc_crc,
              check_crc, crc1_pos + 2, crc2_pos);
      }
      return false;
    }
  ...
  debug("(wmbus) trimmed %zu dll crc bytes from frame b and ignored %zu suffix bytes.", (len - new_len),
        (old_size - new_size) - (len - new_len));
...
bool trimCRCsFrameFormatA(std::vector<uchar> &payload) { return trimCRCsFrameFormatAInternal(payload, false); }

bool trimCRCsFrameFormatB(std::vector<uchar> &payload) { return trimCRCsFrameFormatBInternal(payload, false); }

FrameStatus checkWMBusFrame(vector<uchar> &data, size_t *frame_length, int *payload_len_out, int *payload_offset,
                            bool only_test) {
  ...
  *frame_length = payload_len + offset;
  if (data.size() < *frame_length) {
    ...
    if (only_test) {
      ...
      *payload_len_out = payload_len;
      *frame_length = payload_len + offset;
      warning("(wmbus) not enough bytes, frame length byte changed from %d(%02x) to %d(%02x)!", data[offset - 1],
              data[offset - 1], payload_len, payload_len);
...
bool isCloseEnough(int media1, int media2) { return genericifyMedia(media1) == genericifyMedia(media2); }
