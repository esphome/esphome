...
void spi_init() {
  ...
  spi_bus_config_t buscfg = {.sclk_io_num = CONFIG_SCK_GPIO,   // set SPI CLK pin
  ...
  spi_device_interface_config_t devcfg = {.clock_speed_hz = 5000000,  // SPI clock is 5 MHz!
                                          .queue_size = 7,
                                          .mode = 0,           // SPI mode 0
...
static void IRAM_ATTR gpio_isr_handler(void *arg) { _packetAvailable = true; }
 ...
 */
void setSyncWordArray(byte *sync) { setSyncWord(sync[0], sync[1]); }
