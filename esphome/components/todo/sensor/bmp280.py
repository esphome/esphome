...
PLATFORM_SCHEMA = sensor.PLATFORM_SCHEMA.extend({
    ...
    cv.Optional(CONF_ADDRESS, default=0x77): cv.i2c_address,
    cv.Required(CONF_TEMPERATURE): cv.nameable(BMP280_OVERSAMPLING_SENSOR_SCHEMA.extend({
        cv.GenerateID(): cv.declare_variable_id(BMP280TemperatureSensor),
    })),
    cv.Required(CONF_PRESSURE): cv.nameable(BMP280_OVERSAMPLING_SENSOR_SCHEMA.extend({
...
def to_code(config):
    ...
    if CONF_OVERSAMPLING in config[CONF_TEMPERATURE]:
        ...
        cg.add(bmp280.set_temperature_oversampling(constant))
    if CONF_OVERSAMPLING in config[CONF_PRESSURE]:
        constant = OVERSAMPLING_OPTIONS[config[CONF_PRESSURE][CONF_OVERSAMPLING]]
        cg.add(bmp280.set_pressure_oversampling(constant))
    if CONF_IIR_FILTER in config:
        constant = IIR_FILTER_OPTIONS[config[CONF_IIR_FILTER]]
        cg.add(bmp280.set_iir_filter(constant))
    ...
    register_component(bmp280, config)
