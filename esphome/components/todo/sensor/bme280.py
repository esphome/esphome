...
PLATFORM_SCHEMA = sensor.PLATFORM_SCHEMA.extend({
    ...
    cv.Required(CONF_TEMPERATURE): cv.nameable(BME280_OVERSAMPLING_SENSOR_SCHEMA.extend({
        cv.GenerateID(): cv.declare_variable_id(BME280TemperatureSensor),
    })),
    cv.Required(CONF_PRESSURE): cv.nameable(BME280_OVERSAMPLING_SENSOR_SCHEMA.extend({
        cv.GenerateID(): cv.declare_variable_id(BME280PressureSensor),
    })),
    cv.Required(CONF_HUMIDITY): cv.nameable(BME280_OVERSAMPLING_SENSOR_SCHEMA.extend({
...
def to_code(config):
    ...
    register_component(bme280, config)
