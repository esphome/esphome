...
class OpenTherm42Hub : public Component {
 ...
 protected:
  ...
  // Builds the next request to send, alternating between the essential list (things the master must
  // keep refreshing: the mandatory ids plus any active setpoint) and the informational list (optional
  // reads, round-robined one at a time) so a long informational list can never starve the essentials.
