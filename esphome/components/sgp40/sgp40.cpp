...
static const uint16_t sgp40_CMD_GET_FEATURESET = 0x0020;
static const uint16_t sgp40_CMD_IAQ_INIT = 0x2003;
static const uint16_t sgp40_CMD_MEASURE_IAQ = 0x2008;
static const uint16_t sgp40_CMD_SET_ABSOLUTE_HUMIDITY = 0x2061;
static const uint16_t sgp40_CMD_GET_IAQ_BASELINE = 0x2015;
static const uint16_t sgp40_CMD_SET_IAQ_BASELINE = 0x201E;
...
void SGP40Component::setup() {
  ...
  if (!this->write_command_(sgp40_CMD_IAQ_INIT)) {
    ESP_LOGE(TAG, "Sensor sgp40_iaq_init failed.");
...
bool SGP40Component::is_sensor_baseline_reliable_() {
...
void SGP40Component::read_iaq_baseline_() {
  if (this->is_sensor_baseline_reliable_()) {
    if (!this->write_command_(sgp40_CMD_GET_IAQ_BASELINE)) {
...
void SGP40Component::send_env_data_() {
  ...
  uint8_t data[4];
  data[0] = sgp40_CMD_SET_ABSOLUTE_HUMIDITY & 0xFF;
  data[1] = humidity_full;
  data[2] = humidity_dec;
  data[3] = crc;
  if (!this->write_bytes(sgp40_CMD_SET_ABSOLUTE_HUMIDITY >> 8, data, 4)) {
    ESP_LOGE(TAG, "Error sending compensation data.");
  }
}

void SGP40Component::write_iaq_baseline_(uint16_t eco2_baseline, uint16_t tvoc_baseline) {
  uint8_t data[7];
  data[0] = sgp40_CMD_SET_IAQ_BASELINE & 0xFF;
  ...
  if (!this->write_bytes(sgp40_CMD_SET_IAQ_BASELINE >> 8, data, 7)) {
  ...
  if (this->is_failed()) {
    switch (this->error_code_) {
      ...
      case MEASUREMENT_INIT_FAILED:
        ...
        break;
      case INVALID_ID:
        ESP_LOGW(TAG, "Sensor reported an invalid ID. Is this an sgp40?");
        break;
      case UNSUPPORTED_ID:
...
void SGP40Component::update() {
  if (!this->write_command_(sgp40_CMD_MEASURE_IAQ)) {
    this->status_set_warning();
    return;
  }
