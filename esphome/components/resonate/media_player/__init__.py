...
# TODO: This can't be the best way. Eventually, the resonate media player should only control the group and not play audio at all. Actual audio playback should be handled by the speaker media player (but I need to build a MediaSource abstraction layer first...)
def _final_validate(config):
    """Build the appropriate schema based on control_only setting."""
    base_schema = media_player.media_player_schema(ResonateMediaPlayer).extend(
        {
