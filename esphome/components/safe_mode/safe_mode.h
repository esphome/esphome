...
class SafeModeComponent : public Component {
 public:
  bool should_enter_safe_mode(uint8_t num_attempts, uint32_t enable_time, uint32_t attempt_time);
 ...
 protected:
  ...
  uint32_t safe_mode_attempt_time_{60000};  ///< The time that the boot has before resetting boot_successful
