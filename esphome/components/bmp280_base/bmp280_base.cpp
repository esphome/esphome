...
void BMP280Component::setup() {
  ...
  if (!this->read_byte_(0xD0, &chip_id)) {
  ...
  // Send a soft reset.
  if (!this->write_byte_(BMP280_REGISTER_RESET, BMP280_SOFT_RESET)) {
  ...
  do {
    delay(2);
    if (!this->read_byte_(BMP280_REGISTER_STATUS, &status)) {
  ...
  if (!this->read_byte_(BMP280_REGISTER_CONFIG, &config_register)) {
  ...
  if (!this->write_byte_(BMP280_REGISTER_CONFIG, config_register)) {
...
void BMP280Component::update() {
  ...
  if (!this->write_byte_(BMP280_REGISTER_CONTROL, meas_value)) {
...
float BMP280Component::read_pressure_(int32_t t_fine) {
  uint8_t data[3];
  if (!this->read_bytes_(BMP280_REGISTER_PRESSUREDATA, data, 3))
    return NAN;
  int32_t adc = ((data[0] & 0xFF) << 16) | ((data[1] & 0xFF) << 8) | (data[2] & 0xFF);
  adc >>= 4;
  if (adc == 0x80000) {
...
uint8_t BMP280Component::read_u8_(uint8_t a_register) {
  uint8_t data = 0;
  this->read_byte_(a_register, &data);
  return data;
}
uint16_t BMP280Component::read_u16_le_(uint8_t a_register) {
  uint16_t data = 0;
  this->read_byte_16_(a_register, &data);
