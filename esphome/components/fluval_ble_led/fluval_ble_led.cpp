...
void FluvalBleLed::send_packet_(std::vector<uint8_t> &data) {
  ...
  encrypt_(data);
  ESP_LOGV(TAG, "Writing packet encoded: %s to handle %x", this->pkt_to_hex_(data.data(), data.size()).c_str(), this->write_handle_);
  esp_err_t status = esp_ble_gattc_write_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), this->write_handle_, data.size(), const_cast<uint8_t *>(data.data()), ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
...
void FluvalBleLed::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
  ...
  switch (event) {
    ...
    case ESP_GATTC_READ_CHAR_EVT: {
      ...
      ESP_LOGE(TAG, "READ CHARS FROM %d with status %d: %s ",param->read.handle, param->read.status, this->pkt_to_hex_(param->read.value, param->read.value_len-1).c_str());

      if (this->handshake_step_ == 1) {
        std::vector<uint8_t> header2 { 0x47 };
        ESP_LOGW(TAG, "Writing Header 2 (REG 0x47");
        esp_err_t errb1 = esp_ble_gattc_write_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), this->write_reg_id_handle_, 1, const_cast<uint8_t *>(header2.data()), ESP_GATT_WRITE_TYPE_NO_RSP, ESP_GATT_AUTH_REQ_NONE);
        ESP_LOGW(TAG, "Writing 0x47 RESULT: %s", esp_err_to_name(errb1));
      ...
      if (this->handshake_step_ == 3) {
        ...
        if (now.is_valid()) {
          ...
          uint8_t dayOfWeek = now.day_of_week == 1 ? 7 : now.day_of_week - 1;
          uint8_t hour = now.hour;
          uint8_t minute = now.minute;
          uint8_t second = now.second;
          uint8_t checksum = 0x68 ^ 0x0E ^ year ^ month ^ day ^ dayOfWeek ^ hour ^ minute ^ second;

          ESP_LOGD(TAG, "Year: %d / Month: %d / Day: %d / Day of week: %d / Hour: %d / Minute: %d / Second: %d / Checksumm: %d", year, month, day, dayOfWeek, hour, minute, second, checksum);

          std::vector<uint8_t> value {0x68, 0x0E, year, month, day, dayOfWeek, hour, minute, second, checksum};
          this->send_packet_(value);
        }
    ...
    case ESP_GATTC_WRITE_CHAR_EVT: {
      ...
      if (this->handshake_step_ == 0) {
        ESP_LOGW(TAG, "Reading result of Header 1 (REG 0x0F");
        esp_err_t erra2 = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), this->read_reg_handle_, ESP_GATT_AUTH_REQ_NONE);
        ESP_LOGW(TAG, "READING 0x0F RESULT: %s", esp_err_to_name(erra2));
        this->handshake_step_ = 1;
      }

      if (this->handshake_step_ == 2) {
        ESP_LOGW(TAG, "Reading result of Header 2 (REG 0x47");
        esp_err_t errb2 = esp_ble_gattc_read_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), this->read_reg_handle_, ESP_GATT_AUTH_REQ_NONE);
        ESP_LOGW(TAG, "READING 0x47 RESULT: %s", esp_err_to_name(errb2));
    ...
    case ESP_GATTC_NOTIFY_EVT: {
      ESP_LOGV(TAG, "[%s] ESP_GATTC_NOTIFY_EVT: handle=0x%x, value=0x%x, len=%d",  this->parent_->address_str().c_str(), param->notify.handle, param->notify.value[0], param->notify.value_len);

      //ESP_LOGV(TAG, "Data Encrypted: %s ", this->pkt_to_hex_(param->notify.value, param->notify.value_len).c_str());

      uint8_t decryptedData[param->notify.value_len-3];
      memset(decryptedData, 0, param->notify.value_len-3);

      ESP_LOGE(TAG, "Data Ecrypted: %s ", this->pkt_to_hex_(param->notify.value, param->notify.value_len -1).c_str());
      this->decrypt_(param->notify.value, param->notify.value_len, decryptedData);

      uint8_t decryptedLength = param->notify.value_len - 3;
      // Start of packet - this is the header 0x68
      if (decryptedData[0] == 0x68) {
        // Full packet, expecting more
        if (decryptedLength == 17) {
          ESP_LOGW(TAG, "GOT START PACKET");
          this->received_packet_buffer_.clear();
          this->received_packet_buffer_.insert(this->received_packet_buffer_.end(), &decryptedData[0], &decryptedData[decryptedLength]);
          ESP_LOGW(TAG, "New Vector contents: %s ", this->pkt_to_hex_(this->received_packet_buffer_.data(), this->received_packet_buffer_.size()).c_str());
        }
        // Packet was not full. Decode it right away
        else {
          ESP_LOGW(TAG, "Final Vector contents: %s ", this->pkt_to_hex_(decryptedData, decryptedLength).c_str());
          this->decode_(decryptedData, decryptedLength);
        }
      }
      // Follow-up-Packet received
        ...
        if (decryptedLength == 17) {
          ...
          this->received_packet_buffer_.insert(this->received_packet_buffer_.end(), &decryptedData[0], &decryptedData[decryptedLength]);
          ESP_LOGW(TAG, "Intermediate Vector contents: %s ", this->pkt_to_hex_(this->received_packet_buffer_.data(), this->received_packet_buffer_.size()).c_str());
        }
        // Final follow up - decode the vector
        else {
          ESP_LOGW(TAG, "GOT FINAL PACKET");
          this->received_packet_buffer_.insert(this->received_packet_buffer_.end(), &decryptedData[0], &decryptedData[decryptedLength]);
          ESP_LOGW(TAG, "Final Vector contents: %s ", this->pkt_to_hex_(this->received_packet_buffer_.data(), this->received_packet_buffer_.size()).c_str());
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
      ...
      auto status2 = esp_ble_gattc_write_char_descr(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), 0x25, 1, const_cast<uint8_t *>(descriptor.data()), ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
      if (status2) {
...
void FluvalBleLed::loop() {
  ...
  if (this->synchronize_device_time_ && this->time_.has_value() == false) {
  ...
  if (this->synchronize_device_time_ && this->time_.has_value()) {
      ...
      esp_err_t erra1 = esp_ble_gattc_write_char(this->parent()->get_gattc_if(), this->parent()->get_conn_id(), this->write_reg_id_handle_, 1, const_cast<uint8_t *>(header1.data()), ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
