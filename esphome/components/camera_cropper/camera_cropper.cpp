...
CameraCropper::CameraCropper(camera::CameraImageSpec *spec, camera::CameraImage *output, int crop_x, int crop_y, int crop_width, int crop_height) {
...
size_t CameraCropper::process_pixels(camera::CameraImageSpec *input_spec, camera::CameraImage *input) {
  ...
  if (crop_x_ + crop_width_ > input_spec->width || 
      crop_y_ + crop_height_ > input_spec->height) {
