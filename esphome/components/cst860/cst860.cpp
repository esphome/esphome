...
void CST860Component::check_touch_() {
  ...
  // err = read_register(0x01, &gesture, 1);
  // if (err) {
  //   ESP_LOGI("TAG", "Reg 0x1 Error: %i", err);
  // }
  // ESP_LOGI(TAG, "Gesture: %i", gesture);
  ...
  if (!this->invert_x_)
    touchpoint.x = x; // this->x_raw_max_ * this->display_->get_width();
  else
    touchpoint.x = this->x_raw_max_ - x; // this->x_raw_max_ * this->display_->get_width();
  if (!this->invert_y_)
    touchpoint.y = y; // this->y_raw_max_ * this->display_->get_height();
  else
