...
void FRAM::write8(uint16_t memaddr, uint8_t value) {
  uint8_t val = value;
  this->write_block_(memaddr, (uint8_t *) &val, sizeof(uint8_t));
}

void FRAM::write16(uint16_t memaddr, uint16_t value) {
  uint16_t val = value;
  this->write_block_(memaddr, (uint8_t *) &val, sizeof(uint16_t));
}

void FRAM::write32(uint16_t memaddr, uint32_t value) {
  uint32_t val = value;
  this->write_block_(memaddr, (uint8_t *) &val, sizeof(uint32_t));
}

void FRAM::write_float(uint16_t memaddr, float value) {
  float val = value;
  this->write_block_(memaddr, (uint8_t *) &val, sizeof(float));
}

void FRAM::write_double(uint16_t memaddr, double value) {
  double val = value;
  this->write_block_(memaddr, (uint8_t *) &val, sizeof(double));
}
...
void FRAM::write(uint16_t memaddr, uint8_t *obj, uint16_t size) {
  ...
  while (size >= blocksize) {
    this->write_block_(memaddr, p, blocksize);
    memaddr += blocksize;
    p += blocksize;
    size -= blocksize;
  }
  //  remaining
  if (size > 0) {
    this->write_block_(memaddr, p, size);
  }
}

uint8_t FRAM::read8(uint16_t memaddr) {
  uint8_t val;
  this->read_block_(memaddr, (uint8_t *) &val, sizeof(uint8_t));
  return val;
}

uint16_t FRAM::read16(uint16_t memaddr) {
  uint16_t val;
  this->read_block_(memaddr, (uint8_t *) &val, sizeof(uint16_t));
  return val;
}

uint32_t FRAM::read32(uint16_t memaddr) {
  uint32_t val;
  this->read_block_(memaddr, (uint8_t *) &val, sizeof(uint32_t));
  return val;
}

float FRAM::read_float(uint16_t memaddr) {
  float val;
  this->read_block_(memaddr, (uint8_t *) &val, sizeof(float));
  return val;
}

double FRAM::read_double(uint16_t memaddr) {
  double val;
  this->read_block_(memaddr, (uint8_t *) &val, sizeof(double));
  return val;
}
...
void FRAM::read(uint16_t memaddr, uint8_t *obj, uint16_t size) {
  ...
  while (size >= blocksize) {
    this->read_block_(memaddr, p, blocksize);
    memaddr += blocksize;
    p += blocksize;
    size -= blocksize;
  }
  // remainder
  if (size > 0) {
    this->read_block_(memaddr, p, size);
  }
}
...
uint32_t FRAM::clear(uint8_t value) {
  ...
  for (uint32_t address = start; address < end; address += 16) {
    this->write_block_(address, buffer, 16);
  }
...
void FRAM::write_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
...
void FRAM::read_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
  uint8_t maddr[] = {(uint8_t) (memaddr >> 8), (uint8_t) (memaddr & 0xFF)};
...
void FRAM32::write_block_(uint32_t memaddr, uint8_t *obj, uint8_t size) {
...
void FRAM32::read_block_(uint32_t memaddr, uint8_t *obj, uint8_t size) {
  uint8_t addr = this->address_;
...
void FRAM11::write_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
...
void FRAM11::read_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
  // Device uses Address Pages
  uint8_t device_addr_with_page_bits = this->address_ | ((memaddr & 0x0700) >> 8);
  uint8_t maddr = memaddr & 0xFF;
...
void FRAM9::write_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
  // Device uses Address Pages
...
void FRAM9::read_block_(uint16_t memaddr, uint8_t *obj, uint8_t size) {
  // Device uses Address Pages
  uint8_t device_addr_with_page_bits = this->address_ | ((memaddr & 0x0100) >> 8);
  uint8_t maddr = memaddr & 0xFF;
