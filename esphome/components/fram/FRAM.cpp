...
{
  i2c::ErrorCode err = this->bus_->write_readv(this->address_, nullptr, 0, nullptr, 0);
...
void FRAM::sleep()
{
  ...
  //this->bus_->write(FRAM_SLAVE_ID_, &addr, 1);
  this->bus_->write_readv(FRAM_SLAVE_ID_, &addr, 1, nullptr, 0);
  //this->bus_->write(FRAM_SLEEP_CMD >> 1, nullptr, 0);
...
uint16_t FRAM::_getMetaData(uint8_t field)
{
  ...
  i2c::ErrorCode err =this->bus_->write_readv(FRAM_SLAVE_ID_, &addr, 1, data, 3);
...
void FRAM::_writeBlock(uint16_t memaddr, uint8_t * obj, uint8_t size)
{
  ...
  v.push_back(memaddr >> 8);
  v.push_back(memaddr);
  v.insert(v.end(), obj, obj + size);
  
  //this->bus_->writev(this->address_, buff, 2, true);
  this->bus_->write_readv(this->address_, v.data(), v.size(), nullptr, 0);
}
...
void FRAM32::_writeBlock(uint32_t memaddr, uint8_t * obj, uint8_t size)
{
  ...
  //this->bus_->writev(this->address_, buff, 2, true);
  this->bus_->write_readv(_addr, v.data(), v.size(), nullptr, 0);
}
...
void FRAM32::_readBlock(uint32_t memaddr, uint8_t * obj, uint8_t size)
{
  ...
  uint8_t maddr[] = { (uint8_t)(memaddr >> 8), (uint8_t)(memaddr & 0xFF) };
  //this->bus_->write(this->address_, maddr, 2);
...
void FRAM11::_writeBlock(uint16_t memaddr, uint8_t * obj, uint8_t size)
{
  ...
  std::vector<uint8_t> v(size + 2);
  v.push_back(memaddr && 0xFF);
  v.push_back(memaddr);
  v.insert(v.end(), obj, obj + size);
  
  //this->bus_->writev(this->address_, buff, 2, true);
  this->bus_->write_readv(DeviceAddrWithPageBits, v.data(), v.size(), nullptr, 0);
}
...
void FRAM11::_readBlock(uint16_t memaddr, uint8_t * obj, uint8_t size)
{
  ...
  //this->bus_->write(DeviceAddrWithPageBits, &maddr, 1);
  //this->bus_->read(DeviceAddrWithPageBits, obj, size);
  this->bus_->write_readv(DeviceAddrWithPageBits, &maddr, 1, obj, size);
}
