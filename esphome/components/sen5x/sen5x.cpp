    ...
    if (this->write_command(SEN5X_CMD_VOC_ALGORITHM_STATE)) {
      ...
      this->set_timeout(550, [this]() {
        ...
        if (this->read_data(states, 4)) {
          ...
          if ((uint32_t) std::abs(static_cast<long>(this->voc_baselines_storage_.state0 - state0)) >
                  MAXIMUM_STORAGE_DIFF ||
              (uint32_t) std::abs(static_cast<long>(this->voc_baselines_storage_.state1 - state1)) >
                  MAXIMUM_STORAGE_DIFF) {
            this->seconds_since_last_store_ = 0;
