...
float WifiNowComponent::get_setup_priority() const
{
    return setup_priority::WIFI + 1;
}
...
float WifiNowComponent::get_loop_priority() const
{
    return 10.0f;
}
...
void WifiNowComponent::loop()
{
    ...
    while( !recievequeue_.empty())
    {
        ...
        if( !dispatched)
        {
            ...
            ESP_LOGE(TAG,
                "Packet lost, no dispatcher, bssid = "
...
void WifiNowComponent::set_channel(uint8_t channel)
{
    channel_ = channel;
}
uint8_t WifiNowComponent::get_channel() const
{
    return channel_;
}

void WifiNowComponent::set_aeskey(const aeskey_t &aeskey)
{
    this->aeskey_ = aeskey;
}

const optional<aeskey_t> &WifiNowComponent::get_aeskey() const
{
    return this->aeskey_;
}
...
void WifiNowComponent::add_peer(WifiNowPeer *peer)
{
    this->peers_.push_back(peer);
}

const std::vector<WifiNowPeer*> &WifiNowComponent::get_peers() const
{
    return this->peers_;
}
...
static void send_internal( const WifiNowPacket &packet, std::function<void(bool)> &&callback)
{
    ...
    if( esp_now_send(
        bssid,
        (uint8_t*)packet.get_packetdata().data(),
        packet.get_packetdata().size()
        ))
    {
...
#endif
{
    ...
    if( !queue.empty())
    {
        if( queue.front())
        {
            ...
            sendcallbackqueue_.emplace([=](){
                callback(status == 0 /*MT_TX_STATUS_OK or ESP_NOW_SEND_SUCCESS*/);
                });
        }
        queue.pop();
    }
