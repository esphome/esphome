...
WifiNowPacket::WifiNowPacket(
    ...
    : bssid_(bssid)
    , packetdata_(sizeof(packet_t) + payload.size())
{
...
const bssid_t WifiNowPacket::get_bssid() const
{
    return bssid_;
}

const packetdata_t WifiNowPacket::get_packetdata() const
{
    return packetdata_;
}
...
const payload_t WifiNowPacket::get_payload() const
{
    ...
    {
        return payload_t(
            packetdata_.cbegin() + sizeof(packet_t),
            packetdata_.cend()
            );
    }
}
