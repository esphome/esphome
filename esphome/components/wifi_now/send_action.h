...
template<typename... Ts> class WifiNowSendAction 
    : public Action<Ts...>
    , WifiNowSendActionBase
{
public:
    WifiNowSendAction( WifiNowComponent *component)
    ...
    const WifiNowPeer *get_peer() const
    {
        return peer_;
    }
    ...
    void add_on_fail(const std::vector<Action<Ts...,  WifiNowSendAction<Ts...> *> *> &actions)
    {
        ...
        this->fail_.add_action(new LambdaAction<Ts..., WifiNowSendAction<Ts...> *>([=](Ts... x, WifiNowSendAction<Ts...> * action) { this->play_next_(x...); }));
    }

    void add_on_success(const std::vector<Action<Ts..., WifiNowSendAction<Ts...> *> *> &actions)
    {
        this->success_.add_actions(actions);
        this->success_.add_action(new LambdaAction<Ts..., WifiNowSendAction<Ts...> *>([=](Ts... x, WifiNowSendAction<Ts...> * action) { this->play_next_(x...); }));
    }
    ...
    void send()
    {
        ...
        WifiNowPacket packet(
            this->peer_ ? this->peer_->get_bssid() : bssid_t(),
            this->servicekey_,
            get_payload()
            );
