...
PAYLOAD_GETTER_REGISTRY = Registry(None, c.CONF_ID)
...
def templated_payload_getter_to_code(type, config, payload_id, template_arg, args):
    ...
    template = yield cg.templatable(config[c.CONF_VALUE], args, type)
    cg.add(var.set_value(template))
    yield var

@register_payload_getter( c.PAYLOAD_BOOL, t.TemplatePayloadGetter, cv.maybe_simple_value( cv.Schema({
    cv.GenerateID(): cv.declare_id(t.TemplatePayloadGetter),
    cv.Required(c.CONF_VALUE): cv.templatable( cv.boolean),
    }), key=c.CONF_VALUE))
@coroutine
...
@register_payload_getter( c.PAYLOAD_FLOAT, t.TemplatePayloadGetter, cv.maybe_simple_value( cv.Schema({
    cv.GenerateID(): cv.declare_id(t.TemplatePayloadGetter),
    cv.Required(c.CONF_VALUE): cv.templatable( cv.float_),
    }), key=c.CONF_VALUE))
@coroutine
...
@register_payload_getter( c.PAYLOAD_STRING, t.TemplatePayloadGetter, cv.maybe_simple_value( cv.Schema({
    cv.GenerateID(): cv.declare_id(t.TemplatePayloadGetter),
    cv.Required(c.CONF_VALUE): cv.templatable( cv.string),
    }), key=c.CONF_VALUE))
@coroutine
...
@register_payload_getter( c.PAYLOAD_VECTOR, t.TemplatePayloadGetter, cv.maybe_simple_value( cv.Schema({
    cv.GenerateID(): cv.declare_id(t.TemplatePayloadGetter),
    cv.Required(c.CONF_VALUE): cv.templatable( cv.string),
    }), key=c.CONF_VALUE))
@coroutine
...
@register_payload_getter( c.PAYLOAD_BINARY_SENSOR_EVENT, t.TemplatePayloadGetter, cv.maybe_simple_value( cv.Schema({
    cv.GenerateID(): cv.declare_id(t.TemplatePayloadGetter),
    cv.Required(c.CONF_VALUE): cv.templatable( cv.enum(t.BINARY_SENSOR_EVENTS,lower=True)),
    }), key=c.CONF_VALUE))
@coroutine
def binary_sensor_event_payload_getter_to_code(config, payload_id, template_arg, args):
    yield templated_payload_getter_to_code( t.BinarySensorEvent, config, payload_id, template_arg, args)

@register_payload_getter( c.PAYLOAD_PAYLOAD, t.PayloadPayloadGetter, cv.maybe_simple_value( cv.Schema({
...
def payload_payload_getter_to_code(config, payload_id, template_arg, args):
    var = cg.new_Pvariable(payload_id, template_arg, args)
    template = yield cg.templatable(config[c.CONF_VALUE], args, t.payload_t)
    cg.add(var.set_value(template))
    yield var
