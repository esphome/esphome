...
DlmsMeterComponent::DlmsMeterComponent(const uint32_t receive_timeout_ms, const bool skip_crc_check,
  ...
  if (decryption_key != nullptr) {
    this->parser_.set_decryption_key(*dlms_parser::Aes128GcmDecryptionKey::from_hex(decryption_key));
  }

  if (authentication_key != nullptr) {
    this->parser_.set_authentication_key(*dlms_parser::Aes128GcmAuthenticationKey::from_hex(authentication_key));
  }
...
void DlmsMeterComponent::dump_config() {
  ...
  for (const auto &custom_pattern : this->custom_patterns_) {
    ...
    ESP_LOGCONFIG(TAG, "  Custom Pattern: '%s' (name: %s, priority: %d, default_obis: %.*s)",
                  custom_pattern.pattern, custom_pattern.name, custom_pattern.priority,
                  static_cast<int>(default_obis.size()), default_obis.data());
  }
