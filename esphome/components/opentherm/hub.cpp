...
#endif
        ;
    bool dhw_enable = this->dhw_enable &&
...
#endif
        ;
    bool cooling_enable = this->cooling_enable &&
...
#endif
        ;
    bool otc_active = this->otc_active &&
...
#endif
        ;
    bool ch2_active = this->ch2_active &&
...
#endif
        ;
    return ot->build_set_boiler_status_request(ch_enable, dhw_enable, cooling_enable, otc_active, ch2_active);
  }
...
OpenthermHub::OpenthermHub(void (*handle_interrupt_callback)(void),
                           void (*process_response_callback)(unsigned long, OpenThermResponseStatus))
    : Component(),
      handle_interrupt_callback(handle_interrupt_callback),
      process_response_callback(process_response_callback) {}

void IRAM_ATTR OpenthermHub::handle_interrupt() { this->ot->handle_interrupt(); }
...
void OpenthermHub::process_response(unsigned long response, OpenThermResponseStatus status) {
  ...
  if (!ot->is_valid_response(response)) {
    ESP_LOGW(TAG, "Received invalid OpenTherm response (id: %u): %08x, status=%s", id, response,
             // String(response, HEX).c_str(),
             String(ot->get_last_response_status()).c_str());
    return;
  }
...
void OpenthermHub::on_shutdown() { this->ot->end(); }
...
void OpenthermHub::loop() {
  if (this->ot->is_ready()) {
    ...
    if (this->sync_mode) {
      ...
      this->ot->send_request(request);
    } else {
      this->ot->send_request_aync(request);
