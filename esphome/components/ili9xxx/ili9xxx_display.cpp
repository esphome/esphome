...
void ILI9XXXDisplay::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr,
  ...
  esph_log_d(TAG, "draw_pixels_at %d/%d", x_start, y_start);
  if (w <= 0 || h <= 0)
    return;
  ...
  if (this->rotation_ != display::DISPLAY_ROTATION_0_DEGREES || bitness != display::COLOR_BITNESS_565 ||
      order != this->color_order_ || !big_endian) {

    esph_log_d(TAG, "Rotation %d, bitness %d, color_order %d/%d, big_endian %d",
               this->rotation_, bitness, order, this->color_order_, big_endian);
