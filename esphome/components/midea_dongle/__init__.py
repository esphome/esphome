...
CONF_STRATCHED_ICON = 'stratched_icon'
CONFIG_SCHEMA = cv.Schema({
    cv.GenerateID(): cv.declare_id(MideaDongle),
    cv.Optional(CONF_STRATCHED_ICON): cv.boolean,
...
def to_code(config):
    ...
    if CONF_STRATCHED_ICON in config:
        cg.add(var.use_stretched_icon(config[CONF_STRATCHED_ICON]))
