#include "midea_frame.h"

namespace esphome {
namespace midea_dongle {

const uint8_t NotifyFrame::INIT[] = {0xAA, 0x1F, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0D, 0x01,
                                     0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x01, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

uint8_t Frame::calc_crc_() const {
  static const uint8_t PROGMEM crc8_854_table[] = {
      };
  uint8_t crc = 0;
  uint8_t *src = this->pb_ + OFFSET_BODY;
  uint8_t len = this->len_() - OFFSET_BODY;
  while (--len)
    crc = pgm_read_byte(crc8_854_table + (crc ^ *src++));
  return crc;
}

uint8_t Frame::calc_cs_() const {
  uint8_t cs = 0;
  uint8_t *src = this->pb_ + OFFSET_LENGTH;
  uint8_t len = this->len_();
  while (--len)
    cs -= *src++;
  return cs;
}

void NotifyFrame::set_ip(const IPAddress &ip) {
  this->pb_[13] = ip[3];
  this->pb_[14] = ip[2];
  this->pb_[15] = ip[1];
  this->pb_[16] = ip[0];
}

}  // namespace midea_dongle
}  // namespace esphome
