...
void DS3231Component::update_rtc_time() {
  ...
  ESP_LOGI(TAG, "Updating DS3231 from SNTP: %04d-%02d-%02d %02d:%02d:%02d",
           sntp_time.year, sntp_time.month, sntp_time.day_of_month,
...
bool DS3231Component::update_rtc_manual_time(int year, int month, int day, int hour, int minute, int second) {
  ...
  ESP_LOGI(TAG, "Setting manual time: %04d-%02d-%02d %02d:%02d:%02d",
...
bool DS3231Component::read_time() {
  return this->get_rtc_time().is_valid();
}

bool DS3231Component::write_time() {
  if (this->time_id_ == nullptr) return false;

  ESPTime now = this->time_id_->now();
  return now.is_valid() ? this->write_manual_time(now.year, now.month, now.day_of_month, 
...
bool DS3231Component::write_manual_time(int year, int month, int day, int hour, int minute, int second) {
  ...
  if (success) {
    ESP_LOGD(TAG, "Time written: %04d-%02d-%02d %02d:%02d:%02d (DOW: %d)",
...
void DS3231Component::write_register(uint8_t reg, uint8_t value) {
...
void DS3231Component::debug_registers() {
  ...
  if (this->read_bytes(0x00, data, 19)) {
    ...
    ESP_LOGI(TAG, "Time:    %02X %02X %02X %02X %02X %02X %02X", 
