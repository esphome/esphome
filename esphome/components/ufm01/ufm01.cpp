...
static bool read_ufp_chip_error(const uint8_t data[32]) {
  return data[29] & 0x20;
}

static bool read_flow_direction_wrong(const uint8_t data[32]) {
  return data[29] & 0x08;
}

static bool read_empty_tube(const uint8_t data[32]) {
  return data[28] & 0x20;
}

static bool read_flow_rate_out_of_range(const uint8_t data[32]) {
  return data[29] & 0x04;
}
...
bool UFM01Component::reset_device_() {
  return this->send_command_(RESET_DEVICE);
}

bool UFM01Component::clear_accumulated_flow_() {
  return this->send_command_(CLEAR_ACCUMULATED_FLOW);
}

bool UFM01Component::set_active_mode_() {
  return this->send_command_(ACTIVE_MODE);
}
...
void UFM01Component::loop() {
  ...
  if (this->read_index_ == 32) {
    ...
    for (int i = 2; i < 31 && this->read_index_ == 32; ++i) {
      if (
        ...
        for (int j = i; j < 32; ++j) this->data_[j - i] = this->data_[j];
        this->read_index_ = 32 - i;
      }
    }
    if (this->read_index_ == 32) this->read_index_ = 0;
  }
  ...
  if (this->read_index_ < 32 && this->available()) {
    if (this->read_byte(&this->data_[this->read_index_])) {
      ...
      if (
        (this->read_index_ == 0 && this->data_[0] != 0x3C) ||
        (this->read_index_ == 1 && this->data_[1] != 0x32)
      ) {
