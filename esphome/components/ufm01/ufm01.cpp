...
static const std::vector<uint8_t> CLEAR_ACCUMULATED_FLOW = std::vector<uint8_t>{ 0xFE, 0xFE, 0x11, 0x5A, 0xFD, 0x57, 0x16 };
static const std::vector<uint8_t> RESET_DEVICE = std::vector<uint8_t>{ 0xFE, 0xFE, 0x11, 0x5D, 0xFD, 0x5A, 0x16 };

static float to_float(uint8_t data) {
...
static bool validate_data(uint8_t data[32]) {
  ...
  return 
    !(data[0] != 0x3C && log_problem("bit 0 exception", 0x3C, data[0])) &&
...
static float read_volume(uint8_t data[32]) {
    return (data[8] == 0x1A ? L_PER_M3 : 1.0) * (
      to_float(data[14]) * 10000000.0 + 
      to_float(data[13]) * 100000.0 + 
      to_float(data[12]) * 1000.0 +
      to_float(data[11]) * 10.0 +
      to_float(data[10]) * 0.1 +
      to_float(data[9]) * 0.001
    );
}

static float read_flow(uint8_t data[32]) {
    return (data[20] == 0x80 ? -1.0 : 1.0) * (
      to_float(data[19]) * 10000.0 + 
      to_float(data[18]) * 100.0 + 
      to_float(data[17]) + 
      to_float(data[16]) * 0.01
    )
    * M3_PER_L;
}
...
static float read_temperature(uint8_t data[32]) {
    if ( // happens sometimes before getting real reading
      data[27] == 0X00 &&
      (data[26] == 0x00 || data[26] == 0x70) &&
      data[25] == 0X00
    ) {
      return UNKNOWN;
    } else {
      return
        to_float(data[27]) * 100.0 + 
        to_float(data[26]) + 
        to_float(data[25]) * 0.01;
    }
}

static bool read_UFP_chip_error(uint8_t data[32]) {
  return data[29] & 0x20;
}

static bool read_flow_direction_wrong(uint8_t data[32]) {
  return data[29] & 0x08;
}

static bool read_empty_tube(uint8_t data[32]) {
  return data[28] & 0x20;
}

static bool read_flow_rate_out_of_range(uint8_t data[32]) {
  return data[29] & 0x04;
}

bool UFM01Component::send_command(std::vector<uint8_t> command) {
  this->write_array(command);
  return 0xE5 == this->read();
}

bool UFM01Component::reset_device() {
  return this->send_command(RESET_DEVICE);
}

bool UFM01Component::clear_accumulated_flow() {
  return this->send_command(CLEAR_ACCUMULATED_FLOW);
}

bool UFM01Component::set_active_mode() {
  return this->send_command(ACTIVE_MODE);
}

float UFM01Component::get_setup_priority() const {
...
void UFM01Component::on_data(uint8_t data[32]) {
...
void UFM01Component::loop() {
  ...
  if (this->read_index == 32 && validate_data(this->data)) {
  ...
  if (this->read_index == 32) {
    ...
    for (int i = 2; i < 31 && this->read_index == 32; ++i) {
      if (
        (this->data[i] == 0x3C) &&
        (this->data[i + 1] == 0x32)
      ) {
        for (int j = i; j < 32; ++j) this->data[j - i] = this->data[j];
        this->read_index = 32 - i;
      }
    }
    if (this->read_index == 32) this->read_index = 0;
  }
  if (this->read_index > 32) { // NOT EXPECTED
    ESP_LOGE(TAG, "Read more than 32 bytes into array");
    this->read_index = 0;
  }
  
  if (this->read_index < 32 && this->available()) {
    if (this->read_byte(&this->data[this->read_index])) {
      // ESP_LOGD(TAG, "%2d GOT %02X", this->read_index, this->data[this->read_index]);
      if (
        (this->read_index == 0 && this->data[0] != 0x3C) ||
        (this->read_index == 1 && this->data[1] != 0x32)
      ) {
        ESP_LOGW(TAG, "not start of data at %d (is 0x%02X)", this->read_index, this->data[this->read_index]);
        this->read_index = 0;
      } else {
        this->read_index++;
      }
