...
std::shared_ptr<nfc::NfcTag> PN532::read_mifare_ultralight_tag_(std::vector<uint8_t> &uid) {
  if (!this->is_mifare_ultralight_formatted_()) {
    ESP_LOGD(TAG, "Not NDEF formatted");
    return std::make_shared<nfc::NfcTag>(uid, nfc::NFC_FORUM_TYPE_2);
  }
  ...
  return std::make_shared<nfc::NfcTag>(uid, nfc::NFC_FORUM_TYPE_2, data);
}
