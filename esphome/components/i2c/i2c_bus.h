...
/// @brief the ReadBuffer structure holds a pointer to a read buffer and its length
struct ReadBuffer {
  uint8_t *data;  ///< pointer to the read buffer
  size_t len;     ///< length of the buffer
};

/// @brief the WriteBuffer structure holds a pointer to a write buffer and its length
struct WriteBuffer {
...
/// it also keeps an array of devices found on the bus.
/// @note The I2CBus virtual class follow the Factory design pattern that provides all
/// the interfaces methods required by clients while deferring the actual implementation
/// of these methods to the subclasses.
class I2CBus {
 public:
  /// @brief Creates a ReadBuffer and calls the readv() method to read bytes into this buffer
