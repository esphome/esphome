...
def write_component_boards(
    ...
    for board in boards:
        ...
        families[family] = f"VARIANT_{family}"
    ...
    code = subst_many(
        code,
        (VARIANTS, ", ".join(sorted(families.values()))),
...
def write_const(
    ...
    fam_regex = rf"(# {VARIANTS}.+?\n)(.*?)(\n# {VARIANTS})"
    # build family constants
    fam_defs = "\n".join(f'{VARIANT_PREFIX}{v} = "{v}"' for v in families)
    fam_list = ", ".join(f"{VARIANT_PREFIX}{v}" for v in families)
    fam_friendly = ", ".join(f'{VARIANT_PREFIX}{v}: "{v}"' for v in families)
    fam_component = ", ".join(
        f"{VARIANT_PREFIX}{v}: COMPONENT_{v2f[v]}" for v in families
    )
    fam_lines = [
        fam_defs,
        "VARIANTS = [",
        fam_list,
        ",]",
        f"{VARIANT_PREFIX}FRIENDLY = {{",
        fam_friendly,
        ",}",
        f"{VARIANT_PREFIX}COMPONENT = {{",
        fam_component,
        ",}",
    ]
    var_str = "\n".join(fam_lines)
