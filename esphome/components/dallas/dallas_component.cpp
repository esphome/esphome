  ...
  for (auto &address : raw_sensors) {
    ...
    if (this->auto_setup_sensors_) {
      ...
      if (!skip) {
        ...
        char sensor_name[64];
        snprintf(sensor_name, sizeof(sensor_name), this->sensor_name_template_.c_str(), App.get_name().c_str(), s.c_str());
...
void DallasComponent::set_auto_setup_sensors(bool auto_setup_sensors) { this->auto_setup_sensors_ = auto_setup_sensors; }
void DallasComponent::set_sensor_name_template(const std::string &sensor_name_template) { this->sensor_name_template_ = sensor_name_template; }
void DallasComponent::set_resolution(uint8_t resolution) { this->resolution_ = resolution; }
void DallasComponent::set_unit_of_measurement(const std::string &unit_of_measurement) { this->unit_of_measurement_ = unit_of_measurement; }
