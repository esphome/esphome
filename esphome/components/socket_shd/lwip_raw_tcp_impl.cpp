...
std::string format_addr(ip_addr_t *addr) {
  char buffer[50] = {};
  if (IP_IS_V4_VAL(*addr)) {
    inet_ntoa_r(*addr, buffer, sizeof(buffer));
  }
#if LWIP_IPV6
  else if (IP_IS_V6_VAL(*addr)) {
    inet6_ntoa_r(*addr, buffer, sizeof(buffer));
  }
#endif
...
class LWIPRawImpl : public Socket {
 public:
  LWIPRawImpl(sa_family_t family) : family_(family) {}
  ...
  int getsockname(struct sockaddr *name, socklen_t *addrlen) override {
    ...
    ip_addr_t *local_ip;
    uint16_t *local_port;
    this->pcb_get_local_addr_(&local_ip, &local_port); // accepts double pointers

    return this->ip2sockaddr_(local_ip, *local_port, name, addrlen);
  }
 ...
 protected:
  size_t read_(void *buf, size_t len, bool peek=false) {
    ...
    while (len && this->rx_buf_ != nullptr) {
      ...
      // FIXME || RENAMEME
      // tcp_recved(this->pcb_, len) is called on every iteration
      this->one_pbuf_was_read_(copysize);
  ...
  int sockaddr2ip_(const struct sockaddr *name, socklen_t addrlen, ip_addr_t *ip, uint16_t *port) {
#if LWIP_IPV6
  ...
  // RENAMEME
  virtual void one_pbuf_was_read_(size_t len) {};
  virtual bool pcb_valid_() = 0;
  virtual void pcb_get_local_addr_(ip_addr_t **ip, uint16_t **port) = 0;
...
class LWIPRawImplTCP : public LWIPRawImpl {
 public:
  LWIPRawImplTCP(sa_family_t family, struct tcp_pcb *pcb) : LWIPRawImpl(family), pcb_(pcb) {}
  ~LWIPRawImplTCP() override {
    if (pcb_ != nullptr) {
  ...
  void init() override {
    LWIP_LOG("tcp init(%p)", pcb_);
    tcp_arg(pcb_, this);
    tcp_accept(pcb_, LWIPRawImplTCP::s_accept_fn);
    tcp_recv(pcb_, LWIPRawImplTCP::s_recv_fn);
    tcp_err(pcb_, LWIPRawImplTCP::s_err_fn);
  }
  ...
  std::unique_ptr<Socket> accept(struct sockaddr *addr, socklen_t *addrlen) override {
    ...
    std::unique_ptr<LWIPRawImplTCP> sock = std::move(accepted_sockets_.front());
  ...
  int bind(const struct sockaddr *name, socklen_t addrlen) override {
    ...
    int r = this->sockaddr2ip_(name, addrlen, &ip, &port);
    if (r) return -1;

    LWIP_LOG("tcp_bind(%p ip=%s port=%u)", pcb_, ipaddr_ntoa(&ip), port);
  ...
  std::string getpeername() override {
    ...
    return format_addr(&pcb_->remote_ip);
  }
  ...
  err_t accept_fn(struct tcp_pcb *newpcb, err_t err) {
    ...
    auto sock = make_unique<LWIPRawImplTCP>(family_, newpcb);
    sock->init();
  ...
  static err_t s_accept_fn(void *arg, struct tcp_pcb *newpcb, err_t err) {
    LWIPRawImplTCP *arg_this = reinterpret_cast<LWIPRawImplTCP *>(arg);
 ...
 protected:
  // RENAMEME
  void one_pbuf_was_read_(size_t len) override {
    LWIP_LOG("tcp_recved(%p %u)", this->pcb_, len);
    tcp_recved(this->pcb_, len);
  }

  bool pcb_valid_() override {
    return this->pcb_ != nullptr;
  }

  void pcb_get_local_addr_(ip_addr_t **ip, uint16_t **port) override {
    *ip = &this->pcb_->local_ip;
...
class LWIPRawImplUDP : public LWIPRawImpl {
 public:
  LWIPRawImplUDP(sa_family_t family, struct udp_pcb *pcb) : LWIPRawImpl(family), pcb_(pcb) {}
  ~LWIPRawImplUDP() override {
    if (this->pcb_ != nullptr) {
      LWIP_LOG("udp_remove(%p)", this->pcb_);
  ...
  void init() override {
    LWIP_LOG("udp init(%p)", this->pcb_);
    udp_recv(this->pcb_, LWIPRawImplUDP::s_recv_fn, this);
  }
  ...
  int bind(const struct sockaddr *name, socklen_t addrlen) override {
    ...
    LWIP_LOG("udp_bind(%p ip=%s port=%u)", this->pcb_, ipaddr_ntoa(&ip), port);
  ...
  ssize_t sendto(const void *buf, size_t len, int flags, const struct sockaddr *to, socklen_t tolen) override {
    ...
    ip_addr_t addr;
    uint16_t port;
    int r = this->sockaddr2ip_(to, tolen, &addr, &port);
    if (r) {
      pbuf_free(pb);
      return -1;
    }

    err_t err = udp_sendto(this->pcb_, pb, &addr, port);
    pbuf_free(pb);
  ...
  void recv_fn(struct pbuf *pb, const ip_addr_t *addr, u16_t port) {
    LWIP_LOG("udp_recv(pb=%p)", pb);

    // a new pbuf (chain) is a new datagram
    // read it or lose it
    if (this->rx_buf_ != nullptr) {
      pbuf_free(this->rx_buf_);
    }
  ...
  static void s_recv_fn(void *arg, struct udp_pcb *pcb, struct pbuf *pb, const ip_addr_t *addr, u16_t port) {
    LWIPRawImplUDP *arg_this = reinterpret_cast<LWIPRawImplUDP *>(arg);
    arg_this->recv_fn(pb, addr, port);
  }
...
std::unique_ptr<Socket> socket(int domain, int type, int protocol) {
  ...
  if (type == SOCK_DGRAM) {
    ...
    sock = new LWIPRawImplUDP((sa_family_t) domain, pcb);  // NOLINT(cppcoreguidelines-owning-memory)
  } else {
    auto *pcb = tcp_new();
    if (pcb == nullptr) return nullptr;
    sock = new LWIPRawImplTCP((sa_family_t) domain, pcb);  // NOLINT(cppcoreguidelines-owning-memory)
  }
