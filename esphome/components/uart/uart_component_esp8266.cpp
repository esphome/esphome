...
// Cycles per bit for a byte received now. An 80 MHz build runs at 160 MHz while a CpuFrequencyBoost is open
// and a byte can arrive during one, so the live clock select bit doubles the compile time value. The whole byte
// is read inside the ISR, so the clock cannot change partway through. A 160 MHz build never changes clock.
...
void IRAM_ATTR HOT ESP8266SoftwareSerial::write_byte(uint8_t data) {
  ...
  {
    // Transmit runs from the main loop and never overlaps a CpuFrequencyBoost, so the compile time bit time holds
    InterruptLock lock;
