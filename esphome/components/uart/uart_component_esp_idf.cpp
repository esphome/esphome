...
/// Returns false and selects UART_SCLK_DEFAULT for unsupported sources.
/// Stores the selected clock in out_clk if non-null.
...
void IDFUARTComponent::load_settings(bool dump_config) {
  ...
  if (!resolve_clock_source(this->clock_source_)) {
    ESP_LOGW(TAG, "Clock source %s unsupported on this chip; using default", clock_source_to_str(this->clock_source_));
  }
...
void IDFUARTComponent::dump_config() {
  ...
  if (!resolve_clock_source(this->clock_source_)) {
    clock_source = "DEFAULT (unsupported selection)";
  }
  ESP_LOGCONFIG(TAG,
