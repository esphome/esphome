 ...
 *
 * It costs width*height/8 bytes and is allocated only when partial refresh
 * is enabled at all, so a display configured to refresh fully every time -
 * or a board too small to spare the memory - pays nothing for it.
 *
