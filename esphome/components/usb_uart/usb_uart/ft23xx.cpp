...
void USBUartTypeFT23XX::start_output(USBUartChannel *channel) {
  ...
  if (channel->debug_) {
    std::string debug_prefix = channel->get_debug_prefix();
    uart::UARTDebug::log_hex(uart::UART_DIRECTION_TX, std::vector<uint8_t>(tx_data, tx_data + len), ',', debug_prefix);
  }
#endif
