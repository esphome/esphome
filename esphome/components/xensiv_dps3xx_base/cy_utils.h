...
#define CY_SWAP_ENDIAN32(x) \
  ((uint32_t) ((((x) >> 24U) & 0x000000FFU) | (((x) &0x00FF0000U) >> 8U) | (((x) &0x0000FF00U) << 8U) | ((x) << 24U)))

/** Swap the byte ordering of a 64-bit value */
#define CY_SWAP_ENDIAN64(x) \
  ((uint64_t) (((uint64_t) CY_SWAP_ENDIAN32((uint32_t) (x)) << 32U) | CY_SWAP_ENDIAN32((uint32_t) ((x) >> 32U))))
...
#define CY_MISRA_DEVIATE_LINE(MISRA, MESSAGE) _Pragma(CY_COVERITY_PRAGMA_STR(coverity compliance deviate MISRA MESSAGE))

#define CY_MISRA_FP_LINE(MISRA, MESSAGE) _Pragma(CY_COVERITY_PRAGMA_STR(coverity compliance fp MISRA MESSAGE))
...
#define CY_MISRA_BLOCK_END(MISRA) _Pragma(CY_COVERITY_PRAGMA_STR(coverity compliance end_block MISRA))
