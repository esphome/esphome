...
optional<XiaomiParseResult> parse_xiaomi_header(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  ESP_LOGVV(TAG, "payload: %s",
            hexencode_string(std::string(reinterpret_cast<const char *>(device.get_service_data().data()),
                                         device.get_service_data().size()))
                .c_str());
...
Now switch and capabilities. Looking through other examples, it looks like the 'fourth' byte is the type of sensor (this
would allow for 255 sensors). The first two bytes describe the capabilities of the sensor and the particular message
*/
...
void decrypt_message(const esp32_ble_tracker::ESPBTDevice &device, xiaomi_ble::XiaomiParseResult &result,
  ...
  size_t aadlen = 1;
  int enc_length =
      result.data_length - 11 - 7;  // magic FTW (well actually it is the position of the MAC address plus the length
                                    // from the start [11], then token and payload from the end [7])
  ...
  ESP_LOGVV(TAG, "Data: %s",
            hexencode_string(
                std::string(reinterpret_cast<const char *>(device.get_service_data().data()), result.data_length))
                .c_str());
  ESP_LOGVV(TAG, "Token: %s", hexencode_string(std::string(reinterpret_cast<const char *>(token), 4)).c_str());
  ESP_LOGVV(TAG, "payload_counter: %s",
            hexencode_string(std::string(reinterpret_cast<const char *>(payload_counter), 3)).c_str());
  ESP_LOGVV(TAG, "Nonce: %s", hexencode_string(std::string(reinterpret_cast<const char *>(nonce), 12)).c_str());
  ESP_LOGVV(TAG, "encrypted_payload: %s",
            hexencode_string(std::string(reinterpret_cast<const char *>(encrypted_payload), enc_length)).c_str());
  ...
  ret = ccm_auth_crypt(&ctx, CCM_DECRYPT, 5 /* length*/, nonce, 12 /*iv_len*/, aad /*add*/, aadlen /* add_len*/,
  ...
  ESP_LOGVV(TAG, "decrypted message: %s",
            hexencode_string(std::string(reinterpret_cast<const char *>(message), 5)).c_str());
  ESP_LOGVV(TAG, "decrypted token: %s",
            hexencode_string(std::string(reinterpret_cast<const char *>(check_tag), 6)).c_str());
