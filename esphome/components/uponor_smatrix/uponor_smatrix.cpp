...
void UponorSmatrixComponent::loop() {
  ...
  while (this->available()) {
    // The controller polls devices every 10 seconds in some units or continuously in others with around 200 ms between devices.
  ...
  if (this->rx_buffer_.empty() && (now - this->last_rx_ > 50) && (now - this->last_rx_ < 100) && (now - this->last_tx_ > 200)) {
#ifdef USE_TIME
