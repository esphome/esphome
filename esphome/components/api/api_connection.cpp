...
void APIConnection::try_send_camera_image_() {
  ...
  while (this->image_reader_->available()) {
    ...
    if (!this->send_message(msg, CameraImageResponse::MESSAGE_TYPE)) {
...
bool APIConnection::try_send_log_message(int level, const char *tag, const char *line, size_t message_len) {
  ...
  return this->send_message(msg, SubscribeLogsResponse::MESSAGE_TYPE);
}
...
bool APIConnection::send_message(const ProtoMessage &msg, uint8_t message_type) {
#ifdef HAS_PROTO_MESSAGE_DUMP
  DumpBuffer dump_buf;
