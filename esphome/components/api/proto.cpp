...
void ProtoWriteBuffer::encode_sub_message_(uint32_t field_id, const void *value,
...
void ProtoWriteBuffer::encode_optional_sub_message_(uint32_t field_id, uint32_t nested_size, const void *value,
                                                    void (*encode_fn)(const void *, ProtoWriteBuffer &)) {
  if (nested_size == 0)
    return;
  this->encode_field_raw(field_id, 2);
