...
uint8_t *SerialProxyIdentity::encode(ProtoWriteBuffer &buffer PROTO_ENCODE_DEBUG_PARAM) const {
  ...
  ProtoEncode::encode_uint32(pos PROTO_ENCODE_DEBUG_ARG, 7, this->usb_vendor_id);
  ProtoEncode::encode_uint32(pos PROTO_ENCODE_DEBUG_ARG, 8, this->usb_product_id);
  ProtoEncode::encode_uint32(pos PROTO_ENCODE_DEBUG_ARG, 9, this->usb_bcd_device);
  ProtoEncode::encode_uint32(pos PROTO_ENCODE_DEBUG_ARG, 10, this->usb_interface_number);
  return pos;
}
uint32_t SerialProxyIdentity::calculate_size() const {
  ...
  size += ProtoSize::calc_uint32(1, this->usb_vendor_id);
  size += ProtoSize::calc_uint32(1, this->usb_product_id);
  size += ProtoSize::calc_uint32(1, this->usb_bcd_device);
  size += ProtoSize::calc_uint32(1, this->usb_interface_number);
  return size;
}
#endif
#ifdef USE_BLUETOOTH_PROXY_CONNECTIONS
