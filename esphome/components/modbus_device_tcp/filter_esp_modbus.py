...
_LEGACY_DEVICE = "sla" + "ve"  # keep upstream filename, avoid contiguous token in source
...
if env is not None:
    ...
    if os.path.isfile(dst_defaults):
        with open(dst_defaults, encoding="utf-8") as f:
            existing = f.read()
    ...
    if new_content:
        new_content += "\n"
    new_content += "\n# modbus_device_tcp: esp-modbus + 4MB flash\n"
    ...
    if lib_dir is None:
        ...
        mb_device_c = os.path.join(lib_dir, "modbus", "mb_objects", "mb_" + _LEGACY_DEVICE + ".c")
        ...
        port_tcp_device_c = os.path.join(
            lib_dir, "modbus", "mb_ports", "tcp", "port_tcp_" + _LEGACY_DEVICE + ".c"
        )
