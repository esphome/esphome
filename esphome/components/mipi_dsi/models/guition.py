from esphome.components.mipi import DriverChip
import esphome.config_validation as cv

# fmt: off
DriverChip(
    "JC1060P470",
    width=1024,
    height=600,
    hsync_back_porch=160,
    hsync_pulse_width=40,
    hsync_front_porch=160,
    vsync_back_porch=23,
    vsync_pulse_width=10,
    vsync_front_porch=12,
    pclk_frequency="54MHz",
    lane_bit_rate="750Mbps",
    swap_xy=cv.UNDEFINED,
    color_order="RGB",
    reset_pin=27,
    initsequence=[
        (0x30, 0x00), (0xF7, 0x49, 0x61, 0x02, 0x00), (0x30, 0x01), (0x04, 0x0C), (0x05, 0x00), (0x06, 0x00),
        (0x0B, 0x11), (0x17, 0x00), (0x20, 0x04), (0x1F, 0x05), (0x23, 0x00), (0x25, 0x19), (0x28, 0x18), (0x29, 0x04), (0x2A, 0x01),
        (0x2B, 0x04), (0x2C, 0x01), (0x30, 0x02), (0x01, 0x22), (0x03, 0x12), (0x04, 0x00), (0x05, 0x64), (0x0A, 0x08),
        (0x0B, 0x0A, 0x1A, 0x0B, 0x0D, 0x0D, 0x11, 0x10, 0x06, 0x08, 0x1F, 0x1D),
        (0x0C, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D),
        (0x0D, 0x16, 0x1B, 0x0B, 0x0D, 0x0D, 0x11, 0x10, 0x07, 0x09, 0x1E, 0x1C),
        (0x0E, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D),
        (0x0F, 0x16, 0x1B, 0x0D, 0x0B, 0x0D, 0x11, 0x10, 0x1C, 0x1E, 0x09, 0x07),
        (0x10, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D),
        (0x11, 0x0A, 0x1A, 0x0D, 0x0B, 0x0D, 0x11, 0x10, 0x1D, 0x1F, 0x08, 0x06),
        (0x12, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D, 0x0D),
        (0x14, 0x00, 0x00, 0x11, 0x11), (0x18, 0x99), (0x30, 0x06),
        (0x12, 0x36, 0x2C, 0x2E, 0x3C, 0x38, 0x35, 0x35, 0x32, 0x2E, 0x1D, 0x2B, 0x21, 0x16, 0x29,),
        (0x13, 0x36, 0x2C, 0x2E, 0x3C, 0x38, 0x35, 0x35, 0x32, 0x2E, 0x1D, 0x2B, 0x21, 0x16, 0x29,),
        (0x30, 0x0A), (0x02, 0x4F), (0x0B, 0x40), (0x12, 0x3E), (0x13, 0x78), (0x30, 0x0D), (0x0D, 0x04),
        (0x10, 0x0C), (0x11, 0x0C), (0x12, 0x0C), (0x13, 0x0C), (0x30, 0x00),
    ],
)
DriverChip(
    "JC4880P443C",
	width = 480
	height = 800
	hsync_back_porch = 42
	hsync_pulse_width = 12
	hsync_front_porch = 42
	vsync_back_porch = 8
	vsync_pulse_width = 2
	vsync_front_porch = 166
	pclk_frequency = "34 MHz"
	lane_bit_rate = "500 Mbps"
	swap_xy = False
	color_order = "RGB"
	reset_pin = 5
	initsequence = [
		# Command sequence from ST7701 initialization
		(0xFF, [0x77, 0x01, 0x00, 0x00, 0x13], 0),
		(0xEF, [0x08], 0),
		(0xFF, [0x77, 0x01, 0x00, 0x00, 0x10], 0),
		(0xC0, [0x63, 0x00], 0),
		(0xC1, [0x0D, 0x02], 0),
		(0xC2, [0x10, 0x08], 0),
		(0xCC, [0x10], 0),
		(0xB0, [0x80, 0x09, 0x53, 0x0C, 0xD0, 0x07, 0x0C, 0x09, 0x09, 0x28, 0x06, 0xD4, 0x13, 0x69, 0x2B, 0x71], 0),
		(0xB1, [0x80, 0x94, 0x5A, 0x10, 0xD3, 0x06, 0x0A, 0x08, 0x08, 0x25, 0x03, 0xD3, 0x12, 0x66, 0x6A, 0x0D], 0),
		(0xFF, [0x77, 0x01, 0x00, 0x00, 0x11], 0),
		(0xB0, [0x5D], 0),
		(0xB1, [0x58], 0),
		(0xB2, [0x87], 0),
		(0xB3, [0x80], 0),
		(0xB5, [0x4E], 0),
		(0xB7, [0x85], 0),
		(0xB8, [0x21], 0),
		(0xB9, [0x10, 0x1F], 0),
		(0xBB, [0x03], 0),
		(0xBC, [0x00], 0),
		(0xC1, [0x78], 0),
		(0xC2, [0x78], 0),
		(0xD0, [0x88], 0),
		(0xE0, [0x00, 0x3A, 0x02], 0),
		(0xE1, [0x04, 0xA0, 0x00, 0xA0, 0x05, 0xA0, 0x00, 0xA0, 0x00, 0x40, 0x40], 0),
		(0xE2, [0x30, 0x00, 0x40, 0x40, 0x32, 0xA0, 0x00, 0xA0, 0x00, 0xA0, 0x00, 0xA0, 0x00], 0),
		(0xE3, [0x00, 0x00, 0x33, 0x33], 0),
		(0xE4, [0x44, 0x44], 0),
		(0xE5, [0x09, 0x2E, 0xA0, 0xA0, 0x0B, 0x30, 0xA0, 0xA0, 0x05, 0x2A, 0xA0, 0xA0, 0x07, 0x2C, 0xA0, 0xA0], 0),
		(0xE6, [0x00, 0x00, 0x33, 0x33], 0),
		(0xE7, [0x44, 0x44], 0),
		(0xE8, [0x08, 0x2D, 0xA0, 0xA0, 0x0A, 0x2F, 0xA0, 0xA0, 0x04, 0x29, 0xA0, 0xA0, 0x06, 0x2B, 0xA0, 0xA0], 0),
		(0xEB, [0x00, 0x00, 0x4E, 0x4E, 0x00, 0x00, 0x00], 0),
		(0xEC, [0x08, 0x01], 0),
		(0xED, [0xB0, 0x2B, 0x98, 0xA4, 0x56, 0x7F, 0xFF, 0xFF, 0xFF, 0xFF, 0xF7, 0x65, 0x4A, 0x89, 0xB2, 0x0B], 0),
		(0xEF, [0x08, 0x08, 0x08, 0x45, 0x3F, 0x54], 0),
		(0xFF, [0x77, 0x01, 0x00, 0x00, 0x00], 0),
		(0x11, [0x00], 120),
		(0x29, [0x00], 20),
	],
)
