...
static const uint8_t LAN867X_MODEL_NUM=0x16;
static const uint32_t LAN867X_OUI=0xC0001C;
...
esp_err_t esp_eth_phy_lan867x_read_oui(phy_802_3_t *phy_802_3, uint32_t *oui) {
  ...
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)), err, TAG,
                    "read ID1 failed");
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)), err, TAG,
                    "read ID2 failed");
...
static esp_err_t lan867x_init(esp_eth_phy_t *phy) {
  ...
  ESP_GOTO_ON_ERROR(esp_eth_phy_802_3_read_manufac_info(phy_802_3, &model, NULL), err, TAG,
                    "read manufacturer's info failed");
  ESP_GOTO_ON_FALSE(oui == LAN867X_OUI, ESP_FAIL, err, TAG, "wrong chip OUI %lx (expected %lx)", oui, LAN867X_OUI);
...
static esp_err_t lan867x_advertise_pause_ability(esp_eth_phy_t *phy, uint32_t ability) { return ESP_ERR_NOT_SUPPORTED; }

static esp_err_t lan867x_set_speed(esp_eth_phy_t *phy, eth_speed_t speed) { return ESP_ERR_NOT_SUPPORTED; }

static esp_err_t lan867x_set_duplex(esp_eth_phy_t *phy, eth_duplex_t duplex) { return ESP_ERR_NOT_SUPPORTED; }
...
static esp_err_t lan867x_custom_ioctl(esp_eth_phy_t *phy, uint32_t cmd, void *data) {
  ...
  switch (cmd) {
    case LAN867X_ETH_CMD_S_PLCA:
      ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL0_REG_ADDR, &(plca_ctrl0.val)), err,
                        TAG, "read PLCA_CTRL0 failed");
      plca_ctrl0.en = (*(bool *) data == true);
      ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL0_REG_ADDR, plca_ctrl0.val), err, TAG,
                        "write PLCA_CTRL0 failed");
      break;
    case LAN867X_ETH_CMD_S_PLCA_NCNT:
      ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL1_REG_ADDR, &(plca_ctrl1.val)), err,
                        TAG, "read PLCA_CTRL1 failed");
      plca_ctrl1.ncnt = *(uint8_t *) data;
      ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL1_REG_ADDR, plca_ctrl1.val), err, TAG,
                        "write PLCA_CTRL1 failed");
      break;
...
static esp_err_t lan867x_loopback(esp_eth_phy_t *phy, bool enable) {
  ...
  lan867x_plca_ctrl0_reg_t plca_ctrl0;
  lan867x_plca_ctrl1_reg_t plca_ctrl1;
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_BMCR_REG_ADDR, &(bmcr.val)), err, TAG,
                    "read BMCR failed");
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL0_REG_ADDR, &(plca_ctrl0.val)), err, TAG,
                    "read PLCA_CTRL0 failed");
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL1_REG_ADDR, &(plca_ctrl1.val)), err, TAG,
                    "read PLCA_CTRL1 failed");
  if (enable) {
  ...
  ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, phy_802_3->addr, ETH_PHY_PLCA_CTRL1_REG_ADDR, plca_ctrl1.val), err, TAG,
                    "write PLCA_CTRL1 failed");
  ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, phy_802_3->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val), err, TAG,
                    "write BMCR failed");
  return ESP_OK;
err:
  return ret;
...
esp_eth_phy_t *esp_eth_phy_new_lan867x(const eth_phy_config_t *config) {
  ...
  ESP_GOTO_ON_FALSE(esp_eth_phy_802_3_obj_config_init(&lan867x->phy_802_3, config) == ESP_OK, NULL, err, TAG,
