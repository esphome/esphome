...
void AirConditioner::update() {
  if (0 == UpdateNextCycle) {
    ...
    // set mode flags
    TXData[12] = ((this->preset == ClimatePreset::CLIMATE_PRESET_BOOST) * MODE_FLAG_AUX_HEAT) |
    ...
    // set timer start
    // TODO: This is not tested. If you use it probably want to switch to State.TimerStart so timer doesn't get
    // ovedrridden TXData[9] =  CalculateSetTime(DesiredState.TimerStart); set timer stop TXData[10] =
...
void AirConditioner::ParseResponse() {
  ...
  if ((PREAMBLE == RXData[RX_BYTE_PREAMBLE]) && (PROLOGUE == RXData[RX_BYTE_PROLOGUE]) &&
    ...
    set_outdoor_temperature_sensor(this->outdoor_sensor_, CalculateTemp(RXData[RX_BYTE_T3_TEMP]));
    set_temperature_2a_sensor(this->temperature_2a_sensor_, CalculateTemp(RXData[RX_BYTE_T2A_TEMP]));
    set_temperature_2b_sensor(this->temperature_2b_sensor_, CalculateTemp(RXData[RX_BYTE_T2B_TEMP]));
    set_current_sensor(this->current_sensor_, RXData[RX_BYTE_CURRENT]);
