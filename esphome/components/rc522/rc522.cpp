...
void RC522::setup() {
  ...
  // OC pinMode(_resetPowerDownPin, INPUT);
  ...
  if (reset_pin_->digital_read() == LOW) {  // The MFRC522 chip is in power down mode.
    ...
    // OC if (digitalRead(_resetPowerDownPin) == LOW) {  // The MFRC522 chip is in power down mode.
    reset_pin_->pin_mode(OUTPUT);  // Now set the resetPowerDownPin as digital output.
    // OC pinMode(_resetPowerDownPin, OUTPUT);    // Now set the resetPowerDownPin as digital output.
    reset_pin_->digital_write(LOW);  // Make sure we have a clean LOW state.
    // OC digitalWrite(_resetPowerDownPin, LOW);  // Make sure we have a clean LOW state.
    delayMicroseconds(2);             // 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
    reset_pin_->digital_write(HIGH);  // Exit power down mode. This triggers a hard reset.
    // OC digitalWrite(_resetPowerDownPin, HIGH);  // Exit power down mode. This triggers a hard reset.
  ...
  pcd_write_register_(TxModeReg, 0x00);
  pcd_write_register_(RxModeReg, 0x00);
  // Reset ModWidthReg
  pcd_write_register_(ModWidthReg, 0x26);

  // When communicating with a PICC we need a timeout if something goes wrong.
  // f_timer = 13.56 MHz / (2*TPreScaler+1) where TPreScaler = [TPrescaler_Hi:TPrescaler_Lo].
  // TPrescaler_Hi are the four low bits in TModeReg. TPrescaler_Lo is TPrescalerReg.
  pcd_write_register_(TModeReg, 0x80);  // TAuto=1; timer starts automatically at the end of the transmission in all
  ...
  pcd_write_register_(TReloadRegH, 0x03);  // Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
  pcd_write_register_(TReloadRegL, 0xE8);

  pcd_write_register_(
      TxASKReg,
      0x40);  // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
  pcd_write_register_(ModeReg, 0x3D);  // Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC
...
void RC522::loop() {
  auto card_present = picc_is_new_card_present_();

  if (card_present) {
    picc_read_card_serial_();

    ESP_LOGI(TAG, "Read serial size: %d", uid_.size);
    if (uid_.size >= 4) {
      ESP_LOGI(TAG, "    %x %x %x %x", uid_.uidByte[0], uid_.uidByte[1], uid_.uidByte[2], uid_.uidByte[3]);
    }
...
void RC522::pcd_reset_() {
  ...
  pcd_write_register_(CommandReg, PCD_SoftReset);  // Issue the SoftReset command.
  ...
  do {
    ...
    delay(50);
  } while ((pcd_read_register_(CommandReg) & (1 << 4)) && (++count) < 3);
}
...
void RC522::pcd_antenna_on_() {
  byte value = pcd_read_register_(TxControlReg);
  if ((value & 0x03) != 0x03) {
    pcd_write_register_(TxControlReg, value | 0x03);
  }
}  // End PCD_AntennaOn()
 ...
 */
byte RC522::pcd_read_register_(PCD_Register reg  ///< The register to read from. One of the PCD_Register enums.
) {
  byte value;
  // OC SPI.beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0));  // Set the settings to work with SPI
  // bus digitalWrite(_chipSelectPin, LOW);                                         // Select slave
  enable();
  transfer_byte(0x80 | reg);
  // SPI.transfer(0x80 | reg);  // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.

  value = read_byte();
  // OC value = SPI.transfer(0);   // Read the value back. Send 0 to stop reading.
  disable();
  // OC digitalWrite(_chipSelectPin, HIGH);  // Release slave again
  // OC SPI.endTransaction();                // Stop using the SPI bus
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void RC522::pcd_read_register_(PCD_Register reg,  ///< The register to read from. One of the PCD_Register enums.
  ...
  if (rxAlign) {        // Only update bit positions rxAlign..7 in values[0]
    ...
    byte value = transfer_byte(address);
  ...
  ESP_LOGV(TAG, "read_register_array_(%x, %d, , %d) -> %s", reg, count, rxAlign, buf.c_str());

  disable();
}

void RC522::pcd_write_register_(PCD_Register reg,  ///< The register to write to. One of the PCD_Register enums.
                                byte value         ///< The value to write.
) {
  enable();
  // OC SPI.beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0));  // Set the settings to work with SPI
  // bus OC digitalWrite(_chipSelectPin, LOW);                                         // Select slave
  transfer_byte(reg);
  ...
  // ESP_LOGV(TAG, "write_register_(%x) -> %x", reg, value);
}
...
void RC522::pcd_write_register_(PCD_Register reg,  ///< The register to write to. One of the PCD_Register enums.
  ...
  disable();
  ESP_LOGV(TAG, "write_register_(%x, %d) -> %s", reg, count, buf.c_str());
}
...
RC522::StatusCode RC522::picc_request_a_(
  ...
  return picc_reqa_or_wupa_(PICC_CMD_REQA, bufferATQA, bufferSize);
}  // End PICC_RequestA()
...
RC522::StatusCode RC522::picc_reqa_or_wupa_(
  ...
  pcd_clear_register_bit_mask_(CollReg, 0x80);  // ValuesAfterColl=1 => Bits received after collision are cleared.
  validBits = 7;  // For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and only)
                  // byte. TxLastBits = BitFramingReg[2..0]
  status = pcd_transceive_data_(&command, 1, bufferATQA, bufferSize, &validBits);
  if (status != STATUS_OK) {
    ESP_LOGD(TAG, "picc_reqa_or_wupa_() -> %d", status);
    return status;
  }
  if (*bufferSize != 2 || validBits != 0) {  // ATQA must be exactly 16 bits.

    ESP_LOGD(TAG, "picc_reqa_or_wupa_() -> %d", STATUS_ERROR);
    return STATUS_ERROR;
  }

  ESP_LOGD(TAG, "picc_reqa_or_wupa_() -> %d", STATUS_OK);
  return STATUS_OK;
 ...
 */
void RC522::pcd_set_register_bit_mask_(PCD_Register reg,  ///< The register to update. One of the PCD_Register enums.
                                       byte mask          ///< The bits to set.
) {
  byte tmp;
  tmp = pcd_read_register_(reg);
  pcd_write_register_(reg, tmp | mask);  // set bit mask

  ESP_LOGD(TAG, "pcd_set_register_bit_mask_(%x, %x) -> %x => %x", reg, mask, tmp, tmp | mask);
}  // End PCD_SetRegisterBitMask()

/**
 * Clears the bits given in mask from register reg.
 */
void RC522::pcd_clear_register_bit_mask_(PCD_Register reg,  ///< The register to update. One of the PCD_Register enums.
  ...
  ESP_LOGD(TAG, "pcd_clear_register_bit_mask_(%x, %x) -> %x => %x", reg, mask, tmp, tmp & (~mask));
...
RC522::StatusCode RC522::pcd_transceive_data_(
  ...
  if (sendLen == 2)
    ESP_LOGD(TAG, "pcd_transceive_data_([ %d, %d], %d,  )", sendData[0], sendData[1], sendLen);
  else
    ESP_LOGD(TAG, "pcd_transceive_data_([], %d, ... )", sendLen);
  byte waitIRq = 0x30;  // RxIRq and IdleIRq
  auto ret = pcd_communicate_with_picc_(PCD_Transceive, waitIRq, sendData, sendLen, backData, backLen, validBits,
                                        rxAlign, checkCRC);

  if (ret == STATUS_OK && *backLen == 5)
    ESP_LOGD(TAG, "pcd_transceive_data_(..., %d,  ) -> %d [%x, %x, %x, %x, %x]", sendLen, ret, backData[0], backData[1],
             backData[2], backData[3], backData[4]);
  else
    ESP_LOGD(TAG, "pcd_transceive_data_(..., %d, ... ) -> %d", sendLen, ret);
  return ret;
}
...
RC522::StatusCode RC522::pcd_communicate_with_picc_(
  ...
  ESP_LOGD(TAG, "pcd_communicate_with_picc_(%d, %d,... %d)", command, waitIRq, checkCRC);

  // Prepare values for BitFramingReg
  byte txLastBits = validBits ? *validBits : 0;
  byte bitFraming = (rxAlign << 4) + txLastBits;  // RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]

  pcd_write_register_(CommandReg, PCD_Idle);            // Stop any active command.
  pcd_write_register_(ComIrqReg, 0x7F);                 // Clear all seven interrupt request bits
  pcd_write_register_(FIFOLevelReg, 0x80);              // FlushBuffer = 1, FIFO initialization
  pcd_write_register_(FIFODataReg, sendLen, sendData);  // Write sendData to the FIFO
  pcd_write_register_(BitFramingReg, bitFraming);       // Bit adjustments
  pcd_write_register_(CommandReg, command);             // Execute the command
  if (command == PCD_Transceive) {
    pcd_set_register_bit_mask_(BitFramingReg, 0x80);  // StartSend=1, transmission of data starts
  }
  ...
  byte errorRegValue = pcd_read_register_(
  ...
  if (backData && backLen) {
    byte n = pcd_read_register_(FIFOLevelReg);  // Number of bytes in the FIFO
    if (n > *backLen) {
      return STATUS_NO_ROOM;
    }
    *backLen = n;                                           // Number of bytes returned
    pcd_read_register_(FIFODataReg, n, backData, rxAlign);  // Get received data from FIFO
    _validBits =
        pcd_read_register_(ControlReg) & 0x07;  // RxLastBits[2:0] indicates the number of valid bits in the last
  ...
  if (backData && backLen && checkCRC) {
    ...
    RC522::StatusCode status = pcd_calculate_crc_(&backData[0], *backLen - 2, &controlBuffer[0]);
    if (status != STATUS_OK) {
      return status;
    }
...
RC522::StatusCode RC522::pcd_calculate_crc_(
  ...
  ESP_LOGD(TAG, "pcd_calculate_crc_(..., %d, ...)", length);
  pcd_write_register_(CommandReg, PCD_Idle);       // Stop any active command.
  pcd_write_register_(DivIrqReg, 0x04);            // Clear the CRCIRq interrupt request bit
  pcd_write_register_(FIFOLevelReg, 0x80);         // FlushBuffer = 1, FIFO initialization
  pcd_write_register_(FIFODataReg, length, data);  // Write data to the FIFO
  pcd_write_register_(CommandReg, PCD_CalcCRC);    // Start the calculation
  ...
  for (uint16_t i = 5000; i > 0; i--) {
    ...
    byte n = pcd_read_register_(DivIrqReg);
    if (n & 0x04) {                               // CRCIRq bit set - calculation done
      pcd_write_register_(CommandReg, PCD_Idle);  // Stop calculating CRC for new content in the FIFO.
      // Transfer the result from the registers to the result buffer
      result[0] = pcd_read_register_(CRCResultRegL);
      result[1] = pcd_read_register_(CRCResultRegH);

      ESP_LOGD(TAG, "pcd_calculate_crc_() STATUS_OK");
      return STATUS_OK;
    }
  }
...
bool RC522::picc_is_new_card_present_() {
  byte bufferATQA[2];
  ...
  RC522::StatusCode result = picc_request_a_(bufferATQA, &bufferSize);

  auto ret = (result == STATUS_OK || result == STATUS_COLLISION);
  ESP_LOGD(TAG, "picc_is_new_card_present_() -> %d", ret);
  return ret;
}
...
bool RC522::picc_read_card_serial_() {
  ...
  ESP_LOGD(TAG, "picc_select_(...) -> %d", result);
...
RC522::StatusCode RC522::picc_select_(
    ...
    byte validBits  ///< The number of known UID bits supplied in *uid. Normally 0. If set you must also supply
                    ///< uid->size.
) {
  bool uidComplete;
  bool selectDone;
  ...
  //		Byte 1: NVB					Number of Valid Bits (in complete command, not just the UID): High nibble: complete bytes,
  // Low nibble: Extra bits. 		Byte 2: UID-data or CT		See explanation below. CT means Cascade Tag. 		Byte 3:
  // UID-data Byte
  // 4: UID-data 		Byte 5: UID-data 		Byte 6: BCC					Block Check Character - XOR of bytes 2-5 		Byte 7: CRC_A
  // Byte 8: CRC_A The BCC and CRC_A are only transmitted if we know all the UID bits of the current Cascade Level.
  //
  ...
  ESP_LOGD(TAG, "picc_select_(&, %d)", validBits);
  ...
  while (!uidComplete) {
    ...
    if (bytesToCopy) {
      byte maxBytes =
          useCascadeTag ? 3 : 4;  // Max 4 bytes in each Cascade Level. Only 3 left if we use the Cascade Tag
    ...
    while (!selectDone) {
      ...
      result = pcd_transceive_data_(buffer, bufferUsed, responseBuffer, &responseLength, &txLastBits, rxAlign);
      if (result == STATUS_COLLISION) {  // More than one PICC in the field => collision.
        byte valueOfCollReg = pcd_read_register_(
    ...
    result = pcd_calculate_crc_(responseBuffer, 1, &buffer[2]);
    if (result != STATUS_OK) {
      return result;
    }
 ...
 */
MFRC522::MFRC522()
    : MFRC522(SS, UINT8_MAX) {  // SS is defined in pins_arduino.h, UINT8_MAX means there is no connection from Arduino
                                // to MFRC522's reset and power down input
}  // End constructor
...
void MFRC522::PCD_Init() {
  ...
  // f_timer = 13.56 MHz / (2*TPreScaler+1) where TPreScaler = [TPrescaler_Hi:TPrescaler_Lo].
  // TPrescaler_Hi are the four low bits in TModeReg. TPrescaler_Lo is TPrescalerReg.
  PCD_WriteRegister(TModeReg, 0x80);  // TAuto=1; timer starts automatically at the end of the transmission in all
                                      // communication modes at all speeds
  PCD_WriteRegister(
      TPrescalerReg,
      0xA9);  // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25μs.
  ...
  PCD_WriteRegister(TxASKReg,
                    0x40);  // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
  PCD_WriteRegister(ModeReg, 0x3D);  // Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC
                                     // command to 0x6363 (ISO 14443-3 part 6.2.4)
 ...
 */
void MFRC522::PCD_AntennaOff() { PCD_ClearRegisterBitMask(TxControlReg, 0x03); }  // End PCD_AntennaOff()
 ...
 */
byte MFRC522::PCD_GetAntennaGain() { return PCD_ReadRegister(RFCfgReg) & (0x07 << 4); }  // End PCD_GetAntennaGain()
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(
  ...
  if (backData && backLen) {
    ...
    *backLen = n;                                         // Number of bytes returned
    ...
    _validBits = PCD_ReadRegister(ControlReg) & 0x07;  // RxLastBits[2:0] indicates the number of valid bits in the last
                                                       // received byte. If this value is 000b, the whole byte is valid.
    if (validBits) {
      *validBits = _validBits;
    }
...
/**
 * Transmits a Wake-UP command, Type A. Invites PICCs in state IDLE and HALT to go to READY(*) and prepare for
 * anticollision or selection. 7 bit frame. Beware: When two PICCs are in the field at the same time I often get
...
MFRC522::StatusCode MFRC522::PICC_Select(
  ...
  while (!uidComplete) {
    ...
    while (!selectDone) {
      ...
      if (result == STATUS_COLLISION) {  // More than one PICC in the field => collision.
        byte valueOfCollReg =
            PCD_ReadRegister(CollReg);  // CollReg[7..0] bits are: ValuesAfterColl reserved CollPosNotValid CollPos[4:0]
        if (valueOfCollReg & 0x20) {    // CollPosNotValid
          return STATUS_COLLISION;      // Without a valid collision position we cannot continue
        }
        byte collisionPos = valueOfCollReg & 0x1F;  // Values 0-31, 0 means bit 32.
 ...
 * This command manages MIFARE authentication to enable a secure communication to any MIFARE Mini, MIFARE 1K and MIFARE
 * 4K card. The authentication is described in the MFRC522 datasheet section 10.3.1.9 and
 * http://www.nxp.com/documents/data_sheet/MF1S503x.pdf section 10.1. For use with MIFARE Classic PICCs. The PICC must
 * be selected - ie in state ACTIVE(*) - before calling this function. Remember to call PCD_StopCrypto1() after
 ...
 * Remember to call this function after communicating with an authenticated PICC - otherwise no new communications can
 * start.
 */
 ...
 * The MF0ICU1 responds to the READ command by sending 16 bytes starting from the page address defined by the command
 * argument. For example; if blockAddr is 03h then pages 03h, 04h, 05h, 06h are returned. A roll-back is implemented: If
 * blockAddr is 0Eh, then the contents of pages 0Eh, 0Fh, 00h and 01h are returned.
 *
...
MFRC522::StatusCode MFRC522::MIFARE_Read(
    byte blockAddr,   ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data
                      ///< from.
...
/**
 * MIFARE Decrement subtracts the delta from the value of the addressed block, and stores the result in a volatile
 * memory. For MIFARE Classic only. The sector containing the block must be authenticated before calling this function.
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(
  ...
  for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
    ...
    if (group != 3 && (g[group] == 1 || g[group] == 6)) {  // Not a sector trailer, a value block
      int32_t value =
          (int32_t(buffer[3]) << 24) | (int32_t(buffer[2]) << 16) | (int32_t(buffer[1]) << 8) | int32_t(buffer[0]);
...
/**
 * Calculates the bit pattern needed for the specified access bits. In the [C1 C2 C3] tuples C1 is MSB (=4) and C3 is
 * LSB (=1).
 */
void MFRC522::MIFARE_SetAccessBits(
    byte *accessBitBuffer,  ///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
    ...
    byte g3   ///< Access bits C1 C2 C3] for the sector trailer, block 3 (for sectors 0-31) or block 15 (for sectors
              ///< 32-39)
) {
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
  ...
  byte received;
  MFRC522::StatusCode status =
      PCD_TransceiveData(&cmd, (byte) 1, response, &received, &validBits, (byte) 0, false);  // 40
  if (status != STATUS_OK) {
    if (logErrors) {
