...
void RC522::setup() {
  ...
  pcd_write_register(TModeReg, 0x80);  // TAuto=1; timer starts automatically at the end of the transmission in all
  ...
  pcd_write_register(TxASKReg,
                     0x40);  // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
  pcd_write_register(ModeReg, 0x3D);  // Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC
 ...
 */
void RC522::pcd_antenna_on() {
  byte value = pcd_read_register(TxControlReg);
  if ((value & 0x03) != 0x03) {
 ...
 */
byte RC522::pcd_read_register(PCD_Register reg  ///< The register to read from. One of the PCD_Register enums.
) {
  byte value;
...
void RC522::pcd_write_register(PCD_Register reg,  ///< The register to write to. One of the PCD_Register enums.
