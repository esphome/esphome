 ...
 * and adapted to ESPHome abstractions by @glmnet
 *
...
class RC522 : public PollingComponent,
 ...
 protected:
  ...
  enum PCD_Command : byte {
    ...
    PCD_NoCmdChange = 0x07,  // no command change, can be used to modify the CommandReg register bits without affecting
   ...
   */
  std::vector<uint8_t> RC522_read_data_();

  /** Checks if the RC522 has set its ready status flag.
   *
   * Procedure goes as follows:
   * - Host sends command to RC522 "write data"
   * - Wait for readiness (until RC522 has processed command) by polling "read status"/is_ready_
...
class MFRC522 {
 public:
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  StatusCode PCD_TransceiveData(byte *sendData, byte sendLen, byte *backData, byte *backLen, byte *validBits = nullptr,
                                byte rxAlign = 0, bool checkCRC = false);
  StatusCode PCD_CommunicateWithPICC(byte command, byte waitIRq, byte *sendData, byte sendLen, byte *backData = nullptr,
                                     byte *backLen = nullptr, byte *validBits = nullptr, byte rxAlign = 0,
                                     bool checkCRC = false);
