...
class HBridgeLightOutput final : public Component, public light::LightOutput {
 public:
  ...
  void loop() override {
    // Only enabled when both channels are active — alternate the H-bridge direction
    // to multiplex cold and warm white. This runs at the normal loop interval (~60 Hz)
    // and must NOT use a high-frequency loop: flipping on every iteration rewrites the
    // output's PWM duty far faster than the carrier completes a cycle, so neither
    // channel renders and the output collapses onto one side (issue #17030).
  ...
  void write_state(light::LightState *state) override {
    ...
    if (new_pina != 0.0f && new_pinb != 0.0f) {
      // Both channels active — enable loop to alternate the H-bridge direction
      this->enable_loop();
    } else {
