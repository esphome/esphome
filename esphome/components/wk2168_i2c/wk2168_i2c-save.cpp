...
static char format_char(uint8_t v) { return v >= 10 ? 'A' + (v - 10) : '0' + v; }
std::string print_buffer(const uint8_t *data, size_t length) {
  if (length == 0)
    return "";
  std::string ret;
  ret.resize(2 * length);
  for (size_t i = 0; i < length; i++) {
    ret[2 * i] = format_char((data[i] & 0xF0) >> 4);
    ret[2 * i + 1] = format_char(data[i] & 0x0F);
  }
  return ret;
}
...
void WK2168RegI2C::read_fifo(uint8_t *data, size_t length) const {
  ...
  if (error == i2c::NO_ERROR) {
    this->comp_->status_clear_warning();
    ESP_LOGVV(TAG, "WK2168RegI2C::read_fifo() @%02X ch=%d I2C_code:%d len=%d buf=%s", comp_i2c->address_,
              this->channel_, (int) error, length, print_buffer(data, length).c_str());
  } else {  // error
    this->comp_->status_set_warning();
    ESP_LOGE(TAG, "WK2168RegI2C::read_fifo() @%02X ch=%d I2C_code:%d len=%d buf=%02X...", comp_i2c->address_,
...
void WK2168RegI2C::write_fifo(uint8_t *data, size_t length) {
  ...
  if (error == i2c::NO_ERROR) {
    this->comp_->status_clear_warning();
    ESP_LOGVV(TAG, "WK2168Reg::write_fifo() @%02X ch=%d I2C_code:%d len=%d buf=%s", comp_i2c->address_, this->channel_,
              (int) error, length, print_buffer(data, length).c_str());
  } else {  // error
    this->comp_->status_set_warning();
    ESP_LOGE(TAG, "WK2168Reg::write_fifo() @%02X ch=%d I2C_code:%d len=%d, buf=%02X...", comp_i2c->address_,
             this->channel_, (int) error, length, data[0]);
  }
}
