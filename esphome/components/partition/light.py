...
from esphome.const import CONF_FROM, CONF_ID, CONF_SEGMENTS, CONF_TO, CONF_OUTPUT_ID, CONF_REVERSED
...
def to_code(config):
    ...
    for conf in config[CONF_SEGMENTS]:
        ...
        segments.append(
            AddressableSegment(
                var, conf[CONF_FROM], conf[CONF_TO] - conf[CONF_FROM] + 1, conf[CONF_REVERSED]
            )
        )
