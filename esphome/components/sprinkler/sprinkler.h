...
class Sprinkler : public Component {
 public:
  ...
  /// returns the amount of time remaining for the active valve
 ...
 protected:
  ...
  uint32_t valve_run_duration_(uint8_t valve_number);

  /// returns valve_number's run duration adjusted by multiplier_. note that these times are stored in seconds
  uint32_t valve_run_duration_adjusted_(uint8_t valve_number);
  ...
  /// Iterate through valves in reverse order (descending) when true
  bool reverse_{false};
