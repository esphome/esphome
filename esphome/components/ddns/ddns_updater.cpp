...
void DDNSComponent::setup() {
  ...
  EasyDDNS.on_update(
      [&](const char* old_ip, const char* new_ip) { ESP_LOGD(TAG, "DDNS - IP Change Detected: %s", new_ip); });
}

void DDNSComponent::loop() { EasyDDNS.update(this->update_interval_, this->use_local_ip_); }
}  // namespace ddns
}  // namespace esphome
