...
uint16_t Mcp4461Component::read_wiper_level_(uint8_t wiper_idx, bool *ok) {
  ...
  if (!(this->read_16_(reg, &buf))) {
    ...
    ESP_LOGW(TAG, "Error fetching %swiper %u value",
             (wiper_idx > 3) ? LOG_STR_LITERAL("nonvolatile ") : LOG_STR_LITERAL(""), wiper_idx);
    return 0;
  }
  if (ok != nullptr) {
    *ok = true;
...
void Mcp4461Component::write_wiper_level_(uint8_t wiper, uint16_t value) {
  ...
  if (!(this->mcp4461_write_(this->get_wiper_address_(wiper), value, nonvolatile))) {
    this->error_code_ = MCP4461_STATUS_I2C_ERROR;
    this->status_set_warning();
    ESP_LOGW(TAG, "Error writing %swiper %u level %u",
             (wiper > 3) ? LOG_STR_LITERAL("nonvolatile ") : LOG_STR_LITERAL(""), wiper, value);
  }
}
