...
try:
    ...
    env = None  # Not in PlatformIO (e.g. build_codeowners.py); skip script.
...
if env is not None:
    ...
    for name in (
        ...
        if os.path.isfile(sdkconfig):
            os.remove(sdkconfig)
            print(
                "filter_esp_modbus: removed %s so it is regenerated from sdkconfig.defaults"
                % name
            )
    ...
    if lib_dir is None:
        ...
        if os.path.isfile(port_tcp_slave_c):
            with open(port_tcp_slave_c) as f:
                content = f.read()
