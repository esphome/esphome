...
void Tormatic::set_light_state(bool state) {
  ...
  this->set_timeout("light_retry_1", 100, [this, state]() { this->send_light_command_(state); });

  this->set_timeout("light_retry_2", 200, [this, state]() { this->send_light_command_(state); });
}
