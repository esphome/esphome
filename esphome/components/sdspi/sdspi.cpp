...
//------------------------------------------------------------------------------
//
//     Polling component intrface implementation
//
...
bool SDSPI::initialize() {
  ...
  if (is_init_) {
    ESP_LOGV(TAG, "IO driver already initilized.");  // On mount stage it try to initilize again.
    return true;
  }
  ...
  ESP_LOGD(TAG, "SDSPI Initialized.");
  return true;
}
...
bool SDSPI::wait(uint16_t ms) {  // waitReady
  Timeout timeout(ms);
...
uint8_t SDSPI::spi_app_command(uint8_t cmd, uint32_t arg) {
...
bool SDSPI::write_start(uint32_t sector) {
...
//------------------------------------------------------------------------------
bool SDSPI::write_data(uint8_t token, const uint8_t *src) {
#if USE_SD_CRC
...
bool SDSPI::write_data(const uint8_t *src) {
...
//
uint8_t SDSPI::write_sectors(const uint8_t *buffer, uint32_t sector, unsigned int count) {
  if (!this->write_start(sector)) {
    this->spi_stop();
    return RC_ERROR;
  }
  for (size_t i = 0; i < count; i++, buffer += this->sector_size()) {
    if (!this->write_data(buffer)) {
      this->spi_stop();
      return RC_ERROR;
    }
  }
  return this->write_stop() ? RC_OK : RC_ERROR;
}

//------------------------------------------------------------------------------

bool SDSPI::write_sector(uint32_t sector, const uint8_t *src) {
  // use address if not SDHC card
  if (this->get_type() != SD_CARD_TYPE_SDHC) {
    sector <<= 9;
  }
...
bool SDSPI::read_start(uint32_t sector) {
  if (this->get_type() != SD_CARD_TYPE_SDHC) {
    sector <<= 9;
  }
...
bool SDSPI::read_sector(uint32_t sector, uint8_t *dst) {
  // use address if not SDHC card
  if (this->get_type() != SD_CARD_TYPE_SDHC) {
    sector <<= 9;
  }
...
//   Implements Storage interface
//
uint8_t SDSPI::read_sectors(uint8_t *buffer, uint32_t sector, unsigned int count) {
  // bool SDSPI::read_sectors(uint32_t sector, uint8_t *dst, size_t ns) {
  if (!this->read_start(sector)) {
    return RC_ERROR;
  }
  for (size_t i = 0; i < count; i++, buffer += this->sector_size()) {
    if (!this->read_data(buffer, this->sector_size())) {
      this->spi_stop();
      return RC_ERROR;
    }
  }
  return this->read_stop() ? RC_OK : RC_ERROR;
}

//------------------------------------------------------------------------------

bool SDSPI::read_status(void *status) {
  ...
  response_ = this->read_data((uint8_t *) status, 64);
  this->spi_stop();
  if (!response_) {
    ret = false;
  }
...
bool SDSPI::read_register(uint8_t cmd, void *buf) {
  ...
  if (cmd == CMD58) {
    response_ = this->read_data(dst, 4);
  } else {
...
uint32_t SDSPI::sector_count() {
  csd_t csd;

  uint32_t s_count = read_register(CMD9, &csd) ? sd_card_capacity(&csd) : 0;
...
bool SDSPI::erase(uint32_t first_sector, uint32_t last_sector) {
  csd_t csd;
  if (!this->read_register(CMD9, &csd)) {
    this->spi_stop();
    return false;
  }
...
bool SDSPI::erase_sector_enable() {
  csd_t csd;
  return read_register(CMD9, &csd) ? csd.v1.erase_blk_en : false;
}
...
// CMD_MMC_GET_TYPE 10    // Get card type
// CMD_MMC_GET_CSD 11     // Get CSD
// CMD_MMC_GET_CID 12     // Get CID
// CMD_MMC_GET_OCR 13     // Get OCR
// CMD_MMC_GET_SDSTAT 14  // Get SD status
//
uint8_t SDSPI::ioctl(uint8_t cmd, void *buff) {
  ESP_LOGV(TAG, "ioctl cmd=%d", cmd);
  ...
  switch (cmd) {
    ...
    case CMD_GET_SECTOR_SIZE: {
      *((uint16_t *) buff) = this->sector_size();
      // *((WORD *) buff) = 512;
      return RC_OK;
    }
    ...
    case CMD_MMC_GET_CSD: {
      if (read_register(CMD9, buff)) {  // 16bytes
        return RC_OK;
      }
      return RC_ERROR;
    }
    case CMD_MMC_GET_CID: {
      if (read_register(CMD10, buff)) {  // 16bytes
        return RC_OK;
      }
      return RC_ERROR;
    }
    case CMD_MMC_GET_OCR: {
      if (read_register(CMD58, buff)) {  //  4 bytes
        return RC_OK;
      }
      return RC_ERROR;
    }
    case CMD_MMC_GET_SDSTAT: {
      read_status((uint8_t *) buff);  // 64bytes
      return RC_OK;
    }
  }
  return RC_PARERR;
