...
class ESPHomeHardwareInterface : public SharpAcHardwareInterface {
 public:
  ...
  size_t read_array(uint8_t *data, size_t len) override { return uart_device_->read_array(data, len) ? len : 0; }

  size_t available() override { return uart_device_->available(); }

  void write_array(const uint8_t *data, size_t len) override { uart_device_->write_array(data, len); }

  uint8_t peek() override { return uart_device_->peek(); }

  uint8_t read() override { return uart_device_->read(); }

  unsigned long get_millis() override { return millis(); }
...
class SharpAc : public climate::Climate, public uart::UARTDevice, public Component {
 public:
  ...
  void setIonSwitch(switch_::Switch *ionSwitch) { this->ionSwitch = ionSwitch; };
  void setVaneVerticalSelect(VaneSelectVertical *vane) { this->vaneVertical = vane; };
  void setVaneHorizontalSelect(VaneSelectHorizontal *vane) { this->vaneHorizontal = vane; };
  void setConnectionStatusSensor(text_sensor::TextSensor *sensor) { this->connectionStatusSensor = sensor; };

  void setReconnectButton(button::Button *button) { this->reconnectButton = button; };
