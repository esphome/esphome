...
void MQTTEventComponent::setup() {
  this->event_->add_on_event_fired_callback([this]() { this->publish_state(); });
}
...
bool MQTTEventComponent::publish_state(JsonObject &root, const mqtt::SendStateConfig &config) {
  root[MQTT_EVENT_TYPE] = this->event_->get_active_event_type();
  return true;
}
