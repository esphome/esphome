...
// Returns the value byte count for a fixed-width BTHome type, or 0 for variable-length types.
// constexpr so it can be used in static_assert / static constexpr declarations.
constexpr size_t get_bthome_value_length(BTHomeObjectType obj_type) {
  switch (obj_type) {
    // 1 Byte (uint8 / sint8)
