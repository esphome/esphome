...
void BTHomeServerBase::send_frame_() {
  ...
  char hex_buf[format_hex_size(BTHOME_MAX_PAYLOAD)];
  ESP_LOGV("bthome.server", "Advertising frame with payload size %zu (total %zu): content: %s", payload_size, total,
           format_hex_to(hex_buf, payload, payload_size));
