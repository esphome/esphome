...
// 6.17 23 (0x17) Read/Write Multiple registers: the request PDU is 10 header bytes + 2 per written register,
// so the write count is capped tighter than a plain write (6.12) to keep the request PDU within MAX_PDU_SIZE.
static constexpr uint16_t MAX_NUM_OF_REGISTERS_TO_WRITE_RW = (MAX_PDU_SIZE - 10) / 2;  // 0x79 (121)
