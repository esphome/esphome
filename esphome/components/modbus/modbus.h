...
struct ModbusFrame {
  // Frame held in a small-buffer-optimized buffer. Typical frames -- reads and single-register/coil
  // writes -- are exactly 8 bytes (address + 5-byte PDU + 2-byte CRC) and fit inline with no heap
  // allocation; only larger multi-register or custom frames spill to a single heap allocation. This
  // keeps the common, high-frequency tx traffic off the heap entirely, avoiding per-frame alloc/free
  // churn. The buffer tracks its own length, so no separate size field is needed.
  SmallInlineBuffer<8> data;  // Modbus RTU max is 256 bytes
