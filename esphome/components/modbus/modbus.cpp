...
void ModbusServerHub::process_modbus_client_frame_(uint8_t address, uint8_t function_code, const uint8_t *data) {
  ...
  switch (static_cast<FunctionCode>(function_code)) {
    ...
    case FunctionCode::READ_WRITE_MULTIPLE_REGISTERS: {
      ...
      if (!this->check_address_range_(address, function_code, read_start_address, number_of_registers) ||
          !this->check_address_range_(address, function_code, write_start_address, number_of_write_registers)) {
        return;
      }
