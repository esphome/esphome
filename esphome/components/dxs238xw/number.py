...
from . import dxs238xw_ns, CONF_DXS238XW_ID, SmIdEntity, SmLimitValue, DXS238XW_COMPONENT_SCHEMA
...
TYPES = {
    MAX_CURRENT_LIMIT: (number.NUMBER_SCHEMA.extend(
        {
        ...
        SmLimitValue.MIN_CURRENT, SmLimitValue.MAX_CURRENT, SmIdEntity.NUMBER_MAX_CURRENT_LIMIT),
    MAX_VOLTAGE_LIMIT: (number.NUMBER_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(dxs238xwNumber),
        ...
        SmLimitValue.MIN_VOLTAGE, SmLimitValue.MAX_VOLTAGE, SmIdEntity.NUMBER_MAX_VOLTAGE_LIMIT),
    MIN_VOLTAGE_LIMIT: (number.NUMBER_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(dxs238xwNumber),
            cv.Optional(CONF_ICON, default="mdi:sine-wave"): cv.icon,
            cv.Optional(CONF_UNIT_OF_MEASUREMENT, default=UNIT_VOLT): cv.string_strict,
        ...
        SmLimitValue.MIN_VOLTAGE, SmLimitValue.MAX_VOLTAGE, SmIdEntity.NUMBER_MIN_VOLTAGE_LIMIT),
    ENERGY_PURCHASE_VALUE: (number.NUMBER_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(dxs238xwNumber),
            cv.Optional(CONF_ICON, default="mdi:lightning-bolt"): cv.icon,
            cv.Optional(CONF_UNIT_OF_MEASUREMENT, default=UNIT_KILOWATT_HOURS): cv.string_strict,
            cv.Optional(CONF_ENTITY_CATEGORY, default=ENTITY_CATEGORY_CONFIG): cv.entity_category,
            cv.Optional(CONF_STEP, default=1): cv.uint32_t,
        }),
        SmLimitValue.MIN_ENERGY_PURCHASE_VALUE, SmLimitValue.MAX_ENERGY_PURCHASE_VALUE, SmIdEntity.NUMBER_ENERGY_PURCHASE_VALUE),
    ENERGY_PURCHASE_ALARM: (number.NUMBER_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(dxs238xwNumber),
            cv.Optional(CONF_ICON, default="mdi:lightning-bolt"): cv.icon,
            cv.Optional(CONF_UNIT_OF_MEASUREMENT, default=UNIT_KILOWATT_HOURS): cv.string_strict,
        ...
        SmLimitValue.MIN_ENERGY_PURCHASE_ALARM, SmLimitValue.MAX_ENERGY_PURCHASE_ALARM, SmIdEntity.NUMBER_ENERGY_PURCHASE_ALARM),
    DELAY_VALUE_SET: (number.NUMBER_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(dxs238xwNumber),
        ...
        SmLimitValue.MIN_DELAY_SET, SmLimitValue.MAX_DELAY_SET, SmIdEntity.NUMBER_DELAY_VALUE_SET),
}
...
async def to_code(config):
    ...
    for type, (_, min, max, number_id) in TYPES.items():
        if type in config:
            conf = config[type]
            var = await number.new_number(conf, min_value=min, max_value=max, step=conf[CONF_STEP])
