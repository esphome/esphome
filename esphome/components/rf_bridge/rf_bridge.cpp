...
void RFBridgeComponent::rfbDecode_() {
  ...
  switch(action) {
    ...
    case RF_CODE_RFIN:
      ...
      ESP_LOGD(TAG, "Received RFBridge Code: sync=0x%04X low=0x%04X high=0x%04X code=0x%06X", data.sync, data.low, data.high, data.code);
...
void RFBridgeComponent::send_code(RFBridgeData data) {
  ESP_LOGD(TAG, "Sending code: sync=0x%04X low=0x%04X high=0x%04X code=0x%06X", data.sync, data.low, data.high, data.code);
  this->write(RF_CODE_START);
