...
from .const import CONF_LIBRETUYA_ID, LTComponent
...
CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_LIBRETUYA_ID): cv.use_id(LTComponent),
...
async def to_code(config):
    lt_component = await cg.get_variable(config[CONF_LIBRETUYA_ID])
