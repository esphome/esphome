...
std::string WebServer::cover_json(cover::Cover *obj, JsonDetail start_config) {
  return json::build_json([obj, start_config](JsonObject root) {
    set_json_state_value(root, obj, "cover-" + obj->get_object_id(), obj->is_fully_closed() ? "CLOSED" : "OPEN",
                         int(obj->position*100), start_config);
...
std::string WebServer::climate_json(climate::Climate *obj, JsonDetail start_config) {
  std::string secondjson = json::build_json([obj](JsonObject root) {
  ...
  std::string firstjson = json::build_json([obj, start_config, secondjson](JsonObject root) {
    ...
    root["savant"] = secondjson;
  });
  std::size_t count{};
  std::string what = "\\";
  std::string with = "";
  for (std::string::size_type pos{};
        firstjson.npos != (pos = firstjson.find(what.data(), pos, what.length()));
        pos += with.length(), ++count) {
      firstjson.replace(pos, what.length(), with.data(), with.length());
  }
  count = 0;
  what = "\"{";
  with = "{";
