...
void WebServer::add_origin_header(AsyncWebServerResponse *response) {
  if (js_enable_cors_)
    response->addHeader("Access-Control-Allow-Origin", js_cors_header_);
}

void WebServer::send_response(AsyncWebServerRequest *request,int code, const String& content_type, const String& content){
  AsyncWebServerResponse *response = request->beginResponse(code,content_type, content);
  add_origin_header(response);
  request->send(response);
}
void WebServer::send_response(AsyncWebServerRequest *request,int code){
...
void WebServer::handle_sensor_request(AsyncWebServerRequest *request, UrlMatch match) {
  for (sensor::Sensor *obj : App.get_sensors()) {
    ...
    send_response(request,200, "text/json", data.c_str());
    return;
  }
  send_response(request,404);
}
