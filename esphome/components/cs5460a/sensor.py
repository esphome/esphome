...
from esphome.const import CONF_CURRENT, CONF_ID, CONF_POWER, CONF_VOLTAGE, \
    UNIT_VOLT, UNIT_AMPERE, UNIT_WATT, ICON_EMPTY, DEVICE_CLASS_POWER, \
...
def validate_config(config):
    current_gain = abs(config[CONF_CURRENT_GAIN]) * (1.0 if config[CONF_PGA_GAIN] == '10X' else 5.0)
    ...
    if pulse_energy < min_energy or pulse_energy > max_energy:
        raise cv.Invalid('For given current&voltage gains, the pulse energy must be between '
...
CONFIG_SCHEMA = cv.All(cv.Schema({
    ...
    cv.Optional(CONF_PGA_GAIN, default='10X'): cv.enum(PGA_GAIN_OPTIONS, upper=True),
    ...
    cv.Optional(CONF_VOLTAGE): sensor.sensor_schema(UNIT_VOLT, ICON_EMPTY, 0, DEVICE_CLASS_VOLTAGE),
    cv.Optional(CONF_CURRENT): sensor.sensor_schema(UNIT_AMPERE, ICON_EMPTY, 1,
...
@automation.register_action('sensor.cs5460a.restart', CS5460ARestartAction,
                            maybe_simple_id({
