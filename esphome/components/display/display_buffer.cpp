...
bool Glyph::get_pixel(int x, int y) const {
  ...
  return progmem_read_8(this->glyph_data_->data + (pos / 8u)) & (0x80 >> (pos % 8u));
}
...
bool Image::get_pixel(int x, int y) const {
  ...
  return progmem_read_8(this->data_start_ + (pos / 8u)) & (0x80 >> (pos % 8u));
}
Color Image::get_color_pixel(int x, int y) const {
  if (x < 0 || x >= this->width_ || y < 0 || y >= this->height_)
    return Color::BLACK;
  const uint32_t pos = (x + y * this->width_) * 3;
  const uint32_t color32 = (progmem_read_8(this->data_start_ + pos + 2) << 0) |
                           (progmem_read_8(this->data_start_ + pos + 1) << 8) |
                           (progmem_read_8(this->data_start_ + pos + 0) << 16);
...
Color Image::get_grayscale_pixel(int x, int y) const {
  ...
  const uint8_t gray = progmem_read_8(this->data_start_ + pos);
