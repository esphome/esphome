...
class ABBWelcomeData {
 public:
  ...
  uint8_t size() const { return std::min(static_cast<uint8_t>(10 + (this->data_[2] & 0x7f)), static_cast<uint8_t>(this->data_.size())); }
  bool is_valid() const { return this->data_[0] == 0x55 && this->data_[1] == 0xff && ((this->data_[2] & 0x7f) <= MAX_DATA_LENGTH) && (this->data_[this->size() - 1] == this->calc_cs_()); }
  void set_retransmission(bool retransmission) { if (retransmission) this->data_[2] |= 0x80; else this->data_[2] &= 0x7f; }
  ...
  void set_destination_address(uint16_t address) { this->data_[4] = address >> 8; this->data_[5] = address & 0xff; }
  uint16_t get_destination_address() const { return (this->data_[4] << 8) + this->data_[5]; }
  void set_source_address(uint16_t address) { this->data_[6] = address >> 8; this->data_[7] = address & 0xff; }
  ...
  std::string to_string(uint8_t max_print_bytes = 255) const {
    std::string info;
    if (this->is_valid()) {
      info = str_sprintf("[%04X %s %04X] Type: %02X", this->get_source_address(), this->get_retransmission() ? "»": ">", this->get_destination_address(), this->get_message_type());
  ...
  bool operator==(const ABBWelcomeData &rhs) const {
    ...
    return (this->auto_message_id || rhs.auto_message_id) && this->is_valid() && rhs.is_valid() &&
        ...
        (this->get_destination_address() == rhs.get_destination_address()) &&
        (this->get_data() == rhs.get_data());
  }
...
class ABBWelcomeBinarySensor : public RemoteReceiverBinarySensorBase {
 public:
  ...
  void set_destination_address(const uint16_t destination_address) { this->data_.set_destination_address(destination_address); }
...
template<typename... Ts> class ABBWelcomeAction : public RemoteTransmitterActionBase<Ts...> {
  ...
  void set_data_template(std::function<std::vector<uint8_t>(Ts...)> func) { this->data_func_ = func; has_data_func_ = true; }
