...
void JVCProtocol::encode(RemoteTransmitData *dst, const JVCData &data) {
  ...
  dst->reserve(2 + 2 * NBITS);
  dst->item(HEADER_HIGH_US, HEADER_LOW_US);
  encode_data_msb<uint32_t, NBITS, BIT_MARK_US, BIT_ONE_SPACE_US, BIT_ZERO_SPACE_US>(dst, data.data);
...
optional<JVCData> JVCProtocol::decode(RemoteReceiveData src) {
  ...
  if (!src.expect_item(HEADER_HIGH_US, HEADER_LOW_US))
    return {};
  if (!decode_data_msb<uint32_t, NBITS, BIT_MARK_US, BIT_ONE_SPACE_US, BIT_ZERO_SPACE_US>(src, out.data))
    return {};
  return out;
}
