...
void CanalSatBaseProtocol::encode(RemoteTransmitData *dst, const CanalSatData &data) {
  ...
  uint32_t raw{static_cast<uint32_t>((1 << 23) | (data.device << 16) | (data.address << 10) | (0 << 9) | (data.command << 1))};
  bool wasHigh{true};
...
void CanalSatBaseProtocol::dump(const CanalSatData &data) {
  if(this->tag == CANALSATLD_TAG) {
    ESP_LOGD(this->tag, "Received CanalSatLD: device=0x%02X, address=0x%02X, command=0x%02X, repeat=0x%X", data.device, data.address, data.command, data.repeat);
  } else {
    ESP_LOGD(this->tag, "Received CanalSat: device=0x%02X, address=0x%02X, command=0x%02X, repeat=0x%X", data.device, data.address, data.command, data.repeat);
  }
}
