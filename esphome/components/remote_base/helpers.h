...
template<uint32_t mark_us, uint32_t space_one_us, uint32_t space_zero_us>
class space {
 public:
  ...
  class msb {
   public:
    ...
    template<typename T>
    static size_t decode(RemoteReceiveData &src, T &dst, const size_t nbits = sizeof(T) * 8) {
      static_assert(std::is_integral<T>::value, "T must be an integer.");
      size_t bit = 0;
      for (; bit != nbits; bit++) {
...
template<uint32_t space_us, uint32_t mark_one_us, uint32_t mark_zero_us>
class mark {
 public:
  class msb {
   public:
