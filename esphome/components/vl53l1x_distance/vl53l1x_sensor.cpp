...
void VL53L1XSensor::update() {
    ...
    if (VL53L1X_check_for_data_ready() == 0) {
		distance = VL53L1X_get_distance(); //Get the result of the measurement from the sensor
...
//   VL53L1X_set_ROI(ROI_height, ROI_width, center[Zone]);  // first value: height of the zone, second value: width of the zone
//   delayMicroseconds(50);
//   VL53L1X_set_timing_budget_in_ms(50); // Could this be set into setup as well?
//   VL53L1X_start_ranging(); //Write configuration bytes to initiate measurement
//   distance = VL53L1X_getDistance(); //Get the result of the measurement from the sensor
//   VL53L1X_stop_ranging();
...
bool VL53L1XSensor::VL53L1X_check_for_data_ready()
{
	uint8_t isdataReady;
	...
	//to
	// 1 = VL53L1X_GetInterruptPolarity(&IntPol);
	// 2 = VL53L1_RdByte(Device, GPIO__TIO_HV_STATUS, &Temp); 
	// if (1 and 2 ) instead
	{
		if ((Temp & 1) == IntPol)
			isdataReady = 1;
...
int8_t VL53L1XSensor::VL53L1X_set_timing_budget_in_ms(uint16_t TimingBudgetInMs)
{
	...
	{ /* Short DistanceMode */
		...
		case 15: /* only available in short distance mode */
			this->write_byte_16(RANGE_CONFIG__TIMEOUT_MACROP_A_HI,
						  0x01D);
			this->write_byte_16(RANGE_CONFIG__TIMEOUT_MACROP_B_HI,
						  0x0027);
			break;
		case 20:
			this->write_byte_16(RANGE_CONFIG__TIMEOUT_MACROP_A_HI,
	...
	else
	{
		...
		case 33:
			...
			break;
		case 50:
			this->write_byte_16(ANGE_CONFIG__TIMEOUT_MACROP_A_HI,
						  0x00AD);
			this->write_byte_16(RANGE_CONFIG__TIMEOUT_MACROP_B_HI,
						  0x00C6);
			break;
...
int8_t VL53L1XSensor::VL53L1X_get_timing_budget_in_ms()
{
	...
	status = this->read_byte(Device, RANGE_CONFIG__TIMEOUT_MACROP_A_HI, &Temp);
	switch (Temp)
	{
	case 0x001D:
		TimingBudgetInMs = 15;
...
int8_t VL53L1XSensor::VL53L1X_set_distance_mode_long()
{
	VL53L1X_set_distance_mode(SHORT);
}

int8_t VL53L1XSensor::VL53L1X_set_distance_mode_short()
{
	VL53L1X_set_distance_mode(LONG);
}

bool VL53L1XSensor::VL53L1X_set_distance_mode(DistanceMode mode)
{
	uint16_t TB;
	VL53L1X_ERROR status = 0;
	...
	status = VL53L1X_set_timing_budget(TB);
	return status;
}
// ROI
void VL53L1XSensor::VL53L1X_set_ROI(uint8_t X, uint8_t Y, uint8_t opticalCenter)
{
	...
	status = this->writebyte(ROI_CONFIG__USER_ROI_CENTRE_SPAD, opticalCenter);
	status = this->writebyte(ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE,
						   (Y - 1) << 4 | (X - 1));
	return status;
}
