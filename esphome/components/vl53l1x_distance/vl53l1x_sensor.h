...
class VL53L1XSensor : public sensor::Sensor, public PollingComponent, public i2c::I2CDevice {
 public:
  ...
  bool VL53L1X_check_for_data_ready(uint8_t isdataReady);         // Checks the to see if data is ready
  int8_t VL53L1X_set_timing_budget_in_ms(uint16_t timingBudget);  // Set the timing budget for a measurement
  uint16_t VL53L1X_get_timing_budget_in_ms();                     // Get the timing budget for a measurement
  int8_t VL53L1X_set_distance_mode(DistanceMode mode);
  int8_t VL53L1X_set_distance_mode_long();   // Set to 4M range
  int8_t VL53L1X_set_distance_mode_short();  // Set to 1.3M range

  uint16_t VL53L1X_get_distance();  // Returns distance

  // ROI
  uint8_t VL53L1X_set_ROI(
      uint8_t X, uint8_t Y,
