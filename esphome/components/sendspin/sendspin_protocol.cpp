...
static bool process_server_player_command_object(const JsonObject player_object,
  ...
  // Only volume and mute commands are valid for server->client
  if (!command.has_value() ||
      (command.value() != SendspinCommandType::VOLUME && command.value() != SendspinCommandType::MUTE)) {
    ESP_LOGE(TAG, "Invalid server command type: %s", command_str.c_str());
    return false;
  }
...
std::string format_client_hello_message(const ClientHelloMessage *msg) {
  ...
  return json::build_json([msg](JsonObject root) {
    ...
    if (msg->player_v1_support.has_value()) {
      ...
      root["payload"]["player_support"]["buffer_capacity"] = msg->player_v1_support.value().buffer_capacity;
      JsonArray commands_list = root["payload"]["player_support"]["supported_commands"].to<JsonArray>();
...
std::string format_client_command_message(SendspinCommandType command, std::optional<uint8_t> volume,
  ...
  return json::build_json([command, volume, mute](JsonObject root) {
    ...
    if (command == SendspinCommandType::VOLUME && volume.has_value()) {
      root["payload"]["controller"]["volume"] = volume.value();
    }
    if (command == SendspinCommandType::MUTE && mute.has_value()) {
