...
class NciMessage {
 public:
  ...
  uint8_t get_payload_size(const bool recompute = false);
  uint8_t get_simple_status_response() const;
  uint8_t get_message_byte(const uint8_t offset) const;
  std::vector<uint8_t> &get_message();

  bool has_payload() const;
  bool message_type_is(const uint8_t message_type) const;
  bool message_length_is(const uint8_t message_length, const bool recompute = false);
  bool gid_is(const uint8_t gid) const;
  bool oid_is(const uint8_t oid) const;
  bool simple_status_response_is(const uint8_t response) const;
