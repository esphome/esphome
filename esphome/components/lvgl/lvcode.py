...
def _is_setup_context() -> bool:
    """
    Check if we're currently in LvContext (LVGL setup/initialization context).

    LvContext is used during widget setup and has LVGL_COMP_ARG by default,
    which includes lv_component parameter that shouldn't be passed to widget
    property lambdas.

    Returns:
        True if in LvContext (setup context), False otherwise
    """
    return isinstance(CodeContext.code_context, LvContext)


def get_automation_parameters(args: Iterable) -> Iterable:
    """
    Get automation parameters for lambdas, filtering out setup context parameters.

    If args not provided, retrieves them from the current LambdaContext.
    Filters out LvContext args to ensure lambdas in widget properties don't
    receive lv_component parameter.

    Args:
        args: Automation parameters tuple/list, or empty tuple to auto-retrieve

    Returns:
        Filtered automation parameters, or empty tuple if in setup context
    """
    if not args:
        if _is_setup_context():
            return ()
        return CodeContext.code_context.parameters
