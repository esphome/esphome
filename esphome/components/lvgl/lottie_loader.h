...
inline void lottie_load_task(void *param) {
  ...
  if (!ctx->data_loaded) {
    ...
    if (anim != nullptr) {
      ...
      LV_LOG_TRACE("Anim: frames %d..%d, duration %u ms", (int) ctx->start_frame, (int) ctx->end_frame,
  ...
  LV_LOG_TRACE("Render loop: %u ms/frame, loop=%d", (unsigned) frame_delay_ms, (int) ctx->loop);
...
inline void lottie_free_resources(LottieContext *ctx) {
  ...
  LV_LOG_TRACE("Lottie freed (%ux%u = %u KB %s buf + 64 KB stack)", (unsigned) ctx->width, (unsigned) ctx->height,
...
inline bool lottie_launch(LottieContext *ctx) {
  ...
  LV_LOG_TRACE("Lottie launched (runtime_hidden=%d, %s: %u KB buf + 64 KB stack, free PSRAM: %u KB, free SRAM: %u KB, "
               "largest SRAM: %u KB)",
               (int) ctx->runtime_hidden, ctx->pixel_buffer_internal ? "SRAM" : "PSRAM", (unsigned) (buf_bytes / 1024),
...
inline bool lottie_init(lv_obj_t *obj, const void *data, size_t data_size, const char *file_path, uint32_t width,
  ...
  LV_LOG_TRACE("Lottie registered (%ux%u), waiting for page load to allocate", (unsigned) width, (unsigned) height);
  return true;
}
