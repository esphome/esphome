...
``esphome.espidf.clang_tidy``: generate a Zephyr application whose sources are
the real ESPHome files being analyzed, and run a configure-only west build
with the native sdk-nrf toolchain. Each source gets its own CMake-generated
compile command -- ``script/clang-tidy`` feeds the result straight to
CodeChecker, which adapts GCC's flags/headers for Clang-based analysis.
...
# esphome/components/nrf52/clang_tidy.py -> repo root
...
def _tidy_cmakelists(library_include_dirs: str, source_files: list[str]) -> str:
    ...
    #
    # Real target_sources() entries (not just main.cpp) get CMake to emit an
    # accurate compile_commands.json entry per file, not one reused command.
    sources = "\n".join(f'  "{f}"' for f in ["main.cpp", *source_files])
    return f"""\
...
def generate_compile_commands(
    ...
    ``source_files`` are absolute paths to the ESPHome sources to analyze;
    each gets its own real compile command (see ``_tidy_cmakelists``).
    ...
    (source_dir / "CMakeLists.txt").write_text(
        _tidy_cmakelists(library_include_dirs, source_files), encoding="utf-8"
    )
