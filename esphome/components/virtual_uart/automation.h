...
template<typename... Ts> class VirtualUARTInjectRXAction : public Action<Ts...>, public Parented<VirtualUARTComponent> {
 public:
  ...
  void play(const Ts &...x) override {
    if (this->len_ >= 0) {
      ...
      this->parent_->inject_rx(this->code_.data, static_cast<size_t>(this->len_));
    } else {
      // Template mode: call function
      auto val = this->code_.func(x...);
      this->parent_->inject_rx(val);
    }
  }
...
class VirtualUARTTXTrigger : public Trigger<std::vector<uint8_t>> {
 public:
  explicit VirtualUARTTXTrigger(VirtualUARTComponent *parent) {
