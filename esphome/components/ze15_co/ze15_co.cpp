...
void ZE15COComponent::update() {
    ...
    if (response[0] != 0xFF || response[1] != 0x86) {
        ESP_LOGW(TAG, "Invalid preamble from ZE15-CO: Received: 0x%02X 0x%02X, but expected 0xFF 0x86", response[0], response[1]);
        this->status_set_warning();
        return;
    }
    ...
    if (response[8] != checksum) {
        ESP_LOGVV(TAG, "Received values from ZE15-CO: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", response[0], response[1], response[2], response[3], response[4], response[5], response[6], response[7], response[8]);
