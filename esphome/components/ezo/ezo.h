...
class EZOSensor : public sensor::Sensor, public PollingComponent, public i2c::I2CDevice {
 public:
  ...
  void set_calibration(const std::string &point, const std::string &value) {
    ...
    this->commands_.emplace_back(EzoCommand(to_send, EzoCommandType::EZO_CALIBRATION, 900));
  }
  void get_calibration() { this->commands_.emplace_back(EzoCommand("Cal,?", EzoCommandType::EZO_CALIBRATION)); }
  void clear_calibration() { this->commands_.emplace_back(EzoCommand("Cal,clear", EzoCommandType::EZO_CALIBRATION)); }
  ...
  // Slope
  void get_slope() { this->commands_.emplace_back(EzoCommand("Slope,?", EzoCommandType::EZO_SLOPE)); }
  ...
  void set_led_state(bool on) {
    ...
    to_send += on ? "1" : "0";
    this->commands_.emplace_back(EzoCommand(to_send, EzoCommandType::EZO_LED));
  }
  void get_led_state() { this->commands_.emplace_back(EzoCommand("L,?", EzoCommandType::EZO_LED)); }
