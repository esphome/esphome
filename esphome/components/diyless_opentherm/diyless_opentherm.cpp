...
void DiyLessOpenThermComponent::setup() {
  ...
  if (this->ch_setpoint_temperature_) {
    ...
    this->ch_setpoint_temperature_->add_on_state_callback([this](float temperature) {
      ESP_LOGI(TAG, "Request updating CH setpoint to %f", temperature);
    });
  }
  if (this->dhw_setpoint_temperature_) {
    this->dhw_setpoint_temperature_->setup();
    this->dhw_setpoint_temperature_->add_on_state_callback([this](float temperature) {
      ESP_LOGI(TAG, "Request updating CH setpoint to %f", temperature);
    });
  }
}
...
void DiyLessOpenThermComponent::loop() {
  ...
  if(millis() - lastMillis > 2000) {
    ...
    lastMillis = millis();
    this->enqueueRequest(openTherm->buildRequest(
      ihormelnyk::OpenThermMessageType::WRITE_DATA,
    ...
    if (this->confirmedDHWSetpoint != this->dhw_setpoint_temperature_->state) {
      this->enqueueRequest(openTherm->buildRequest(
        ihormelnyk::OpenThermMessageType::WRITE_DATA,
        ihormelnyk::OpenThermMessageID::TdhwSet,
...
void DiyLessOpenThermComponent::update() {
  this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::Tret, 0));
  this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::Tboiler, 0));
  this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::CHPressure, 0));
  this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::RelModLevel, 0));
  this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::RBPflags, 0));
  if (!this->DHWMinMaxRead) {
    this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::TdhwSetUBTdhwSetLB, 0));
  }
  if (!this->CHMinMaxRead) {
    this->enqueueRequest(openTherm->buildRequest(ihormelnyk::OpenThermMessageType::READ_DATA, ihormelnyk::OpenThermMessageID::MaxTSetUBMaxTSetLB, 0));
  }
  this->processStatus();
}
...
void DiyLessOpenThermComponent::logMessage(esp_log_level_t level, const char* preMessage, unsigned long message) {
  switch(level) {
    case ESP_LOG_DEBUG:
      ESP_LOGD(TAG, "%s: %s(%i, 0x%04hX)", preMessage, this->formatMessageType(message), openTherm->getDataID(message), openTherm->getUInt(message));
      break;
    default:
      ESP_LOGW(TAG, "%s: %s(%i, 0x%04hX)", preMessage, this->formatMessageType(message), openTherm->getDataID(message), openTherm->getUInt(message));
  }
}
...
void DiyLessOpenThermComponent::processStatus() {
  ...
  unsigned int data = this->CHEnabled | (this->DHWEnabled << 1) | (this->coolingEnabled << 2) | (false << 3) | (false << 4);
  data <<= 8;
  this->enqueueRequest(openTherm->buildRequest(
    ihormelnyk::OpenThermMessageType::READ_DATA,
    ihormelnyk::OpenThermMessageID::Status,
    data
  ));
}
