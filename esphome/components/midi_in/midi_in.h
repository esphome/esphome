...
namespace esphome
{
  namespace midi
  {
    ...
    public:
      ...
      void add_on_voice_message_callback(std::function<void(MidiVoiceMessage)> &&callback) { this->voice_message_callback_.add(std::move(callback)); }
      void add_on_system_message_callback(std::function<void(MidiSystemMessage)> &&callback) { this->system_message_callback_.add(std::move(callback)); }
