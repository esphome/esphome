...
// bDeviceClass = 0xEF (Miscellaneous Device Class), bDeviceSubClass = 0x02 (Common Class), bDeviceProtocol = 0x01
// (Interface Association Descriptor), or when bDeviceClass, bDeviceSubClass, and bDeviceProtocol are 0x00 (Null class
// code triple), as per https://www.usb.org/defined-class-codes, "Base Class 00h (Device)" section
 ...
 */
static bool cdc_parse_is_cdc_compliant(const usb_device_desc_t *device_desc, const usb_config_desc_t *config_desc,
                                       uint8_t intf_idx) {
  int desc_offset = 0;
  ...
  if (((device_desc->bDeviceClass == USB_CLASS_MISC) && (device_desc->bDeviceSubClass == USB_SUBCLASS_COMMON) &&
    ...
    while ((this_desc = usb_parse_next_descriptor_of_type(this_desc, config_desc->wTotalLength,
                                                          USB_B_DESCRIPTOR_TYPE_INTERFACE_ASSOCIATION, &desc_offset))) {
      const usb_iad_desc_t *iad_desc = (const usb_iad_desc_t *) this_desc;
      if ((iad_desc->bFirstInterface == intf_idx) && (iad_desc->bInterfaceCount == 2) &&
 ...
 */
static cdc_func_array_t *cdc_parse_functional_descriptors(const usb_intf_desc_t *intf_desc, uint16_t total_len,
                                                          int desc_offset, int *desc_cnt) {
...
esp_err_t cdc_parse_interface_descriptor(const usb_device_desc_t *device_desc, const usb_config_desc_t *config_desc,
  ...
  for (int i = 0; i < first_intf_desc->bNumEndpoints; i++) {
    const usb_ep_desc_t *this_ep =
        usb_parse_endpoint_descriptor_by_index(first_intf_desc, i, config_desc->wTotalLength, &desc_offset);
    assert(this_ep);
  ...
  if (cdc_compliant) {
    info_ret->notif_intf = first_intf_desc;  // We make sure that intf_desc is set for CDC compliant devices that use
                                             // EP0 as notification element
    info_ret->func =
        cdc_parse_functional_descriptors(first_intf_desc, config_desc->wTotalLength, desc_offset, &info_ret->func_cnt);
  }
  ...
  if (!info_ret->data_intf && cdc_compliant) {
    ...
    // First interface with 0 endpoints (default control pipe only) and second with standard 2 endpoints for full-duplex
    // data We always select interface with 2 bulk endpoints
    ...
    for (int i = 0; i < num_of_alternate + 1; i++) {
      const usb_intf_desc_t *second_intf_desc =
          usb_parse_interface_descriptor(config_desc, intf_idx + 1, i, &desc_offset);
      ...
      if (second_intf_desc && second_intf_desc->bNumEndpoints == 2) {
        for (int i = 0; i < second_intf_desc->bNumEndpoints; i++) {
          const usb_ep_desc_t *this_ep =
              usb_parse_endpoint_descriptor_by_index(second_intf_desc, i, config_desc->wTotalLength, &desc_offset);
          assert(this_ep);
