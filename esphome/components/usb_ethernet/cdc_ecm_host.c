 ...
 * @return esp_err_t
 */
static esp_err_t cdc_ecm_start(cdc_dev_t *cdc_dev, cdc_ecm_host_dev_callback_t event_cb, cdc_ecm_data_callback_t in_cb,
                               void *user_arg) {
  ...
  ESP_GOTO_ON_ERROR(
      usb_host_interface_claim(p_cdc_ecm_obj->cdc_ecm_client_hdl, cdc_dev->dev_hdl,
  ...
  if (cdc_dev->notif.xfer) {
    if (cdc_dev->notif.intf_desc != cdc_dev->data.intf_desc) {
      ESP_GOTO_ON_ERROR(usb_host_interface_claim(p_cdc_ecm_obj->cdc_ecm_client_hdl, cdc_dev->dev_hdl,
...
err:
  usb_host_interface_release(p_cdc_ecm_obj->cdc_ecm_client_hdl, cdc_dev->dev_hdl,
...
static void cdc_ecm_device_remove(cdc_dev_t *cdc_dev) {
  ...
  // We don't check the error code of usb_host_device_close, as the close might fail, if someone else is still using the
  // device (not all interfaces are released)
 ...
 * 1. USB device with matching VID/PID is already opened by this driver: allocate new CDC device on top of the already
 * opened USB device.
 * 2. USB device with matching VID/PID is NOT opened by this driver yet: poll USB connected devices until it is found.
 *
 * @note This function will block for timeout_ms, if the device is not enumerated at the moment of calling this
 * function.
...
esp_err_t cdc_ecm_host_install(const cdc_ecm_host_driver_config_t *driver_config) {
  ...
  xTaskCreatePinnedToCore(cdc_ecm_client_task, "USB-CDC", driver_config->driver_task_stack_size, NULL,
  ...
  const usb_host_client_config_t client_config = {.is_synchronous = false,
                                                  .max_num_event_msg = 3,
                                                  .async.client_event_callback = usb_event_cb,
                                                  .async.callback_arg = NULL};
  ESP_GOTO_ON_ERROR(usb_host_client_register(&client_config, &usb_client), err, TAG,
                    "Failed to register USB host client");
...
esp_err_t cdc_ecm_host_uninstall() {
  ...
  CDC_ECM_ENTER_CRITICAL();
  if (SLIST_EMPTY(&p_cdc_ecm_obj->cdc_devices_list)) {  // Check that device list is empty (all devices closed)
 ...
 */
static esp_err_t cdc_ecm_transfers_allocate(cdc_dev_t *cdc_dev, const usb_ep_desc_t *notif_ep_desc,
                                            const usb_ep_desc_t *in_ep_desc, size_t in_buf_len,
                                            const usb_ep_desc_t *out_ep_desc, size_t out_buf_len) {
  assert(in_ep_desc);
  assert(out_ep_desc);
  esp_err_t ret;
  ...
  if (notif_ep_desc) {
    ESP_LOGD(TAG, "Setting up Notifications transfer on endpoint: 0x%02X, MPS: %d", notif_ep_desc->bEndpointAddress,
             USB_EP_DESC_GET_MPS(notif_ep_desc));
    ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(USB_EP_DESC_GET_MPS(notif_ep_desc), 0, &cdc_dev->notif.xfer), err, TAG, );
  ...
  ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(CDC_ECM_CTRL_TRANSFER_SIZE, 0, &cdc_dev->ctrl_transfer), err, TAG, );
  ...
  if (in_buf_len != 0) {
    ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(in_buf_len, 0, &cdc_dev->data.in_xfer), err, TAG, );
    assert(cdc_dev->data.in_xfer);
  ...
  if (out_buf_len != 0) {
    ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(out_buf_len, 0, &cdc_dev->data.out_xfer), err, TAG, );
...
esp_err_t cdc_ecm_host_open(uint16_t vid, uint16_t pid, uint8_t interface_idx,
                            const cdc_ecm_host_device_config_t *dev_config, cdc_ecm_dev_hdl_t *cdc_hdl_ret) {
  esp_err_t ret;
  ...
  ESP_GOTO_ON_ERROR(cdc_parse_interface_descriptor(device_desc, config_desc, interface_idx, &cdc_info), err, TAG,
                    "Could not open required interface as CDC");
  ...
  const size_t in_buf_size = (dev_config->data_cb && (dev_config->in_buffer_size == 0))
                                 ? USB_EP_DESC_GET_MPS(cdc_info.in_ep)
                                 : dev_config->in_buffer_size;

  ESP_LOGD(TAG,
           "CDC-ECM device opened: VID: 0x%04X, PID: 0x%04X, Notification Endpoint: 0x%02X, IN Endpoint: 0x%02X, OUT "
           "Endpoint: 0x%02X, bAlternateSetting: %d",
           device_desc->idVendor, device_desc->idProduct, cdc_info.notif_ep->bEndpointAddress,
           cdc_info.in_ep->bEndpointAddress, cdc_info.out_ep->bEndpointAddress,
           cdc_dev->data.intf_desc->bAlternateSetting);

  // Allocate USB transfers, claim CDC interfaces and return CDC-ECM handle
  ESP_GOTO_ON_ERROR(cdc_ecm_transfers_allocate(cdc_dev, cdc_info.notif_ep, cdc_info.in_ep, in_buf_size, cdc_info.out_ep,
                                               dev_config->out_buffer_size),
                    err, TAG, );  // TODO: update buffers based on device configuration values.
  ESP_GOTO_ON_ERROR(cdc_ecm_start(cdc_dev, dev_config->event_cb, dev_config->data_cb, dev_config->user_arg), err,
                    TAG, );
...
esp_err_t cdc_ecm_host_close(cdc_ecm_dev_hdl_t cdc_hdl) {
  ...
  ESP_ERROR_CHECK(usb_host_interface_release(p_cdc_ecm_obj->cdc_ecm_client_hdl, cdc_dev->dev_hdl,
                                             cdc_dev->data.intf_desc->bInterfaceNumber));
...
static bool cdc_ecm_is_transfer_completed(usb_transfer_t *transfer) {
  ...
  switch (transfer->status) {
    ...
    default:
      ...
      if (cdc_dev->notif.cb) {
        const cdc_ecm_host_dev_event_data_t error_event = {.type = CDC_ECM_HOST_EVENT_ERROR,
...
static void in_xfer_cb(usb_transfer_t *transfer) {
  ...
  if (cdc_dev->data.in_cb) {
    ...
    // In order to save RAM and CPU time, the application can indicate that the received data was not processed and that
    // the application expects more data. In this case, the next received data must be appended to the existing buffer.
  ...
  if (cdc_ecm_is_transfer_completed(transfer)) {
    ...
    switch (notif->bNotificationCode) {
      case USB_CDC_NOTIF_NETWORK_CONNECTION: {
        if (cdc_dev->notif.cb) {
          const cdc_ecm_host_dev_event_data_t net_conn_event = {.type = CDC_ECM_HOST_EVENT_NETWORK_CONNECTION,
      ...
      case USB_CDC_NOTIF_CONNECTION_SPEED_CHANGE: {
        if (cdc_dev->notif.cb) {
          if (notif->wLength == sizeof(cdc_ecm_speed_change_data_t)) {
            ...
            const cdc_ecm_host_dev_event_data_t speed_change_event = {.type = CDC_ECM_HOST_EVENT_SPEED_CHANGE,
...
static void usb_event_cb(const usb_host_client_event_msg_t *event_msg, void *arg) {
  switch (event_msg->event) {
    ...
    case USB_HOST_CLIENT_EVENT_DEV_GONE: {
      ...
      cdc_dev_t *cdc_dev;
      cdc_dev_t *tcdc_dev;
      // We are using 'SAFE' version of 'SLIST_FOREACH' which enables user to close the disconnected device in the
      // callback
...
esp_err_t cdc_ecm_host_data_tx_blocking(cdc_ecm_dev_hdl_t cdc_hdl, const uint8_t *data, size_t data_len,
                                        uint32_t timeout_ms) {
  esp_err_t ret;
  CDC_ECM_CHECK(cdc_hdl, ESP_ERR_INVALID_ARG);
  cdc_dev_t *cdc_dev = (cdc_dev_t *) cdc_hdl;
  ...
  if (!taken) {
    cdc_acm_reset_transfer_endpoint(
        cdc_dev->dev_hdl,
        cdc_dev->data.out_xfer);  // Resetting the endpoint will cause all in-progress transfers to complete
    ESP_LOGW(TAG, "TX transfer timeout");
    ret = ESP_ERR_TIMEOUT;
    goto unblock;
  }

  ESP_GOTO_ON_FALSE(cdc_dev->data.out_xfer->status == USB_TRANSFER_STATUS_COMPLETED, ESP_ERR_INVALID_RESPONSE, unblock,
                    TAG, "Bulk OUT transfer error");
  ESP_GOTO_ON_FALSE(cdc_dev->data.out_xfer->actual_num_bytes == data_len, ESP_ERR_INVALID_RESPONSE, unblock, TAG,
                    "Incorrect number of bytes transferred");
  ret = ESP_OK;
...
esp_err_t cdc_ecm_set_interface(cdc_ecm_dev_hdl_t cdc_hdl) {
  ...
  ESP_LOGD(TAG, "Setting Ethernet Interface using bAlternateSetting: %d, bInterfaceNumber %d", bAlternateSetting,
           cdc_dev->data.intf_desc->bInterfaceNumber);

  esp_err_t err = cdc_ecm_host_send_custom_request(
      cdc_hdl, USB_BM_REQUEST_TYPE_DIR_OUT | USB_BM_REQUEST_TYPE_TYPE_STANDARD | USB_BM_REQUEST_TYPE_RECIP_INTERFACE,
      USB_B_REQUEST_SET_INTERFACE, bAlternateSetting, cdc_dev->data.intf_desc->bInterfaceNumber, 0, NULL);
  if (err != ESP_OK) {
...
esp_err_t cdc_ecm_set_packet_filter(cdc_ecm_dev_hdl_t cdc_hdl, uint16_t filter_mask) {
  ...
  esp_err_t err = cdc_ecm_host_send_custom_request(
      cdc_hdl, USB_BM_REQUEST_TYPE_DIR_OUT | USB_BM_REQUEST_TYPE_TYPE_CLASS | USB_BM_REQUEST_TYPE_RECIP_INTERFACE,
      USB_CDC_REQ_SET_ETHERNET_PACKET_FILTER, aligned_filter_mask, cdc_dev->data.intf_desc->bInterfaceNumber, 0, NULL);
  if (err != ESP_OK) {
...
esp_err_t cdc_ecm_set_multicast_filter(cdc_ecm_dev_hdl_t cdc_hdl, uint8_t *mac) {
  ...
  ESP_LOGI(TAG, "Setting Ethernet Multicast Filter: %02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3],
           mac[4], mac[5]);

  esp_err_t err = cdc_ecm_host_send_custom_request(
      cdc_hdl, USB_BM_REQUEST_TYPE_DIR_OUT | USB_BM_REQUEST_TYPE_TYPE_CLASS | USB_BM_REQUEST_TYPE_RECIP_INTERFACE,
      USB_CDC_REQ_SET_ETHERNET_MULTICAST_FILTERS, 0, cdc_dev->data.intf_desc->bInterfaceNumber, sizeof(mac) * 6,
      (uint8_t *) &mac);
  if (err != ESP_OK) {
...
esp_err_t cdc_ecm_get_string_desc(cdc_ecm_dev_hdl_t cdc_hdl, uint16_t index, uint8_t *output) {
  ...
  esp_err_t err = cdc_ecm_host_send_custom_request(
      cdc_hdl, USB_BM_REQUEST_TYPE_DIR_IN | USB_BM_REQUEST_TYPE_TYPE_STANDARD | USB_BM_REQUEST_TYPE_RECIP_DEVICE,
      USB_CDC_REQ_GET_DESCRIPTOR, (uint16_t) ((USB_DESCRIPTOR_TYPE_STRING << 8) | index),
...
esp_err_t cdc_ecm_host_send_custom_request(cdc_ecm_dev_hdl_t cdc_hdl, uint8_t bmRequestType, uint8_t bRequest,
  ...
  if (wLength > 0) {
    ...
    ESP_LOGI(TAG, "Setting Custom Data: %02X:%02X:%02X:%02X:%02X:%02X", data[0], data[1], data[2], data[3], data[4],
             data[5]);
  }
  CDC_ECM_CHECK(cdc_dev->ctrl_transfer->data_buffer_size >= wLength,
                ESP_ERR_INVALID_SIZE);  // TODO: need to reinstte this
  ...
  ESP_GOTO_ON_ERROR(usb_host_transfer_submit_control(p_cdc_ecm_obj->cdc_ecm_client_hdl, cdc_dev->ctrl_transfer),
  ...
  ESP_GOTO_ON_FALSE(cdc_dev->ctrl_transfer->status == USB_TRANSFER_STATUS_COMPLETED, ESP_ERR_INVALID_RESPONSE, unblock,
                    TAG, "Control transfer error");
...
esp_err_t cdc_ecm_host_cdc_desc_get(cdc_ecm_dev_hdl_t cdc_hdl, cdc_desc_subtype_t desc_type,
...
static esp_err_t netif_transmit(void *h, void *buffer, size_t len) {
  ...
  while (remaining_len > 0) {
    ...
    if (err != ESP_OK) {
      ...
      ESP_LOGW(TAG, "netif_transmit: dropping frame chunk, tx_blocking failed: %s", esp_err_to_name(err));
...
esp_err_t cdc_ecm_netif_init(cdc_ecm_dev_hdl_t cdc_hdl, cdc_ecm_params_t *params) {
  ...
  esp_netif_inherent_config_t base_cfg = {.flags = netif_flags,
                                          .ip_info = &ip_info,
  ...
  esp_netif_driver_ifconfig_t driver_cfg = {
      .handle = cdc_hdl, .transmit = netif_transmit, .driver_free_rx_buffer = l2_free};
...
static void cdc_ecm_task(void *arg) {
  ...
  BaseType_t task_created =
      xTaskCreate(usb_lib_task, "usb_lib", 4096, xTaskGetCurrentTaskHandle(), CDC_ECM_USB_HOST_PRIORITY, NULL);
...
void cdc_ecm_init(cdc_ecm_params_t *cdc_ecm_params) {
  ...
  BaseType_t task_created =
      xTaskCreate(cdc_ecm_task, "cdc_ecm_task", 1024 * 8, cdc_ecm_params, CDC_ECM_USB_HOST_PRIORITY, NULL);
  assert(task_created == pdTRUE);
}
