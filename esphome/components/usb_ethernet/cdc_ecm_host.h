 ...
 */
typedef void (*cdc_ecm_event_callback_t)(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data);
 ...
 * PN COMPLETED
 *
 * The driver first looks for CDC compliant descriptor, if it is not found the driver checks if the interface has 2 Bulk
 * endpoints that can be used for data
 *
 * Use CDC_HOST_ANY_* macros to signal that you don't care about the device's VID and PID. In this case, first USB
 * device will be opened. It is recommended to use this feature if only one device can ever be in the system (there is
...
class CdcAcmDevice {
 public:
  ...
  inline esp_err_t open_vendor_specific(uint16_t vid, uint16_t pid, uint8_t interface_idx,
                                        const cdc_ecm_host_device_config_t *dev_config) {
    return cdc_ecm_host_open(vid, pid, interface_idx, dev_config, &this->cdc_hdl);
  }
  ...
  virtual inline esp_err_t set_interface() { return cdc_ecm_set_interface(this->cdc_hdl); }

  virtual inline esp_err_t set_packet_filter(uint32_t filter_mask) {
    return cdc_ecm_set_packet_filter(this->cdc_hdl, filter_mask);
  }

  inline esp_err_t send_custom_request(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex,
