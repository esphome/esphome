...
uint8_t BedJetHub::set_notify_(const bool enable) {
  uint8_t status;
  if (enable) {
    status = esp_ble_gattc_register_for_notify(this->parent->get_gattc_if(), this->parent_->remote_bda,
                                               this->char_handle_status_);
    if (status) {
      ESP_LOGW(TAG, "[%s] esp_ble_gattc_register_for_notify failed, status=%d", this->get_name().c_str(), status);
    }
  } else {
    status = esp_ble_gattc_unregister_for_notify(this->parent->get_gattc_if(), this->parent_->remote_bda,
                                                 this->char_handle_status_);
    if (status) {
...
void BedJetHub::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
  ...
  switch (event) {
    ...
    case ESP_GATTC_OPEN_EVT: {
      ...
      this->parent->get_conn_id() = param->open.conn_id;
      this->open_conn_id_ = param->open.conn_id;
      break;
    }

    case ESP_GATTC_CONNECT_EVT: {
      if (this->parent->get_conn_id() != param->connect.conn_id && this->open_conn_id_ != 0xff) {
        // FIXME: bug in BLEClient
        ESP_LOGW(TAG, "[%s] CONNECT_EVT unexpected conn_id; open=%d, parent=%d, param=%d", this->get_name().c_str(),
                 this->open_conn_id_, this->parent->get_conn_id(), param->connect.conn_id);
        this->parent->get_conn_id() = this->open_conn_id_;
      }
      break;
    }
    ...
    case ESP_GATTC_NOTIFY_EVT: {
      ...
      if (param->notify.conn_id != this->parent->get_conn_id()) {
        ESP_LOGW(TAG, "[%s] Received notify event for unexpected parent conn: expect %x, got %x",
                 this->get_name().c_str(), this->parent->get_conn_id(), param->notify.conn_id);
...
uint8_t BedJetHub::write_notify_config_descriptor_(bool enable) {
  ...
  auto status = esp_ble_gattc_write_char_descr(this->parent->get_gattc_if(), this->parent->get_conn_id(), handle,
  ...
  ESP_LOGD(TAG, "[%s] wrote notify=%s to status config 0x%04x, for conn %d", this->get_name().c_str(),
           enable ? "true" : "false", handle, this->parent->get_conn_id());
  return ESP_GATT_OK;
}
