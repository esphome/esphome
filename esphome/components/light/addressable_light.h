...
class ESPColorCorrection {
 public:
  ...
  inline uint8_t color_correct_red(uint8_t red) const ALWAYS_INLINE {
    uint8_t res = esp_scale8(esp_scale8(red, this->max_individual_brightness_.red), this->local_brightness_);
    return this->gamma_table_[res];
  }
  inline uint8_t color_correct_green(uint8_t green) const ALWAYS_INLINE {
    uint8_t res = esp_scale8(esp_scale8(green, this->max_individual_brightness_.green), this->local_brightness_);
    return this->gamma_table_[res];
  }
  inline uint8_t color_correct_blue(uint8_t blue) const ALWAYS_INLINE {
    uint8_t res = esp_scale8(esp_scale8(blue, this->max_individual_brightness_.blue), this->local_brightness_);
    return this->gamma_table_[res];
  }
  ...
  inline Color color_uncorrect(Color color) const ALWAYS_INLINE {
    // uncorrected = corrected^(1/gamma) / (max_individual_brightness * local_brightness)
  ...
  inline uint8_t color_uncorrect_red(uint8_t red) const ALWAYS_INLINE {
    if (this->max_individual_brightness_.red == 0 || this->local_brightness_ == 0)
      return 0;
    uint16_t uncorrected = this->gamma_reverse_table_[red] * 255UL;
    uint8_t res = ((uncorrected / this->max_individual_brightness_.red) * 255UL) / this->local_brightness_;
    return res;
  }
  inline uint8_t color_uncorrect_green(uint8_t green) const ALWAYS_INLINE {
    if (this->max_individual_brightness_.green == 0 || this->local_brightness_ == 0)
      return 0;
    uint16_t uncorrected = this->gamma_reverse_table_[green] * 255UL;
    uint8_t res = ((uncorrected / this->max_individual_brightness_.green) * 255UL) / this->local_brightness_;
    return res;
  }
  inline uint8_t color_uncorrect_blue(uint8_t blue) const ALWAYS_INLINE {
    if (this->max_individual_brightness_.blue == 0 || this->local_brightness_ == 0)
      return 0;
    uint16_t uncorrected = this->gamma_reverse_table_[blue] * 255UL;
    uint8_t res = ((uncorrected / this->max_individual_brightness_.blue) * 255UL) / this->local_brightness_;
    return res;
  }
...
class AddressableLight : public LightOutput, public Component {
 public:
  ...
  void set_max_brightness(float max) { this->correction_.set_max_overall_brightness(uint8_t(roundf(max * 255.0f))); }
  void set_min_brightness(float min) { this->correction_.set_min_overall_brightness(uint8_t(roundf(min * 255.0f))); }
