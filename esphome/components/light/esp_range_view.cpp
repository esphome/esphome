...
void ESPRangeView::set_cold_white(uint8_t cold_white) {
  for (auto c : *this)
    c.set_cold_white(cold_white);
}
void ESPRangeView::set_warm_white(uint8_t warm_white) {
  for (auto c : *this)
    c.set_warm_white(warm_white);
}
