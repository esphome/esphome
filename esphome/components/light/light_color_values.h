...
class LightColorValues {
 public:
  ...
  ///
  /// Note on gamma and constant_brightness: This method operates on raw (linear) channel
  /// values. For constant_brightness=false, gamma can be applied after this method since
  /// gamma commutes with simple multiplication. For constant_brightness=true, the caller
  /// (LightState::current_values_as_cwww) must apply gamma to the individual channel values
  /// BEFORE the balancing formula, because the nonlinear max/sum ratio does not commute
  /// with gamma. See LightState::current_values_as_cwww() for the correct implementation.
