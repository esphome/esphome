...
class LightColorValues {
 public:
  ...
  void as_emulated_rgbww(float *red, float *green, float *blue, float *white, float blue_white_percentage,
                         float red_white_percentage, float min_mireds, float max_mireds, bool additive_brightness,
                         float gamma = 0) const {
    if (this->color_mode_ & ColorCapability::RGB) {
      this->as_rgb(red, green, blue, gamma);
      *white = 0;
      ...
      *white = (additive_brightness)
                   ? gamma_correct(this->state_ * this->brightness_ * this->white_, gamma)
                   : gamma_correct(this->state_ * (this->brightness_) * this->white_, gamma) - *blue - *red;
    }
  }
