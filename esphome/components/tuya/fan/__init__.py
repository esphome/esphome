...
CONF_PRESET_MODES_DATAPOINT = "preset_modes_datapoint"
    ...
    .extend(
        {
            ...
            cv.Optional(CONF_PRESET_MODES_DATAPOINT): cv.uint8_t,
            cv.Optional(CONF_PRESET_MODES): validate_preset_modes,
        }
    )
    .extend(cv.COMPONENT_SCHEMA),
    cv.has_at_least_one_key(CONF_SPEED_DATAPOINT, CONF_SWITCH_DATAPOINT),
    cv.has_none_or_all_keys(CONF_PRESET_MODES_DATAPOINT, CONF_PRESET_MODES),
)
...
async def to_code(config):
    ...
    if CONF_PRESET_MODES_DATAPOINT in config:
        cg.add(var.set_preset_modes_id(config[CONF_PRESET_MODES_DATAPOINT]))
