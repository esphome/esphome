...
NUMBER_OF_INTERRUPT_PULSECOUNTERS = 0

ESP32PulseCounterSensor = esp32_pulse_counter_ns.class_(
    "ESP32PulseCounterSensor", sensor.Sensor, cg.PollingComponent
)
...
def validate_pulse_counter_pin(value):
    ...
    if NUMBER_OF_INTERRUPT_PULSECOUNTERS < 8:
        NUMBER_OF_INTERRUPT_PULSECOUNTERS += 1
    else:
        raise cv.Invalid("Only 8 interrupt pulse-counters are supported by ESP32")
...
CONFIG_SCHEMA = (
    ...
    .extend(
        {
            cv.GenerateID(): cv.declare_id(ESP32PulseCounterSensor),
