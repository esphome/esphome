...
static const char *TAG = "esp32_pulse_counter";

const char *EDGE_MODE_TO_STRING[] = {"DISABLE", "INCREMENT", "DECREMENT"};

bool ESP32PulseCounterStorage::esp32_pulse_counter_setup(GPIOPin *pin) {
  this->pin = pin;
  this->pin->setup();
  ...
  if (error != ESP_OK) {
    ESP_LOGE(TAG, "Configuring ESP32 Pulse Counter failed: %s", esp_err_to_name(error));
    return false;
  }
...
pulse_counter_t ESP32PulseCounterStorage::read_raw_value() {
...
void ESP32PulseCounterSensor::setup() {
  ESP_LOGCONFIG(TAG, "Setting up pulse counter '%s'...", this->name_.c_str());
  if (!this->storage_.esp32_pulse_counter_setup(this->pin_)) {
    this->mark_failed();
    return;
  }
}

void ESP32PulseCounterSensor::dump_config() {
...
void ESP32PulseCounterSensor::update() {
