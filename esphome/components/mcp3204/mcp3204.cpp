...
float MCP3204::read_data(uint8_t pin) {
  uint8_t adc_primary_config = 0b00000110 & 0b00000111; // ensures the ADC register is limited to the mask and assembles the configuration byte to send to ADC.
  uint8_t adc_secondary_config = pin << 6; // shift the bits to get a binary number
  this->enable();
  this->transfer_byte(adc_primary_config); //  send in the primary configuration address byte -- we don't need a return
  uint8_t adc_primary_byte = this->transfer_byte(adc_secondary_config); // read the primary byte, also sending in the secondary address byte.
  uint8_t adc_secondary_byte = this->transfer_byte(0x00); // read the secondary byte, also sending 0 as this doesn't matter.
  this->disable();
  uint16_t digital_value = ( adc_primary_byte << 8 | adc_secondary_byte ) & 0b111111111111; //cast to keep only our 12 bits
  return float(digital_value) / 4096.000;   // 4096 is the max, so divide according to the datasheet
}
