    ...
    case 1: {
      ...
      for (auto *lora_component : this->switches_) {
        if (lora_component->receive_from_lora && lora_component->lora_name == lora_packet->component_name) {
          ...
          lora_component->component->publish_state(lora_packet->state != 0);
          break;
        }
      }
#endif  // USE_SWITCH
...
std::string LoraComponent::build_to_send(const std::string &type, const std::string &name, const std::string &state) {
  // App name
...
bool LoraComponent::process_component(LoraBaseComponent *lora_component, float state) {
  if (!lora_component->send_to_lora)
    return true;

  std::string to_send = this->build_to_send(lora_component->type, lora_component->lora_name, to_string(state));
...
void LoraComponent::register_binary_sensor(binary_sensor::BinarySensor *component, bool send_to_lora,
                                           bool receive_from_lora, const std::string &lora_name) {
  ...
  if (send_to_lora) {
    ...
    component->add_on_state_callback(
        [this, lora_component](bool state) { this->process_component(lora_component, state); });
  }
}
#endif  // USE_BINARY_SENSOR
...
void LoraComponent::register_text_sensor(text_sensor::TextSensor *component, bool send_to_lora, bool receive_from_lora,
  ...
  if (send_to_lora) {
    ...
    component->add_on_state_callback(
        [this, lora_component](std::string state) { this->process_component(lora_component, std::move(state)); });
  }
}
#endif  // USE_TEXT_SENSOR

#ifdef USE_SWITCH
bool LoraComponent::process_component(LoraBaseComponent *lora_component, bool state) {
  if (!lora_component->send_to_lora)
    return true;

  std::string to_send = this->build_to_send(lora_component->type, lora_component->lora_name, state ? "1" : "0");
...
void LoraComponent::register_sensor(sensor::Sensor *sensor, bool send_to_lora, bool receive_from_lora,
  ...
  if (send_to_lora) {
    ...
    sensor->add_on_state_callback(
        [this, lora_component](float state) { this->process_component(lora_component, state); });
  }
}
#endif  // USE_SENSOR
