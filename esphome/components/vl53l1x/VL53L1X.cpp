...
bool VL53L1X::init(bool io_2v8) {
  ...
  if (read_reg16_bit(IDENTIFICATION__MODEL_ID) != 0xEACC) {
    return false;
  }
  ...
  write_reg16_bit(ALGO__PART_TO_PART_RANGE_OFFSET_MM, read_reg16_bit(MM_CONFIG__OUTER_OFFSET_MM) * 4);
...
bool VL53L1X::set_measurement_timing_budget(uint32_t budget_us) {
  ...
  write_reg16_bit(MM_CONFIG__TIMEOUT_MACROP_A, encode_timeout(timeout_microseconds_to_mclks(1, macro_period_us)));

  // "Update Range Timing A timeout"
  write_reg16_bit(RANGE_CONFIG__TIMEOUT_MACROP_A,
                  encode_timeout(timeout_microseconds_to_mclks(range_config_timeout_us, macro_period_us)));
  ...
  write_reg16_bit(MM_CONFIG__TIMEOUT_MACROP_B, encode_timeout(timeout_microseconds_to_mclks(1, macro_period_us)));

  // "Update Range Timing B timeout"
  write_reg16_bit(RANGE_CONFIG__TIMEOUT_MACROP_B,
                  encode_timeout(timeout_microseconds_to_mclks(range_config_timeout_us, macro_period_us)));
...
uint32_t VL53L1X::get_measurement_timing_budget() {
  ...
  uint32_t range_config_timeout_us =
      timeout_mclks_to_microseconds(decode_timeout(read_reg16_bit(RANGE_CONFIG__TIMEOUT_MACROP_A)), macro_period_us);
...
void VL53L1X::update_dss_() {
  ...
  if (spad_count != 0) {
    ...
    uint32_t total_rate_per_spad =
        (uint32_t) results_.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 + results_.ambient_count_rate_mcps_sd0;
...
void VL53L1X::get_ranging_data_() {
  ...
  ranging_data.ambient_count_rate_MCPS = count_rate_fixed_to_float_(results_.ambient_count_rate_mcps_sd0);
}

// Decode sequence step timeout in MCLKs from register value
// based on VL53L1_decode_timeout()
uint32_t VL53L1X::decode_timeout(uint16_t reg_val) { return ((uint32_t)(reg_val & 0xFF) << (reg_val >> 8)) + 1; }
