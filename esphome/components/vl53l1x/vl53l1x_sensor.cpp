...
void VL53L1XSensor::setup() {
  ...
  if (!esphome::vl53l1x::VL53L1XSensor::enable_pin_setup_complete) {
    for (auto &vl53_sensor : vl53l1x_sensors) {
      if (vl53_sensor->enable_pin_ != nullptr) {
        ...
        vl53_sensor->enable_pin_->digital_write(LOW);
      }
    }
  ...
  if (this->offset_ != 0) { this->vl53l1x_set_offset(this->offset_); }
...
bool VL53L1XSensor::begin(uint8_t i2c_addr) {
  if (this->enable_pin_ != nullptr) {
    this->enable_pin_->digital_write(HIGH);
    this->enable_pin_->digital_write(LOW);
    delay(5);
    this->enable_pin_->digital_write(HIGH);
  }
  delay(5);
...
VL53L1X_Error VL53L1XSensor::vl53l1x_set_distance_threshold(
    uint16_t thresh_low,
    uint16_t thresh_high, // NOLINT(bugprone-easily-swappable-parameters)
    uint8_t window,
...
VL53L1X_Error VL53L1XSensor::vl53l1x_get_roi_xy(uint16_t *roi_x, uint16_t *roi_y) // NOLINT(bugprone-easily-swappable-parameters)
{
  VL53L1X_Error status = 0;
  uint8_t tmp;
...
VL53L1X_Error VL53L1XSensor::vl53l1x_wr_word(uint16_t index, uint16_t data) // NOLINT(bugprone-easily-swappable-parameters)
{
  uint8_t buffer[2];
...
VL53L1X_Error VL53L1XSensor::vl53l1x_wr_dword(uint16_t index, uint32_t data) // NOLINT(bugprone-easily-swappable-parameters)
{
  uint8_t buffer[4];
...
VL53L1X_Error VL53L1XSensor::vl53l1x_update_byte(uint16_t index, uint8_t and_data, uint8_t or_data) // NOLINT(bugprone-easily-swappable-parameters)
{
  int  status;
  uint8_t buffer = 0;
...
VL53L1X_Error VL53L1XSensor::vl53l1x_get_tick_count(
...
VL53L1X_Error VL53L1XSensor::vl53l1x_wait_value_mask_ex(
    uint32_t      timeout_ms, // NOLINT(bugprone-easily-swappable-parameters)
    uint16_t      index,
    uint8_t       value,
    uint8_t       mask, // NOLINT(bugprone-easily-swappable-parameters)
    uint32_t      poll_delay_ms)
{
  ...
  vl53l1x_get_tick_count(&start_time_ms);
  ...
  while ((status == this->VL53L1X_ERROR_NONE) &&
