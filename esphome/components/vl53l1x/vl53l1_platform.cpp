...
void set_bootstrap_device(::esphome::i2c::I2CDevice *dev) { bootstrap_device = dev; }

/** Clears the bootstrap device. */
void clear_bootstrap_device() { bootstrap_device = nullptr; }
...
void register_sensor(::esphome::i2c::I2CDevice *dev) { devices[dev->get_i2c_address()] = dev; }
...
int8_t VL53L1_RdWord(uint16_t dev, uint16_t index, uint16_t *data) {
  ...
  uint16_t tmp_data = 0;
  if ((status = VL53L1_ReadMulti(dev, index, reinterpret_cast<uint8_t *>(&tmp_data), sizeof(tmp_data))) ==
      VL53L1X_ERROR_NONE) {
