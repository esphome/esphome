...
void AddressableLightDisplay::setup() {
  this->buffer_.resize(this->width_ * this->height_, {0,0,0,0});
};
...
void AddressableLightDisplay::display() {
  ...
  for (uint32_t offset = 0; offset < this->buffer_.size(); offset++) {
    c = &(this->buffer_[offset]);
    ...
    if (pixel.get_red() != old_r || pixel.get_green() != old_g || pixel.get_blue() != old_b || pixel.get_white() != old_w) {
      dirty = true;
    }
  }
...
void HOT AddressableLightDisplay::draw_absolute_pixel_internal(int x, int y, Color color) {
  ...
  this->buffer_[y * this->get_width_internal() + x] = color;
}
}  // namespace addressable_light_display
}  // namespace esphome
