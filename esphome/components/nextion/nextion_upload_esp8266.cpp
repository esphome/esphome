...
int Nextion::upload_by_chunks_(HTTPClient &http_client, uint32_t &range_start) {
  ...
  ESP_LOGV(TAG, "Free heap: %" PRIu32, EspClass::getFreeHeap());
...
bool Nextion::upload_tft(uint32_t baud_rate, bool exit_reparse) {
  ...
  ESP_LOGD(TAG, "  Free heap: %" PRIu32, EspClass::getFreeHeap());
  ...
  while (this->content_length_ > 0) {
    ...
    App.feed_wdt();
    ESP_LOGV(TAG, "Free heap: %" PRIu32 ", Bytes left: %" PRIu32, EspClass::getFreeHeap(), this->content_length_);
  }
