...
void ZigbeeTime::sync_time(zb_ret_t status, zb_uint32_t auth_level, zb_uint16_t short_addr, zb_uint8_t ep,
                           zb_uint32_t nw_time) {
...
void ZigbeeTime::setup() {
  ...
  parent_->add_join_callback([this]() { zb_zcl_time_server_synchronize(endpoint_, sync_time); });
}
    ...
    case ZB_ZCL_SET_ATTR_VALUE_CB_ID:
      if (cluster_id == ZB_ZCL_CLUSTER_ID_TIME) {
        if (attr_id == ZB_ZCL_ATTR_TIME_TIME_ID) {
          ...
          this->has_time_ = ZB_ZCL_TIME_TIME_STATUS_SYNCHRONIZED_BIT_IS_SET(value);
        }
      } else {
