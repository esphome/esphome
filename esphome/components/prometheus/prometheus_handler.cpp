...
std::string PrometheusHandler::relabel_(const std::string &value) {
  auto item = relabel_map_.find(value);
  return item == relabel_map_.end() ? value : item->second;
}
...
void PrometheusHandler::sensor_row_(AsyncResponseStream *stream, sensor::Sensor *obj) {
  ...
  if (!std::isnan(obj->state)) {
    ...
    stream->print(relabel_(obj->get_object_id()).c_str());
    stream->print(F("\",name=\""));
    stream->print(relabel_(obj->get_name()).c_str());
    stream->print(F("\"} 0\n"));
    // Data itself
