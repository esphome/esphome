...
std::string PrometheusHandler::relabel_id(auto obj) {
  auto item = relabel_map_id_.find(obj->get_object_id());
  return item == relabel_map_id_.end() ? obj.get_object_id() : item->second;
}

std::string PrometheusHandler::relabel_name(auto obj) {
  auto item = relabel_map_name_.find(obj->get_object_id());
