    ...
    return cv.Schema(
        {
            ...
            cv.Optional(CONF_CHANNEL, default=default_channel): cv.one_of(
                CONF_MONO, CONF_LEFT, CONF_RIGHT, CONF_STEREO
            ),
            ...
            cv.Optional(CONF_BITS_PER_SAMPLE, default=default_bits_per_sample): cv.All(
                _validate_bits, cv.one_of(*I2S_SLOT_BIT_WIDTH)
            ),
...
def validate_use_legacy(value):
    ...
    if CONF_USE_LEGACY in value:
        if value[CONF_USE_LEGACY]:
            raise cv.Invalid(
                "The legacy I2S driver is no longer supported. "
                "Arduino-ESP32 3.x and ESP-IDF 5.0+ use the same modern I2S driver. "
                "Please remove 'use_legacy: true' from your configuration."
            )
        _use_legacy_driver = False
    return value
