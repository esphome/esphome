...
def _final_validate(config):
    if config[CONF_DAC_TYPE] == "internal":
        raise cv.Invalid(
            "Internal DAC is only compatible with the legacy i2s driver which has been removed. "
            "The legacy I2S driver is no longer supported in Arduino-ESP32 3.x."
        )
    if config[CONF_I2S_COMM_FMT] == "stand_max":
        raise cv.Invalid(
            "I2S standard max format is only implemented with the legacy i2s driver which has been removed."
        )
