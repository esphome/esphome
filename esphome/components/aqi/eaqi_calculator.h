...
public:
  ...
  uint16_t calculate_eaqi(float pm2_5, float pm10, float o3, float no2, float so2) {
    ...
    // The overall EAQI is the maximum (poorest) of the individual indices
...
protected:
  ...
  static constexpr float PM2_5_GRID[NUM_LEVELS][2] = {
    ...
    {50.0f, 90.0f},
    {90.0f, 140.0f},
    {140.0f, std::numeric_limits<float>::max()}
  };

  // PM10 Breakpoints (µg/m³)
  static constexpr float PM10_GRID[NUM_LEVELS][2] = {
    {0.0f, 15.0f},
    {15.0f, 45.0f},
    {45.0f, 120.0f},
    {120.0f, 195.0f},
    {195.0f, 270.0f},
    {270.0f, std::numeric_limits<float>::max()}
  };

  // O3 Breakpoints (µg/m³)
  static constexpr float O3_GRID[NUM_LEVELS][2] = {
    {0.0f, 60.0f},
    {60.0f, 100.0f},
    {100.0f, 120.0f},
    {120.0f, 160.0f},
    {160.0f, 180.0f},
    {180.0f, std::numeric_limits<float>::max()}
  };

  // NO2 Breakpoints (µg/m³)
  static constexpr float NO2_GRID[NUM_LEVELS][2] = {
    {0.0f, 10.0f},
    {10.0f, 25.0f},
    {25.0f, 60.0f},
    {60.0f, 100.0f},
    {100.0f, 150.0f},
    {150.0f, std::numeric_limits<float>::max()}
  };

  // SO2 Breakpoints (µg/m³)
  static constexpr float SO2_GRID[NUM_LEVELS][2] = {
    {0.0f, 20.0f},
    {20.0f, 40.0f},
    {40.0f, 125.0f},
    {125.0f, 190.0f},
    {190.0f, 275.0f},
    {275.0f, std::numeric_limits<float>::max()}
  };

  static float calculate_index(float value, const float array[NUM_LEVELS][2]) {
    if (value < 0) return 0.0f; // Handle invalid negative values
    ...
    // For EAQI, since aqi_lo == aqi_hi, this effectively returns the level index
    // unless value falls exactly on a boundary where interpolation matters.
    if (conc_hi == conc_lo) return aqi_lo;
  ...
  static int get_grid_index(float value, const float array[NUM_LEVELS][2]) {
    for (int i = 0; i < NUM_LEVELS; i++) {
      // Check range: inclusive lower bound, exclusive upper bound (except last)
      bool in_range = (value >= array[i][0]) && 
                      ((i == NUM_LEVELS - 1) ? true : (value < array[i][1]));
