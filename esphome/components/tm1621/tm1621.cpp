...
//    AddLog(LOG_LEVEL_DEBUG, PSTR("TM1: Dump%d %4_H"), j +1, row);
  ...
  if (this->fahrenheit) { buffer[1] |= 0x80; }
  if (this->celsius) { buffer[3] |= 0x80; }
  if (this->kwh) { buffer[4] |= 0x08; }
  if (this->humidity) { buffer[6] |= 0x08; }
  if (this->voltage) { buffer[7] |= 0x08; }
...
int TM1621Display::GetCommandCode(char* destination, size_t destination_size, const char* needle, const char* haystack) {
