...
void PylontechComponent::process_line_(std::string &buffer) {
  ...
  char base_st[TEXT_SENSOR_MAX_LEN], volt_st[TEXT_SENSOR_MAX_LEN], curr_st[TEXT_SENSOR_MAX_LEN], temp_st[TEXT_SENSOR_MAX_LEN];
  const int parsed = sscanf(buffer.c_str(), "%d %d %d %d %d %d %d %d %7s %7s %7s %7s %d%% %*d-%*d-%*d %*d:%*d:%*d %*s %*s %d %*s",  // NO LINT
                        &bat_num, &volt, &curr, &tempr, &tlow, &thigh, &vlow, &vhigh, base_st, volt_st,                             // NO LINT
                        curr_st, temp_st, &coulomb, &mostempr);                                                                     // NO LINT
