...
void OpenThreadComponent::ot_main() {
  ...
  if (this->poll_period > 0) {
    ...
    ESP_LOGI(TAG, "Link Polling Period: %d", link_polling_period);
  }
  ...
  ESP_LOGI(TAG, "Link Mode Device Type: %s", link_mode_config.mDeviceType ? "true" : "false");
  ESP_LOGI(TAG, "Link Mode Network Data: %s", link_mode_config.mNetworkData ? "true" : "false");
  ESP_LOGI(TAG, "Link Mode RX On When Idle: %s", link_mode_config.mRxOnWhenIdle ? "true" : "false");
