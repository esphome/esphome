...
struct gsl_DISTANCE_TYPE {
  uint32_t i;
  uint32_t j;
  uint32_t min;                     /* distance min */
  uint32_t d[POINT_MAX][POINT_MAX]; /* distance; */
};
...
union gsl_FLAG_TYPE {
  struct {
    uint32_t over_report_mask : 1;
    uint32_t opposite_x : 1;
    uint32_t opposite_y : 1;
    uint32_t opposite_xy : 1;
    uint32_t line : 1;
    uint32_t line_neg : 1;
    uint32_t line_half : 1;
    uint32_t middle_drv : 1;

    uint32_t key_only_one : 1;
    uint32_t key_line : 1;
    uint32_t refe_rt : 1;
    uint32_t refe_var : 1;
    uint32_t base_median : 1;
    uint32_t key_rt : 1;
    uint32_t refe_reset : 1;
    uint32_t sub_cross : 1;

    uint32_t row_neg : 1;
    uint32_t sub_line_coe : 1;
    uint32_t sub_row_coe : 1;
    uint32_t c2f_able : 1;
    uint32_t thumb : 1;
    uint32_t graph_h : 1;
    uint32_t init_repeat : 1;
    uint32_t near_reset_able : 1;

    uint32_t emb_dead : 1;
    uint32_t emb_point_mask : 1;
    uint32_t interpolation : 1;
    uint32_t sum2_able : 1;
    uint32_t reduce_pin : 1;
    uint32_t drv_order_ex : 1;
    uint32_t id_over : 1;
    uint32_t rev_1 : 1;
  } other;
  uint32_t all;
...
union gsl_ID_FLAG_TYPE {
  struct {
    uint32_t reso_y : 1;
    uint32_t reso_x : 1;
    uint32_t screen_core : 1;
    uint32_t screen_real : 1;
    uint32_t ignore_pri : 1;
    uint32_t id_prec_able : 1;
    uint32_t first_avg : 1;
    uint32_t round : 1;

    uint32_t stretch_off : 1;
    uint32_t rev_7 : 7;
...
static uint32_t pressure_now[POINT_MAX];
static uint32_t pressure_array[PRESSURE_DEEP][POINT_MAX];
static uint32_t pressure_report[POINT_MAX];
static uint32_t *pressure_pointer[PRESSURE_DEEP];
...
static uint32_t csensor_count;
static int point_n;
static int point_num;
static int prev_num;
static int point_near;
static uint32_t point_shake;
static uint32_t reset_mask_send;
static uint32_t reset_mask_max;
static uint32_t reset_mask_count;
static gsl_FLAG_TYPE global_flag;
static gsl_ID_FLAG_TYPE id_flag;
static uint32_t id_first_coe;
static uint32_t id_speed_coe;
static uint32_t id_static_coe;
static uint32_t average;
static uint32_t soft_average;
static uint32_t report_delay;
static uint32_t delay_key;
static uint32_t report_ahead;
static uint32_t report_delete;
static unsigned char median_dis[4];
static uint32_t shake_min;
...
static uint32_t drv_num;
static uint32_t sen_num;
static uint32_t drv_num_nokey;
static uint32_t sen_num_nokey;
static uint32_t coordinate_correct_able;
static uint32_t coordinate_correct_coe_x[64];
static uint32_t coordinate_correct_coe_y[64];
static uint32_t edge_cut[4];
static uint32_t stretch_array[4 * 4 * 2];
static uint32_t stretch_active[4 * 4 * 2];
static uint32_t shake_all_array[2 * 8];
static uint32_t edge_start;
static uint32_t reset_mask_dis;
static uint32_t reset_mask_type;
static uint32_t key_map_able;
static uint32_t key_range_array[8 * 3];
static int filter_able;
static uint32_t filter_coe[4];
static uint32_t multi_x_array[4], multi_y_array[4];
static uint32_t multi_group[4][64];
...
static uint32_t press_move;
/* uint32_t key_dead_time			; */
/* uint32_t point_dead_time		; */
/* uint32_t point_dead_time2		; */
/* uint32_t point_dead_distance	; */
/* uint32_t point_dead_distance2	; */
/* uint32_t pressure_able; */
/* uint32_t pressure_save[POINT_MAX]; */
static uint32_t edge_first;
static uint32_t edge_first_coe;
static uint32_t point_corner;
static uint32_t stretch_mult;
/* ------------------------------------------------- */
static uint32_t config_static[CONFIG_LENGTH];
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
static uint32_t CC128(uint32_t x, uint32_t **coe, int k) {
  if (k == 3) {
...
static uint32_t CCO(uint32_t x, uint32_t coe[], int k) {
  if (k == 0) {
    if (x & 32)
...
#endif
  int i, j;
  uint32_t *px[LINE_SIZE + 1], *py[LINE_SIZE + 1];
  struct MULTI_TYPE multi_x[LINE_SIZE], multi_y[LINE_SIZE];
  uint32_t edge_size = 64;
  int kx, ky;
...
static void PointPredictOne(uint32_t n) {
  pp[0][n].all = pp[1][n].all & FLAG_COOR;
  pp[0][n].other.predict = 0;
}

static void PointPredictD2(uint32_t n) {
  int x, y;
...
static void PointPredictTwo(uint32_t n) {
  int x, y;
...
static void PointPredictSpeed(uint32_t n) {
  int x, y;
...
static void PointPredictD3(uint32_t n) {
  int x, y;
...
static void PointPredictThree(uint32_t n) {
  int x, y;
...
static uint32_t PointDistance(gsl_POINT_TYPE *p1, gsl_POINT_TYPE *p2) {
  int a, b, ret;
...
static uint32_t PointMOne(uint32_t x0, uint32_t x1) {
  int e1, e2;
...
static uint32_t FilterSpeed(int i) {
...
static uint32_t KeyMap(int *drv, int *sen) {
  ...
  for (i = 0; i < 8; i++) {
    if ((uint32_t) *drv >= (key_range[i].up_down >> 16) && (uint32_t) *drv <= (key_range[i].up_down & 0xffff) &&
        (uint32_t) *sen >= (key_range[i].left_right >> 16) && (uint32_t) *sen <= (key_range[i].left_right & 0xffff)) {
...
static uint32_t ScreenResolution(gsl_POINT_TYPE *p) {
  int x, y;
  ...
  for (i = 0; i < POINT_MAX; i++) {
    ...
    if (i == 0) {
      if (edge_first != 0 && ps[1][i].all == 0) {
        ...
        if (point_edge.coor.other.x < (uint32_t) ((edge_first >> 24) & 0xff))
        ...
        if (point_edge.coor.other.y < (uint32_t) ((edge_first >> 8) & 0xff))
    ...
    if (ps[1][i].all == 0) {
      ...
      if (PointDistance(&ps[0][i], &ps[j][i]) >= (uint32_t) shake_min * 4) {
        point_delay[i].other.init = 1;
        point_delay[i].other.able = 1;
        point_delay[i].other.report = 1;
        point_delay[i].other.dele = 1;
      ...
      if (PointDistance(&ps[0][i], &ps[1][i]) < (uint32_t) shake_min) {
...
static void PressMask(void) {
  int i, j;
  uint32_t press_max = press_mask & 0xff;
  uint32_t press_range_s = (press_mask >> 8) & 0xff;
  uint32_t press_range_d = (press_mask >> 16) & 0xff;
  uint32_t press_range;
...
static void PressMove(void) {
  ...
  /* uint32_t press_move=0x01000010; */
  if (press_move == 0)
    return;
  if (pr[0][0].all == 0)
    goto press_move_err;
...
void gsl_ReportPressure(uint32_t *p) {
  int i;
...
uint32_t gsl_version_id(void) { return GSL_VERSION; }

uint32_t gsl_mask_tiaoping(void) {
...
static void GetFlag(void) {
  ...
  inte_count++;
  csensor_count = ((uint32_t) point_num) >> 16;
