...
//    keep std::atomic).
//  - ESPHOME_THREAD_SINGLE: one thread on one in-order core, so the only
//    possible concurrency is same-core interrupt preemption (on RP2 the
//    BTstack packet handler runs in the CYW43 async-context low-priority IRQ
//    on the same core as the main loop). Using plain accesses here also
//    avoids __atomic_* library calls on RP2040 (Cortex-M0+, no LDREX/STREX).
